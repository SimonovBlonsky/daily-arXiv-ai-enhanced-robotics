...
Result: 仿真结果表明，与传统PID和单一避障算法相比，该方法在密集干扰场景下实现了98%的避障成功率，显著提高了路径平滑度，并将有效观测数据量增加了约27%。

Conclusion: 该研究为复杂光照环境下的精确无人机水质监测提供了一种有效的工程解决方案，能够有效应对动态光照阴影干扰，提高数据质量和飞行安全性。
...
TL;DR: PaceForecaster通过结合人类指令预测环境地图和子目标，帮助机器人在复杂环境中更果断地规划路径，在保持安全的同时提高导航性能36%。


<details>
  <summary>Details</summary>
Motivation: 在杂乱环境中，运动规划器通常面临安全性与速度之间的权衡，主要由于遮挡和有限传感器范围带来的不确定性。本研究探讨人类指令能否帮助机器人更果断地规划路径同时保持安全。

Method: 提出PaceForecaster方法，将人类指令整合到局部规划器中。该方法以机器人的局部传感器覆盖范围（Level-1）和人类指令作为输入，预测：(i) 从Level-1可见的所有区域的预测地图（Level-2），(ii) 基于指令条件化的Level-2内子目标。子目标为规划器提供明确指导，以目标导向方式利用预测环境。

Result: 将PaceForecaster与Log-MPPI控制器集成，实验表明使用语言条件化预测和目标的导航性能比仅使用局部地图的基线提高了36%（在多边形环境中）。

Conclusion: 人类指令辅助的预测方法能够显著提升机器人在复杂环境中的导航性能，通过预测未来可见区域和设定指令条件化子目标，帮助规划器在安全前提下做出更果断的决策。
...
<details>
  <summary>Details</summary>
Motivation: 随着无人机密度增加，城市空中交通(UAM)预计会出现类似地面交通的拥堵问题，但目前对UAM交通流的基本特性，特别是在真实操作条件下的特性，仍然缺乏深入理解。

Method: 提出一个构建UAM交通基本图的通用框架，包括：1)设计两种无人机碰撞避免控制律；2)开发基于仿真的交通生成方法；3)应用交通流理论构建基本图；4)在缩比实验平台上进行物理实验验证；5)收集数据构建UAMTra2Flow数据集。

Result: 研究发现：1)地面交通的经典基本图结构同样适用于UAM系统；2)物理实验获得的基本图曲线与仿真结果存在偏差，凸显了实验验证的重要性；3)通过缩比实验结果推算出真实操作条件下的实用见解。

Conclusion: 该研究首次使用真实物理测试数据推导出UAM基本图，为理解UAM交通流特性提供了重要框架，数据集和代码已公开，对未来UAM交通系统规划具有实用价值。
...
TL;DR: EVE框架通过生成-验证机制提升预训练生成策略的性能，无需额外训练，在推理时使用零样本VLM验证器改进动作预测


<details>
  <summary>Details</summary>
Motivation: 当前基于生成架构的视觉运动策略在分布偏移下性能下降，缺乏恢复能力，而语言模型领域通过测试时计算扩展显著提升了推理能力。本文假设生成策略同样可以通过零样本VLM验证器在推理时受益于额外计算

Method: 提出EVE框架：将冻结的基础策略与多个零样本VLM验证器封装，验证器提出动作改进建议，动作整合器融合验证器输出与基础策略预测，生成最终执行动作

Result: 在多样化操作任务套件中，EVE持续提升任务成功率，无需额外策略训练。通过消融实验验证了验证器能力和动作整合策略的贡献
...
TL;DR: 该研究创建了首个基于火星和月球地形的大型路径规划基准数据集MarsPlanBench和MoonPlanBench，并在统一框架下评估了经典和基于学习的路径规划算法在行星地形上的表现。
...
Conclusion: 该研究为行星路径规划研究提供了首个标准化基准，解释了NASA在实践中使用经典算法的原因，并指出了学习模型在行星领域泛化方面的挑战，为后续研究奠定了基础。
...
TL;DR: 将时空管框架扩展到动态不确定环境中的概率时间可达-规避-停留任务，开发实时管合成方法并提供形式化概率安全保证


<details>
  <summary>Details</summary>
Motivation: 在动态环境中处理不确定障碍物时的概率时间可达-规避-停留任务需要形式化安全保证和实时执行能力

Method: 将时空管建模为状态空间中的时变球体，基于不确定传感信息在线更新中心点和半径，推导闭式无近似控制律将系统轨迹限制在管内

Result: 实现了概率规避和有限时间任务完成的正式保证，控制器无模型、无近似、无优化，支持实时高效执行并保证收敛到目标

Conclusion: 该方法在移动机器人、无人机和7自由度机械臂的仿真和硬件实验中展示了有效性和可扩展性，适用于杂乱不确定环境
...
TL;DR: 提出了一种新型螺旋缠绕结构静电层卡滞（HWS-ELJ）可变刚度机制，应用于机器人手指设计，通过静电吸引力增强层间摩擦实现刚度调节，相比传统平面ELJ具有指数级刚度增强效果。


<details>
  <summary>Details</summary>
Motivation: 开发一种更高效的可变刚度机制用于机器人手指设计，解决传统平面静电层卡滞（ELJ）刚度调节能力有限、占用空间大的问题，实现电压控制下的精确刚度调节。

Method: 采用螺旋缠绕结构的静电层卡滞（HWS-ELJ）机制，利用静电吸引力增强层间摩擦，抑制相对滑动；通过改变螺旋缠绕角度实现指数级刚度调节；设计并测试了集成该机制的机器人手指原型。

Result: HWS-ELJ相比传统平面ELJ，在相同电极接触面积下能实现显著更大的刚度增强，在相同刚度条件下减少所需占用空间；实验结果显示其刚度调节特性符合理论推导趋势；机器人手指原型成功实现了电压驱动的刚度调节。

Conclusion: HWS-ELJ是一种有效的可变刚度机制，具有优异的刚度调节性能和空间效率，适用于机器人手指等应用场景，验证了电压控制可变刚度机制的可行性。
...
TL;DR: 提出一种轻量级框架，从单目视频中恢复世界坐标系人体运动并重定向到人形机器人，使用SAM 3D Body作为感知骨干和Momentum HumanRig作为机器人友好中间表示，通过滑动窗口优化和物理约束实现稳定运动重建。


<details>
  <summary>Details</summary>
Motivation: 从单目视频恢复世界坐标系人体运动对于具身智能和机器人学很重要，但现有方法通常需要复杂的SLAM流程或重型时序模型，希望开发更轻量、工程化的解决方案。

Method: 使用SAM 3D Body作为冻结感知骨干，Momentum HumanRig作为机器人友好中间表示。方法包括：(1)锁定身份和骨架尺度参数确保时序一致的骨骼长度；(2)在低维MHR潜在空间通过高效滑动窗口优化平滑每帧预测；(3)使用可微软足地接触模型和接触感知全局优化恢复物理合理的全局根轨迹；(4)通过运动学感知的两阶段逆运动学管道将重建运动重定向到Unitree G1人形机器人。

Result: 在真实单目视频上的结果显示，该方法具有稳定的世界轨迹和可靠的机器人重定向效果，表明结构化人体表示结合轻量级物理约束可以从单目输入产生机器人就绪的运动。

Conclusion: 结构化人体表示配合轻量级物理约束能够有效从单目视频中恢复机器人可用的运动，避免了复杂SLAM流程和重型时序模型，为机器人运动重定向提供了工程友好的解决方案。
...
TL;DR: 本文提出了一种结构诱导探索框架，将结构先验集成到蚁群优化中，用于多机器人路径规划，以平衡全局效率和个体工作量分配。


<details>
  <summary>Details</summary>
Motivation: 多机器人路径规划具有组合复杂性，需要在全局效率和机器人间公平任务分配之间取得平衡。传统的群体智能方法在小规模实例上有效，但容易过早收敛，难以扩展到复杂环境。

Method: 提出结构诱导探索框架：1) 利用任务空间分布初始化结构先验，约束搜索空间；2) 设计信息素更新规则，强调结构上有意义的连接；3) 引入负载感知目标，协调总行程距离与个体机器人工作量；4) 采用显式重叠抑制策略，确保任务在团队中保持区分和平衡。

Result: 在多种基准场景（不同实例规模和机器人团队配置）上进行验证，结果显示在路径紧凑性、稳定性和工作量分布方面相比代表性元启发式基线方法有持续改进。

Conclusion: 该方法不仅提高了性能，还提供了一个可扩展且可解释的框架，可应用于物流、监视和搜救等需要可靠大规模协调的应用场景。
...
TL;DR: MAction-SocialNav是一个用于社会合规导航的高效视觉语言模型，通过处理动作模糊性来生成单个场景中的多个合理动作，并引入元认知提示方法增强推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有社会导航方法通常假设每个场景只有一个正确动作，无法处理现实世界中的社会不确定性，因为社会规范往往是模糊的，单个场景中可能有多个同样可接受的动作。

Method: 提出MAction-SocialNav模型，引入元认知提示方法增强推理能力，并构建包含789个样本的多动作社会合规导航数据集，每个样本包含三轮对话，涵盖不同人群密度、室内外环境和双人标注。

Result: 模型在决策质量（APG：0.595 vs. 0.000/0.025）和安全对齐（ER：0.264 vs. 0.642/0.668）方面显著优于零样本GPT-4o和Claude，同时保持实时效率（1.524 FPS，快3倍以上）。

Conclusion: MAction-SocialNav通过显式处理动作模糊性，在保持高效率的同时实现了强大的社会推理性能，展示了在现实世界人机导航中的潜力。
...
TL;DR: MoonBot是一种模块化可重构机器人系统，专为月球表面探索和开发设计，能在严格的质量约束下适应不同环境条件和任务需求，通过现场演示验证了其在月球基础设施建立任务中的可行性。


<details>
  <summary>Details</summary>
Motivation: 月球表面探索和开发受到全球广泛关注，机器人对于探索未知地形、发现利用当地资源以及促进未来人类栖息地建设至关重要。需要一种能在严格质量约束下适应不同任务需求的机器人系统。

Method: 设计开发了模块化按需可重构机器人系统MoonBot，采用模块化架构允许根据任务需求重新配置。通过初步现场演示验证概念，执行模拟月球基础设施建立的里程碑任务，包括土木工程操作、基础设施组件运输部署以及与充气模块的辅助操作。

Result: 成功演示了MoonBot在模拟月球基础设施建立任务中的能力，包括土木工程操作、组件运输部署和辅助操作。系统总结了测试中的经验教训，特别关注连接器设计，为未来月球任务中模块化机器人系统的发展提供了宝贵见解。

Conclusion: MoonBot作为一种模块化可重构机器人系统，展示了在月球探索和开发中的潜力，能够适应严格的质量约束和多样化任务需求。测试经验为未来月球任务中模块化机器人系统的改进和发展提供了重要指导。
...
TL;DR: 提出了一种基于非线性优化的轨迹规划方法，用于空间碎片清除任务中接近翻滚目标的近距离交会，引入动态禁区球体提高安全性，并开发了考虑实际约束的ON/OFF推进器控制策略。


<details>
  <summary>Details</summary>
Motivation: 空间碎片清除任务中，使用机械臂的服务卫星安全接近翻滚目标是一个关键挑战，需要将自由漂浮、旋转的碎片物体引入机械臂工作空间，为后续捕获做准备。

Method: 提出基于非线性优化的轨迹规划方法，引入动态禁区球体根据接近条件自适应调整，允许更近更安全地接近目标；开发了使用离散ON/OFF推进器的控制策略来复现优化轨迹。

Result: 该方法能够实现安全接近翻滚空间碎片目标，动态禁区球体提高了接近过程的安全性，ON/OFF推进器控制策略考虑了实际工程约束。

Conclusion: 提出的轨迹规划和控制方法为空间碎片清除任务中的安全接近操作提供了有效解决方案，动态禁区球体和实际推进器约束的考虑增强了方法的实用性和安全性。
...
TL;DR: 提出了一种用于自由漂浮基座机器人的未知目标物体惯性参数在线估计方法，通过动量守恒原理扩展现有识别方法，适用于在轨服务等空间任务。


<details>
  <summary>Details</summary>
Motivation: 在空间机器人等自由漂浮基座系统中，掌握被抓取物体的惯性参数对于动力学感知的操控至关重要，特别是在在轨服务等空间任务中。

Method: 应用并扩展现有的在线识别方法，通过融入动量守恒原理，使其适用于自由漂浮基座机器人系统。

Result: 通过数值仿真验证了所提方法的有效性，估计参数与真实值对比显示在测试场景中实现了准确识别。

Conclusion: 该方法能够准确识别未知物体的惯性参数，证明了其在在轨服务和其他空间任务中的适用性。
...
TL;DR: ANWM是一种无人机导航世界模型，通过预测未来视觉观测来评估轨迹的语义合理性和导航效用，提升无人机在大规模3D环境中的自主导航能力。


<details>
  <summary>Details</summary>
Motivation: 现有无人机导航策略通常只优化低层目标（如避障和轨迹平滑），缺乏将高层语义融入规划的能力。需要弥合这一差距，使无人机能够基于语义信息进行更智能的导航决策。

Method: 提出ANWM（空中导航世界模型），基于过去帧和动作预测未来视觉观测。引入物理启发的未来帧投影（FFP）模块，将过去帧投影到未来视点以提供粗略几何先验，减少长距离视觉生成的表示不确定性，并捕捉3D轨迹与自我中心观测之间的映射关系。

Result: ANWM在长距离视觉预测方面显著优于现有世界模型，并提高了无人机在大规模环境中的导航成功率。

Conclusion: ANWM通过结合语义理解和物理先验，为无人机导航提供了更智能的决策框架，能够有效处理大规模3D环境中的复杂导航任务。
...
TL;DR: 提出模块化扩散策略框架，通过分解复杂动作分布为多个专门扩散模型，提高机器人多任务学习性能
...
Method: 引入模块化扩散策略框架，将复杂动作分布分解为多个专门扩散模型的组合，每个模型捕捉行为空间的不同子模式

Result: 在仿真和真实世界机器人操作场景中，该方法持续优于现有的模块化和单一模型基线方法

Conclusion: 模块化扩散策略框架能更有效地拟合复杂动作分布，其模块化结构支持灵活的策略适应新任务，同时缓解灾难性遗忘问题
...
<details>
  <summary>Details</summary>
Motivation: 立体视觉能提供丰富的空间线索，对机器人精确操作至关重要，但在现有的视觉-语言-动作模型中尚未得到充分探索。目前大多数VLAs仅使用单目视觉，缺乏深度感知能力。

Method: 提出StereoVLA模型，包含：1）几何-语义特征提取模块，利用视觉基础模型提取并融合立体视图差异的几何特征和单目视图的语义特征；2）交互区域深度估计辅助任务，增强空间感知并加速模型收敛。

Result: 在立体视觉设置下的多样化任务中，StereoVLA大幅超越基线模型，表现出对相机姿态变化的强鲁棒性，验证了立体视觉在VLA模型中的有效性。

Conclusion: StereoVLA成功将立体视觉几何线索整合到VLA模型中，显著提升了空间感知和操作性能，为机器人视觉-语言-动作系统提供了新的研究方向。
...
TL;DR: BRIS是一个紧凑的人机协同机器人插管系统，集成了四向可操纵光纤支气管镜、独立的气管导管推进机制和摄像头增强口器，通过实时形状传感和单目内窥镜深度估计提供直观的远程操作和解剖感知引导。


<details>
  <summary>Details</summary>
Motivation: 现有机器人插管系统主要关注气道导航，缺乏对气管导管推进的集成控制和相对于隆突的客观深度验证，导致插管失败或导管位置不当可能引发严重并发症。

Method: 系统整合了四向可操纵光纤支气管镜、独立导管推进机制和摄像头增强口器。采用学习型闭环控制框架，利用实时形状传感将操纵杆输入映射到支气管镜尖端在笛卡尔空间的运动。使用单目内窥镜深度估计对气道区域进行分类，提供可解释的解剖感知引导。

Result: 在高保真气道模型上进行验证，包括标准和困难气道配置，系统展示了可靠的气道导航和可控的导管放置性能。

Conclusion: BRIS代表了向更安全、更一致且临床兼容的机器人气道管理迈出的重要一步，通过集成导航、导管推进和深度验证功能，提高了插管的安全性和可靠性。
