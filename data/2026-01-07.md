...
### [1] [Modeling the Mental World for Embodied AI: A Comprehensive Review](https://arxiv.org/abs/2601.02378)
*Biyuan Liu,Daigang Xu,Lei Jiang,Wenjun Guo,Ping Chen*

Main category: cs.RO

TL;DR: 该论文综述了面向具身AI的心理世界模型研究，系统梳理了心理世界模型的理论框架、核心组件、心智理论推理范式及评估基准，旨在推动具身智能体更好地融入人类社会。


<details>
  <summary>Details</summary>
Motivation: 随着具身AI在虚拟化身、可穿戴设备和机器人系统中的深入应用，研究重点已从物理环境交互转向对社会互动的准确理解。传统物理世界模型无法满足社会智能建模需求，而心理世界模型作为人类内部心理状态的结构化表示，是实现自然人机协作和动态社会适应的关键认知基础。当前心理世界模型研究面临概念框架碎片化、推理机制脱节、评估与实践分离等瓶颈。

Method: 通过系统综述100多项权威研究，构建完整的心理世界模型理论框架：1）明确区分心理世界模型与物理世界模型的本质差异；2）通过两种心理元素表征范式系统定义心理世界模型的关键组件；3）全面分析两种核心心智理论推理范式及19种具体方法；4）阐明神经符号混合架构的融合趋势；5）综合26个心智理论评估基准。

Result: 首次构建了完整的心理世界模型理论框架，系统定义了心理世界模型的关键组件，全面分析了心智理论推理范式和方法，阐明了神经符号混合架构的融合趋势，并综合了心智理论评估基准体系。

Conclusion: 该综述为具身AI的心理世界模型研究提供了系统性的理论框架和方法论指导，有助于推动具身智能体更好地理解人类心理状态，实现更自然的人机协作和社会适应，促进具身智能体融入人类社会，推动人机协同交互的深入发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型擅长语义理解但缺乏物理世界动态推理能力，而基于视频预测的世界模型又缺乏语义基础且对预测错误敏感。需要结合两者的优势，实现语义理解与动态预测的协同。

Method: 采用统一的混合专家Transformer架构，协调三个专家模块：场景理解、视觉预测生成和动作执行。基于InternVL3和Qwen3-VL构建2B和3B参数模型，在混合合成-真实数据集上进行预训练，涵盖超过5.33亿帧数据。

Result: 在12个真实世界机器人任务和仿真基准测试中，InternVLA-A1显著优于pi0和GR00T N1.5等领先模型，日常任务提升14.5%，动态场景（如传送带分拣）提升40%-73.3%。

Conclusion: InternVLA-A1成功结合了语义理解和动态预测能力，通过混合专家架构和混合训练策略，在真实世界机器人任务中表现出色，为VLA模型的发展提供了新方向。
...
TL;DR: STEAM问题框架结合E-ITAGS算法，通过特征效能映射优化多机器人任务分配，同时考虑时空约束，并利用可实现性感知的主动学习模块高效学习效能映射。


<details>
  <summary>Details</summary>
Motivation: 复杂多机器人任务需要异构团队在严格约束下联合优化任务分配、调度和路径规划以提高团队性能。现有基于特征的框架通常采用二元成功-失败模型，无法建模任务性能如何随分配能力变化。

Method: 提出STEAM问题框架，引入特征效能映射来编码分配能力如何决定任务性能。开发E-ITAGS算法，通过交织任务分配、调度和路径规划来同时优化任务性能并遵守时间预算。采用可实现性感知的主动学习模块高效学习特征效能映射。

Result: 数值模拟和应急响应领域的实验表明，E-ITAGS相比基线方法能生成更高效能的任务分配，同时尊重资源和时空约束。主动学习方法具有样本效率，并在数据效率和计算效率之间建立了原则性权衡。

Conclusion: STEAM框架和E-ITAGS算法有效解决了复杂多机器人任务中的联合优化问题，通过特征效能映射和可实现性感知学习，在满足时空约束的同时显著提高了任务分配效能。
...
TL;DR: BRiDGE：一种用于三维环境中导航的采样规划器，通过移动物体搭建桥梁连接不可达区域


<details>
  <summary>Details</summary>
Motivation: 现有路径规划器假设目标区域可达，但在实际三维环境中，由于高度差、孔洞或横向分离，表面可能相互断开，导致目标区域无法到达。需要让机器人能够与环境互动，重新排列可移动物体来创建新的可穿越连接。

Method: 开发了BRiDGE（基于块的三维几何环境断开连接重构）采样规划器，通过增量构建机器人和物体配置的树结构，计算可行计划，指定移动哪些物体、放置位置和顺序，同时考虑有限数量的可移动物体。引入非均匀采样策略加速规划。

Result: 该方法被证明是概率完备的，并通过大量数值和硬件实验验证了其有效性。

Conclusion: BRiDGE能够有效解决三维环境中导航可移动物体的问题，通过战略性地放置障碍物来桥接物理断开，使机器人能够到达原本不可达的目标区域。
...
<details>
  <summary>Details</summary>
Motivation: 在线3D装箱在物流中至关重要，但现有深度强化学习方法难以适应现实世界中的短期分布偏移问题，即不同批次货物顺序到达导致的性能下降。作者认为现代物流系统中的短期前瞻信息是解决这一问题的关键。

Method: 将带前瞻包裹的在线3D装箱问题建模为模型预测控制问题，采用蒙特卡洛树搜索框架求解。设计动态探索先验来自动平衡学习到的RL策略和鲁棒随机策略，同时设计辅助奖励来惩罚单个放置产生的长期空间浪费。

Result: 在真实世界数据集上的大量实验表明，该方法在分布偏移下获得超过10%的性能提升，在线部署平均改进4%，最佳情况下提升超过8%，显著优于现有最先进方法。

Conclusion: 提出的结合前瞻信息的框架有效解决了在线3D装箱中的分布偏移问题，通过模型预测控制和蒙特卡洛树搜索实现了更好的适应性和性能，证明了前瞻信息在现代物流系统中的重要性。
...
TL;DR: DCBF是一种可组合的对象中心安全函数，通过学习物理交互的安全约束，实现密集杂乱环境中机器人的安全导航和接触式操作，无需重新训练即可扩展到任意对象集。


<details>
  <summary>Details</summary>
Motivation: 传统安全框架将接触视为不安全，限制了机器人在密集日常环境中的功能。随着物体数量增加，基于模型的方法计算不可行，而学习方法通常与特定任务绑定，难以迁移到新任务。

Method: 提出密集接触屏障函数(DCBF)，通过离线学习少量物体交互的可组合对象中心函数，隐式捕捉物理交互产生的安全约束。运行时可将DCBF组合到任意物体集，生成单一全局安全过滤器。

Result: 在密集杂乱环境的模拟实验中验证了DCBF的能力，能够实现无碰撞导航和在合适环境下的安全接触式交互，且计算复杂度线性扩展。

Conclusion: DCBF提供了一种可扩展、可迁移的方法，使机器人能够在密集日常环境中安全运行，克服了传统安全框架和现有方法的局限性。
...
TL;DR: 该研究通过多目标优化方法探讨了6-7自由度机械臂的最优结构设计，从末端执行器可达性和关节扭矩两个角度进行分析，为未来机械臂设计提供指导。


<details>
  <summary>Details</summary>
Motivation: 虽然已有多种6-7自由度机械臂被开发，但其关节配置和连杆长度比例通常基于经验确定。随着机器人基础模型的发展，需要支持这些模型的各种机械臂，但现有机械臂结构各不相同。为了讨论机械臂的最优结构，需要进行系统性的优化分析。

Method: 采用多目标优化方法，从末端执行器可达性和关节扭矩两个关键性能指标出发，对机械臂结构进行优化。通过采样分析现有机械臂结构在优化结果中的位置。

Result: 分析了现有机械臂结构在优化采样结果中的位置分布，揭示了现有设计在优化空间中的相对位置和性能表现。

Conclusion: 研究为未来机械臂设计提供了重要见解，通过系统性的多目标优化方法可以指导机械臂结构的改进和创新设计。
...
TL;DR: 提出了一种改进DPV-SLAM中闭环检测性能的方法，用基于学习的AnyLoc视觉位置识别技术替代传统的词袋模型，并引入自适应相似度阈值机制


<details>
  <summary>Details</summary>
Motivation: 闭环检测对于视觉SLAM的准确性和一致性至关重要，传统基于词袋模型的方法依赖手工特征，在不同视角和光照条件下鲁棒性有限，需要手动调参

Method: 1) 用基于深度特征表示的AnyLoc视觉位置识别技术替代传统的词袋模型闭环检测方法；2) 提出自适应机制，根据环境条件动态调整相似度阈值，无需手动调参

Result: 在室内和室外数据集上的实验表明，该方法在闭环检测准确性和鲁棒性方面显著优于原始DPV-SLAM

Conclusion: 该方法为现代SLAM系统中增强闭环检测性能提供了实用且可扩展的解决方案
...
TL;DR: 提出自监督残差学习和轨迹优化框架，通过混合动力学模型学习未知动态效应，优化轨迹以最小化残差物理影响，实现四旋翼无人机精确跟踪激进轨迹


<details>
  <summary>Details</summary>
Motivation: 现实物理系统难以精确建模，存在残差物理效应，导致激进轨迹跟踪困难，需要解决控制器合成中的模型不精确问题

Method: 1) 自监督残差学习：将未知动态效应作为名义动力学的残差学习，形成混合模型；2) 使用解析梯度仅需轨迹级数据即可实现准确长时程预测；3) 轨迹优化器：计算最优参考轨迹，最小化沿轨迹的残差物理效应

Result: 通过混合动力学模型，优化器能够生成可精确跟踪的激进运动轨迹，在四旋翼无人机敏捷飞行中得到验证

Conclusion: 提出的自监督残差学习和轨迹优化框架有效解决了复杂机器人系统中物理建模不精确导致的激进轨迹跟踪问题，通过混合动力学模型实现了精确的轨迹生成和控制
...
Method: 结合元学习和反馈校准在线适应，从有限时间窗口的过去观测中提取特征，学习无需预定义结构假设的统一表示，通过状态反馈机制校准在线适应过程以衰减学习和泛化限制带来的残差

Result: 理论分析表明在线学习误差和扰动估计误差可同时收敛，通过统一元表示能有效估计多种快速变化扰动，四旋翼飞行实验验证了框架的有效性

Conclusion: 该框架为机器人应用中处理非结构性时变扰动提供了通用且有效的解决方案，通过元表示学习和反馈校准实现了对多种扰动的精确估计
...
TL;DR: 本文提出了一种新型多模态电动轮椅控制系统，集成了四种控制接口（操纵杆、语音、手势、眼电）和连续生命体征监测，旨在提升残疾患者的独立性和护理人员的实时监护能力。


<details>
  <summary>Details</summary>
Motivation: 电动轮椅对于ALS、中风后偏瘫、痴呆相关行动障碍等残疾人士至关重要。现有系统缺乏多模式控制和实时健康监测的集成，无法同时满足患者独立性和护理人员监护需求。

Method: 开发了集成四种控制接口（操纵杆、语音、手势、眼电）的多模态控制系统，结合连续生命体征监测（心率变异性、血氧饱和度、皮肤温度）。采用两点校准方法，通过加密云传输实现生理数据实时传输，Android应用提供护理人员警报。

Result: 传感器校准误差：心率≤2bpm，皮肤温度≤0.5°C，血氧饱和度≤1%。20名参与者执行500次室内导航命令，识别准确率：操纵杆99%，语音97±2%，手势95±3%，平均闭环延迟20±0.5ms。系统符合ISO 7176-31和IEC 80601-2-78安全标准。

Conclusion: 该原型系统通过集成多模态移动控制和云健康监测，解决了辅助机器人关键挑战，为未来自适应机器学习增强奠定了基础，同时满足安全标准要求。
...
<details>
  <summary>Details</summary>
Motivation: 传统帧式相机在四足机器人敏捷运动场景中容易产生模糊图像，特别是在低光条件下。现有事件相机数据集在立体配置和多波段感知方面存在局限，需要更全面的数据集来支持敏捷机器人感知研究。

Method: 使用Unitree Go2四足机器人配备立体事件相机、帧式相机、IMU和关节编码器，采集了30多个真实世界序列，涵盖不同速度水平、光照波长和照明条件，并提供完整的校准数据。

Result: 创建了M-SEVIQ数据集，包含多波段立体事件视觉和惯性数据，支持传感器融合和基准测试，为敏捷机器人感知研究提供了全面的数据资源。

Conclusion: M-SEVIQ数据集填补了现有事件相机数据集的空白，能够支持敏捷机器人感知、传感器融合、语义分割和多模态视觉等研究领域，特别是在挑战性环境中的应用。
...
TL;DR: 提出了一种实用的仿真到真实强化学习框架，通过密集触觉反馈和关节扭矩传感实现灵巧手控制策略的零样本迁移，无需在真实硬件上微调。


<details>
  <summary>Details</summary>
Motivation: 人类水平的灵巧手具有多指操控能力，但由于接触丰富的物理特性和不完美的驱动系统，直接在真实硬件上部署控制策略仍然困难。需要解决仿真到真实的差距。

Method: 1) 快速触觉仿真：通过并行前向运动学计算密集虚拟触觉单元与物体之间的距离，提供RL所需的高速率、高分辨率触觉信号；2) 电流到扭矩校准：通过映射电机电流到关节扭矩，消除灵巧手上扭矩传感器的需求；3) 执行器动力学建模：通过随机化非理想效应（如间隙、扭矩-速度饱和）来弥合驱动差距。使用非对称actor-critic PPO管道在仿真中训练。

Result: 策略直接部署到五指手上，展示了两种基本技能：1) 基于指令的可控抓握力跟踪；2) 手中物体的重新定向。两种技能都能在没有机器人微调的情况下稳健执行。

Conclusion: 通过将触觉和扭矩结合在观测空间中，配合有效的传感/驱动建模，该系统为实现可靠的灵巧操作提供了实用解决方案。这是首次在仿真中完全训练并在真实硬件上零样本迁移的多指灵巧手可控抓握演示。
...
TL;DR: 提出基于柔性连续体结构的跟随式内窥镜机器人，采用视觉深度强化学习框架，通过单目深度估计实现肠道自主导航，减少与肠壁接触


<details>
  <summary>Details</summary>
Motivation: 医疗和工业内窥镜机器人需要自主导航能力，但传统方法难以避免与狭窄管状环境内壁接触，导致患者不适

Method: 1) 柔性连续体结构机器人设计；2) 基于单目深度估计的视觉深度强化学习框架；3) 在NVIDIA Omniverse构建真实肠道仿真环境；4) 使用NVIDIA Replicator生成合成图像微调Depth Anything模型；5) 引入几何感知的奖励惩罚机制实现准确管腔跟踪

Result: 相比原始Depth Anything模型，深度准确度δ1提升39.2%；相比次优方法，导航J指数降低0.67，证明方法的鲁棒性和有效性

Conclusion: 提出的柔性连续体内窥镜机器人结合视觉深度强化学习框架，能够有效减少与肠壁接触，实现安全高效的自主导航
...
TL;DR: 该研究提出了一种软硬协同设计框架，利用非牛顿流体软响应材料增强人形机器人安全性，材料在正常交互时保持柔顺，在冲击下快速硬化吸收能量，通过仿真指导保护器布局和学习主动跌倒策略，在42公斤真人尺寸机器人上显著降低冲击峰值，实现从3米高处跌落和长楼梯翻滚等场景的重复安全跌落。


<details>
  <summary>Details</summary>
Motivation: 人形机器人作为人类中心环境中的通用平台，其部署受到易跌倒和刚性金属塑料结构对人员及环境构成风险的限制。需要解决机器人脆弱性和安全风险问题。

Method: 采用软硬协同设计框架，利用非牛顿流体基软响应材料，材料在正常交互时保持柔顺，在冲击下快速硬化以吸收和耗散跌倒产生的力。通过物理仿真指导保护器布局和厚度设计，并学习主动跌倒策略。

Result: 在42公斤真人尺寸人形机器人上应用，保护器显著降低峰值冲击，允许重复跌落而不损坏硬件，包括从3米高处跌落和长楼梯翻滚。在各种场景中提高了机器人鲁棒性和环境安全性。
...
<details>
  <summary>Details</summary>
Motivation: 传统优化控制器在处理多障碍物时难以快速生成可行解，而扩散模型虽然能生成多样轨迹但缺乏对场景结构的高效条件化方法

Method: 使用基于对象中心的槽注意力机制提供紧凑障碍物表示，通过扩散变换器生成初始轨迹，再用碰撞感知的MPC进行精炼以强制执行刚体动力学和碰撞约束

Result: 在基准任务中，该方法比基于采样的规划器或单独组件获得显著更高的成功率和更低延迟；真实机器人实验证实了可靠安全的执行性能

Conclusion: 结合扩散模型条件化预热和碰撞感知MPC的混合方法能够在严格时间限制下实现可靠高效的运动生成
...
TL;DR: LOST-3DSG：一种轻量级开放词汇3D场景图方法，用于动态环境中的物体跟踪，避免使用重型基础模型，通过语义嵌入实现高效跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有动态物体跟踪方法依赖重型基础模型导致效率低下，需要一种轻量级、开放词汇的解决方案来提升实时环境中的跟踪性能。

Method: 采用基于word2vec和句子嵌入的语义实体跟踪方法，构建开放词汇3D场景图，避免存储密集的CLIP视觉特征，实现轻量化表示。

Result: 在真实3D环境中使用TIAGo机器人进行定性和定量实验，结果显示LOST-3DSG在动态物体跟踪方面优于依赖高维视觉嵌入的方法。

Conclusion: LOST-3DSG提供了一种高效、轻量级的动态物体跟踪解决方案，在保持开放词汇能力的同时显著提升了性能，代码已公开。
...
TL;DR: PRMPPI控制框架结合在线参数学习和概率安全约束，通过并行优化性能驱动轨迹和安全备份轨迹，实现动态环境中机器人的安全控制
...
Method: 提出PRMPPI控制框架：1) 使用Stein变分梯度下降维护参数的粒子信念分布；2) 使用Conformal Prediction评估安全约束；3) 并行优化名义性能驱动轨迹和安全备份轨迹

Result: 仿真和硬件实验表明，相比基线方法，PRMPPI具有更高的成功率、更低的跟踪误差和更准确的参数估计

Conclusion: PRMPPI框架能够实现从谨慎开始、随着参数学习提升性能、同时确保全过程安全的机器人控制，适用于动态不确定环境
...
TL;DR: VILA提出了一种基于潜在动作的视角不变表示学习方法，通过建模轨迹间的转移模式来学习基于物理动态的视角不变表示，显著提升了机器人策略在未见视角下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的机器人策略通常对视角变化敏感，在真实世界中视角变化不可避免且会显著影响策略性能。现有方法主要从场景级的多视角观察中学习不变性，但仅依赖视觉外观而忽略了物理动态，限制了泛化鲁棒性。

Method: VILA（View-Invariant Latent Action）通过建模捕捉轨迹间转移模式的潜在动作来学习基于物理动态的视角不变表示。使用基于真实动作序列的动作引导目标在不同视角间对齐这些潜在动作，从而学习到对视角变化鲁棒的表示。

Result: 在仿真和真实世界的实验中，基于VILA的策略能够有效泛化到未见视角，并能很好地迁移到新任务。VILA作为一个强大的预训练框架，显著提升了鲁棒性和下游学习性能。

Conclusion: VILA通过将物理动态信息融入视角不变表示学习，解决了现有方法仅依赖视觉外观的局限性，为机器人策略在真实世界中的视角变化鲁棒性提供了有效解决方案。
...
TL;DR: 提出双向协作降落框架，将移动平台从被动目标转变为主动参与者，通过平台倾斜表面创造最佳终端姿态，并行化对齐和下降阶段，实现敏捷高效的四旋翼自主降落。
...
Method: 引入双向协作降落框架，将问题从单智能体跟踪转变为耦合系统优化。移动平台主动倾斜表面为接近的四旋翼创造稳定终端姿态，四旋翼规划时间最优且动态可行的轨迹以最小化能耗，实现对齐和下降阶段的并行化。

Result: 在动态场景中验证了框架的有效性，显著提高了自主四旋翼回收在复杂和时间受限任务中的效率、精度和鲁棒性，实现了敏捷的轨迹跟踪和瞬态窗口内的快速状态同步。

Conclusion: 通过将移动平台重新定义为主动参与者，双向协作框架突破了传统顺序降落范式的限制，实现了更高效、精确和鲁棒的四旋翼自主降落，特别适用于动态和复杂任务场景。
...
TL;DR: 提出一个结合抽象推理与具体系统证伪的两层验证框架，用于验证通用机器人系统，在桌面操作任务中成功发现了NVIDIA GR00T控制器的故障案例
...
Method: 采用两层验证框架：抽象层使用情境演算建模世界并推导最弱前置条件，通过约束感知的组合测试系统生成多样化、语义有效的世界-任务配置；具体层将这些配置实例化，进行基于仿真的证伪，并使用STL监控

Result: 在桌面操作任务上的实验表明，该框架有效地发现了NVIDIA GR00T控制器的故障案例，展示了验证通用机器人自主性的潜力

Conclusion: 提出的两层验证框架结合了抽象推理和具体系统证伪，为验证通用机器人系统提供了一种有效方法，能够发现传统方法可能遗漏的故障案例
...
TL;DR: SOP系统实现了视觉-语言-动作模型在物理世界中的在线、分布式、多任务后训练，通过机器人舰队持续收集经验并异步更新策略，显著提升预训练模型的性能。
...
Method: 提出SOP系统，采用闭环架构：机器人舰队持续流式传输在线策略经验和人类干预信号到集中式云端学习器，并异步接收更新后的策略。系统支持交互式模仿学习和强化学习两种后训练算法。

Result: 在布料折叠、箱子组装、杂货补货等真实世界操作任务中，SOP显著提升了大型预训练VLA模型的性能，同时保持跨任务的共享策略。后训练可在数小时内完成，性能随机器人数量接近线性扩展。
...
### [23] [A Fast Semidefinite Convex Relaxation for Optimal Control Problems With Spatio-Temporal Constraints](https://arxiv.org/abs/2601.03055)
*Shiying Dong,Zhipeng Shen,Rudolf Reiter,Hailong Huang,Bingzhao Gao,Hong Chen,Wen-Hua Chen*

Main category: cs.RO

TL;DR: 提出了一种时间缩放直接多重射击方案，用于解决具有时空约束的最优控制问题，通过凸松弛方法提高计算效率和最优性。


<details>
  <summary>Details</summary>
Motivation: 自主智能体在时空约束下的最优控制问题（如自动驾驶节能驾驶、四旋翼导航）通常通过非线性规划近似，但由于动力学与事件时间的耦合导致非凸性，求解困难。现有方法通常预定义航点时间或使用非凸轨迹优化，这简化了问题但往往得到次优解。

Method: 提出了时间缩放直接多重射击方案，将预测时域划分为与特征时间约束对齐的段；开发了基于半定规划的快速凸松弛方法，利用提升公式的稀疏模式。

Result: 综合仿真研究表明了方法的解最优性和计算效率；在四旋翼航点飞行任务（具有约束开放时间窗口）的实际实验中，验证了该方法在复杂环境中的实际适用性。

Conclusion: 提出的时间缩放直接多重射击方案和凸松弛方法显著改善了具有时空约束的最优控制问题的数值特性，在保持解最优性的同时提高了计算效率，适用于实际复杂环境。
...
### [24] [HEXAR: a Hierarchical Explainability Architecture for Robots](https://arxiv.org/abs/2601.03070)
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人系统日益复杂，对可解释决策的需求变得至关重要。现有的机器人可解释性方法要么关注单个模块（难以从高层行为角度查询），要么采用整体方法（未能利用机器人架构的模块化特性）。

Method: 提出HEXAR（机器人分层可解释性架构），这是一个新颖的框架，采用可插拔的分层方法来生成机器人系统解释。HEXAR包含专门针对特定机器人模块的组件解释器（使用LLM推理、因果模型、特征重要性等多样化技术），由解释器选择器根据给定查询选择最合适的解释器。

Result: 在TIAGo机器人执行家庭环境辅助任务的180个场景-查询变体上评估HEXAR，与端到端和聚合基线方法比较。HEXAR在根本原因识别、错误信息排除和运行时间方面显著优于基线方法。

Conclusion: HEXAR为透明自主系统提供了一个有前景的方向，通过模块化、分层的可解释性架构解决了现有方法的局限性。
...
### [25] [Dual-quaternion learning control for autonomous vehicle trajectory tracking with safety guarantees](https://arxiv.org/abs/2601.03097)
*Omayra Yago Nieto,Alexandre Anahory Simoes,Juan I. Giribet,Leonardo Colombo*

Main category: cs.RO

TL;DR: 提出一种基于学习的SE(3)运动机器人轨迹跟踪控制器，使用对偶四元数框架，通过高斯过程回归在线学习补偿未知扰动，具有形式化稳定性保证。


<details>
  <summary>Details</summary>
Motivation: 自主机器人系统在实际应用中面临传感器扰动、未建模的驱动耦合和环境不确定性等挑战，需要能够在线学习和补偿这些未知影响的鲁棒控制方法，同时保持刚体运动的几何结构特性。

Method: 基于对偶四元数框架设计几何反馈控制器，在速度层面操作，集成高斯过程回归在线学习状态相关的未知扰动和建模误差，保持刚体运动的代数结构和耦合特性，不依赖显式参数模型。

Result: 通过李雅普诺夫分析建立了在有限高斯过程不确定性下的姿态跟踪误差概率最终有界性，提供了形式化稳定性保证。仿真结果表明在存在局部扰动（包括磁力计扰动引起的相关旋转和平移效应）时能够实现准确平滑的轨迹跟踪。

Conclusion: 结合几何建模和概率学习的方法能够为自主机器人系统实现鲁棒、数据高效的姿态控制，该方法不依赖显式参数模型，适合处理传感器扰动、未建模驱动耦合和环境不确定性。
...
### [26] [A High-Fidelity Digital Twin for Robotic Manipulation Based on 3D Gaussian Splatting](https://arxiv.org/abs/2601.03200)
*Ziyang Sun,Lingfan Bao,Tianhu Peng,Jingcheng Sun,Chengxu Zhou*

Main category: cs.RO

TL;DR: 该论文提出了一个基于3D高斯泼溅的快速数字孪生构建框架，能够在几分钟内从稀疏RGB输入创建高质量数字孪生，支持机器人运动规划和真实世界执行。


<details>
  <summary>Details</summary>
Motivation: 现有数字孪生方法存在重建速度慢、视觉保真度有限、难以将逼真模型转换为规划就绪的碰撞几何等问题，阻碍了从仿真到真实环境的有效转移。

Method: 使用3D高斯泼溅(3DGS)作为统一场景表示进行快速逼真重建，增强可见性感知语义融合进行准确3D标注，并引入基于滤波器的高效几何转换方法生成碰撞就绪模型，与Unity-ROS2-MoveIt物理引擎无缝集成。

Result: 在Franka Emika Panda机器人执行拾放任务的实验中，增强的几何精度有效支持了真实世界试验中的稳健操作，实现了从感知到操纵的快速可靠路径。

Conclusion: 基于3DGS的数字孪生，通过语义和几何一致性增强，为无结构环境中从感知到操纵提供了快速、可靠且可扩展的解决方案。
