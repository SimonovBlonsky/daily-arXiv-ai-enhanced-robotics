...
TL;DR: PhyGile是一个统一框架，通过物理前缀引导的机器人原生运动生成和通用运动跟踪，解决文本到运动生成模型直接应用于人形机器人时的物理可行性问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到运动生成模型主要基于人类运动数据集训练，其先验假设人类生物力学、驱动、质量分布和接触策略。当这些运动直接重定向到人形机器人时，虽然满足几何约束，但经常违反实际执行的物理可行性要求。

Method: 1) 使用课程式专家混合方案训练GMT控制器，并在未标记运动数据上进行后训练以提高鲁棒性；2) 在推理时执行物理前缀引导的机器人原生运动生成，直接在262维骨骼空间中生成机器人原生运动；3) 在物理前缀适应阶段，使用生成目标在物理衍生前缀下微调GMT控制器。

Result: PhyGile扩展了文本驱动人形控制的边界，能够稳定跟踪敏捷、高难度的全身运动，远超先前方法通常实现的行走和低动态运动。离线和真实机器人实验验证了其有效性。

Conclusion: PhyGile通过统一机器人原生运动生成和通用运动跟踪，消除了推理时重定向伪影，减少了生成与执行之间的差异，实现了真实机器人上复杂运动的敏捷稳定执行。
...
TL;DR: VAMPO是一个后训练框架，通过策略优化直接改进视频动作模型的视觉动态，将多步去噪过程视为序列决策问题，优化去噪策略以匹配专家视觉动态。
...
Method: 将多步去噪过程构建为序列决策过程，使用潜在空间中专家视觉动态定义的奖励来优化去噪策略；引入Euler Hybrid采样器，仅在第一步去噪注入随机性，实现低方差策略梯度估计；结合GRPO和可验证的非对抗奖励。

Result: 在多样化的模拟和现实世界操作任务中，VAMPO改进了任务相关的视觉动态，带来了更好的下游动作生成和更强的泛化能力。

Conclusion: VAMPO通过策略优化直接改进视频动作模型的视觉动态，解决了扩散模型训练目标与操作需求之间的不匹配问题，为视觉-语言-动作系统提供了更可靠的视觉动态预测基础。
...
<details>
  <summary>Details</summary>
Motivation: 软连续体机械臂虽然具有被动安全性，但仍需要主动控制来实现可靠的避障。现有方法（如基于采样的规划）计算成本高且缺乏形式化安全保证，限制了其在实时全身避障中的应用。

Method: 提出了一种闭式控制Lyapunov函数-控制屏障函数（CLF-CBF）控制器，通过解析地将安全约束嵌入控制输入，确保在建模假设下的稳定性和安全性，避免了在线优化方法的可行性问题。

Result: 在肌腱驱动软机械臂上的仿真和硬件实验表明，该方法能在杂乱环境中实现精确的3D轨迹跟踪和鲁棒的避障。控制器比标准CLF-CBF二次规划方法快10倍，比传统基于采样的规划器快100倍。

Conclusion: 该框架为在动态、安全关键环境中操作的软机器人提供了一种可扩展且可证明安全的控制策略。
...
TL;DR: 该研究创建了首个评估大语言模型在机器人路径规划算法近似比证明能力的基准，包含34个研究级证明任务，发现当前最强模型在没有领域知识的情况下难以生成有效证明，但提供任务特定的引理可以显著改善推理质量。


<details>
  <summary>Details</summary>
Motivation: 机器人路径规划问题通常是NP难问题，实际解决方案依赖具有可证明性能保证的近似算法。虽然设计这类算法具有挑战性，但形式化证明其近似最优性更为困难，需要领域特定的几何洞察力和对复杂操作约束的多步数学推理。尽管最近的大语言模型在数学推理基准测试中表现出色，但它们在机器人路径规划研究级最优性证明方面的能力尚未得到充分探索。

Method: 研究者创建了首个评估大语言模型在机器人路径规划算法近似比证明能力的基准，包含34个研究级证明任务，涵盖不同规划问题类型和复杂度级别。每个任务都需要对算法描述、问题约束和理论保证进行结构化推理。评估了最先进的专有和开源大语言模型，并测试了不同提示策略的效果，包括提供任务特定引理、通用思维链提示以及提供真实近似比作为后验知识。

Result: 评估显示，即使是最强的模型在没有外部领域知识的情况下也难以生成完全有效的证明。然而，为大语言模型提供任务特定的上下文引理可以显著提高推理质量，这比通用思维链提示或提供真实近似比作为后验知识更有效。研究还提供了细粒度的错误分析，描述了常见的逻辑失败和幻觉，并展示了如何通过有针对性的上下文增强来缓解每种错误类型。

Conclusion: 该研究建立了机器人路径规划算法近似比证明的基准测试，揭示了大语言模型在该领域的当前能力局限性和改进潜力。研究表明，提供领域特定的上下文知识（特别是任务相关引理）是提高大语言模型在复杂数学证明任务中表现的关键因素，为未来开发更有效的AI辅助研究工具提供了重要见解。
...
TL;DR: 基于事件相机的光学通信系统，实现移动发射器跟踪和实时消息解码，解码准确率超95%，处理速度比现有方法快7倍


<details>
  <summary>Details</summary>
Motivation: 传统RF通信在多机器人系统中存在竞争和干扰问题，而传统帧式相机存在帧率限制、运动模糊和高动态范围光照下的鲁棒性问题。事件相机具有微秒级时间分辨率和高动态范围，适合快速相对运动下的光学通信。

Method: 开发完整的光学通信系统，采用几何感知无迹卡尔曼滤波器（GA-UKF）跟踪移动发射器并实时解码消息。利用事件相机的高时间分辨率和高动态范围特性。

Result: 系统在运动期间文本传输的解码准确率超过95%，处理速度比现有最优方法快7倍，在传输频率≥1kHz时保持相当的跟踪精度。

Conclusion: 基于事件相机的光学通信系统能够有效解决多机器人系统中的通信问题，在保持高跟踪精度的同时显著提升处理速度，为动态环境下的可靠通信提供了新方案。
...
TL;DR: CeRLP是一个跨具身机器人局部规划框架，通过统一几何表示解决不同机器人配置下的视觉导航问题，无需大量数据收集或微调。


<details>
  <summary>Details</summary>
Motivation: 跨具身机器人视觉导航面临机器人配置（尺寸、相机参数、类型）差异带来的挑战，现有方法需要大量数据收集或模型微调，且缺乏对机器人几何的显式考虑。

Method: 提出CeRLP框架：1）深度估计尺度校正方法，通过离线预标定解决单目深度估计的尺度模糊问题；2）视觉到扫描抽象模块，将不同视觉输入投影为高度自适应的激光扫描，使策略对异构机器人具有鲁棒性。

Result: 仿真实验显示CeRLP优于对比方法，验证了其作为局部规划器的鲁棒避障能力。真实世界实验在点对点导航和视觉语言导航任务中验证了有效性，展示了跨不同机器人相机配置的泛化能力。

Conclusion: CeRLP通过统一几何表示和尺度校正方法，实现了跨异构机器人的通用视觉导航，无需大量数据收集或模型微调，具有良好的泛化性能。
...
TL;DR: 本文提出了两种新方法提升行星表面探测能力：高机动性腿式机器人通过直接表面交互收集地形力学信息指导探测；人类启发式数据采集算法使机器人能基于科学假设和实时地面传感测量自适应调整探测优先级。


<details>
  <summary>Details</summary>
Motivation: 当前行星表面探测受限于轮式机器人在复杂地形中的移动能力，以及预编程数据采集计划缺乏现场灵活性。需要突破这些限制以探索更广泛的行星环境。

Method: 1) 开发高机动性腿式机器人，通过直接表面交互收集地形力学特性信息；2) 设计人类启发式数据采集算法，使机器人能够基于科学假设推理，并根据实时地面传感测量自适应调整探测优先级。

Result: 实验室和两个行星模拟环境的实地部署验证了方法的有效性。腿式机器人测量土壤力学特性的新能力使其能够有效穿越复杂地形。土壤力学测量与其他地质特性结合揭示了地质环境形成和发展的关键因素。人类启发式算法将地形传感机器人转变为科学家的队友，支持更灵活自适应的数据采集决策。

Conclusion: 通过集成人类-机器人系统，该方法能够探索更广泛的行星环境，提供新的基质调查机会，实现最大化的科学回报。腿式机器人的地形传感能力与自适应算法相结合，为行星探测提供了更强大的工具。
...
TL;DR: 本文提出了一种轻量级实时框架，将3D LiDAR数据投影到2D鸟瞰图，用于资源受限机器人上的结构感知，比较了传统几何方法和深度学习方法的性能。


<details>
  <summary>Details</summary>
Motivation: 资源受限机器人需要高效的结构感知用于建图和自主导航，现有3D方法计算成本高，传统2D几何方法鲁棒性不足。

Method: 将3D LiDAR数据投影到2D鸟瞰图，系统评估多种特征提取策略：传统几何方法（霍夫变换、RANSAC、LSD）和基于YOLO-OBB的深度学习检测器，通过时空融合模块整合检测结果。

Result: 实验表明：传统方法（霍夫和LSD）响应快但对噪声敏感，LSD产生过多片段导致系统拥塞；RANSAC鲁棒性更好但无法满足实时要求；YOLO-OBB方法在鲁棒性和计算效率间达到最佳平衡，在单板计算机上满足10Hz实时操作。

Conclusion: 本文的主要贡献是提出了一种计算高效的基于鸟瞰图的感知流程，能够在资源受限的机器人平台上实现可靠的实时结构检测，无需GPU加速处理。
...
TL;DR: 提出了一种用于完整多智能体系统的新型混合运动规划方法，结合去中心化MPC与集中式高阶控制屏障函数，显著提升了可扩展性并在真实工业磁悬浮平台上验证了实时能力。


<details>
  <summary>Details</summary>
Motivation: 传统集中式模型预测控制（MPC）在处理多智能体系统时，随着智能体数量增加会变得难以计算，缺乏可扩展性。需要一种既能保证安全性又能实现实时计算的方法。

Method: 提出去中心化MPC框架，结合交替方向乘子法（ADMM）的去中心化版本与集中式高阶控制屏障函数（HOCBF）架构。通过C++高效实现并部署到工业磁悬浮平台。

Result: 仿真结果显示相比传统集中式MPC在可扩展性上有显著提升。在真实工业磁悬浮平台上的部署验证了方法的有效性和实时能力。

Conclusion: 该混合运动规划方法成功解决了多智能体系统中MPC的可扩展性问题，同时提供了安全保证，并在实际工业应用中展示了实时性能。
...
TL;DR: 提出了一种用于星载地球观测处理的分层多智能体架构，通过事件驱动决策管道协调专用AI智能体，在资源受限条件下实现灾害事件的快速识别。


<details>
  <summary>Details</summary>
Motivation: 当前地球观测监测管道主要基于地面处理，存在下行链路限制、多源数据融合约束和全面场景分析计算成本高等延迟问题，需要下一代自主EO星座的快速灾害响应能力。

Method: 采用分层多智能体架构，包括早期预警智能体生成快速假设并选择性激活领域特定分析智能体，决策智能体整合证据发出最终警报。结合视觉语言模型、传统遥感分析工具和角色专用智能体，在分布式卫星平台上实现结构化多模态观测推理。

Result: 在已部署在轨的边缘计算平台工程模型上进行了概念验证，使用代表性卫星数据。在野火和洪水监测场景实验中，基于路由的管道显著降低了计算开销，同时保持一致的决策输出。

Conclusion: 证明了分布式基于智能体的推理对未来自主地球观测星座的可行性，能够在严格资源和带宽约束下实现快速灾害事件识别。
...
<details>
  <summary>Details</summary>
Motivation: 现有雷达-惯性里程计方法通常假设已知雷达-IMU外参校准，或依赖充分运动激励进行在线外参估计，同时经常忽略传感器间的时间偏差问题，或将其独立处理。

Method: 采用基于均匀三次B样条的连续时间建模方法表示惯性测量，在因子图优化框架内联合在线估计空间和时间校准参数，准确捕捉雷达-IMU测量的异步特性。

Result: 该方法能够可靠地收敛时间偏移和外参校准参数，无需依赖扫描匹配、目标跟踪或环境特定假设，在具有挑战性的条件下提供鲁棒的里程计估计。

Conclusion: 提出的连续时间雷达-惯性里程计框架通过联合空间和时间校准，解决了现有方法在传感器校准方面的局限性，为恶劣条件下的导航提供了更可靠的解决方案。
...
TL;DR: GustPilot：结合深度强化学习规划和增量非线性动态逆控制的抗风无人机导航系统，在强风环境下实现高成功率飞行


<details>
  <summary>Details</summary>
Motivation: 风扰动是轻型四旋翼无人机自主导航的主要障碍，传统方法难以应对快速变化的气流对规划和跟踪的干扰，需要开发更鲁棒的抗风导航系统

Method: 采用分层架构：上层使用深度强化学习策略生成惯性坐标系速度参考用于穿越门框；下层使用几何增量非线性动态逆控制器提供快速扰动抑制，通过传感器测量实现线性加速度和角加速度率的增量反馈

Result: 在50g四旋翼平台上进行真实飞行测试，相比DRL-PID基线，DRL-INDI平均总体成功率从55.0%提升到94.7%，跟踪RMSE降低达50%，在3.5m/s风速下仍能保持1.34m/s飞行速度

Conclusion: 深度强化学习速度规划与结构化INDI扰动抑制相结合，为抗风自主飞行导航提供了实用且可泛化的解决方案，即使在仅使用单门单风扇训练的情况下也能泛化到更复杂环境
...
TL;DR: 提出了一种用于在线地图生成的结构化概率方法，通过低秩加对角协方差分解显式建模道路元素内部的空间相关性，在nuScenes数据集上展示了地图生成质量和运动预测性能的提升。
...
Method: 提出结构化概率公式，通过预测密集协方差矩阵显式建模元素内部依赖关系，采用低秩加对角协方差分解参数化，将不确定性表示为低秩分量（捕捉全局空间结构）和对角分量（表示独立局部噪声）的组合。

Result: 在nuScenes数据集上的评估表明，该不确定性感知框架相比确定性基线在在线地图生成质量上获得一致改进，并为基于地图的运动预测建立了新的最先进性能。

Conclusion: 该方法通过结构化概率建模有效捕捉道路几何的空间相关性，证明了不确定性在规划任务中的关键作用，为自动驾驶感知-预测-规划管道提供了更可靠的地图生成解决方案。
...
TL;DR: Co-Ego系统通过融合机器人地面视角和用户高处视角，解决四足机器人导盲犬导航中的视角不对称问题，显著提高避障安全性


<details>
  <summary>Details</summary>
Motivation: 导盲犬数量有限，大多数视障人士无法获得服务。现有四足机器人导盲犬仅依赖地面传感器，无法检测对人类身高危险但对机器人透明的障碍物（如弯曲树枝），存在视角不对称问题

Method: 提出Co-Ego系统，采用双分支避障框架，集成机器人中心的地面感知和用户高处自我中心视角，确保全面的导航安全

Result: 在受控用户研究中，与无辅助、单视角相比，跨视角融合显著减少了碰撞次数和认知负荷，验证了视角互补对安全导航的必要性

Conclusion: 解决视角不对称问题是实现安全机器人导盲犬导航的关键，Co-Ego系统的跨视角融合方法为此提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 为了解决在建筑环境中操作大型机械臂时存在的安全性、可重复性和效率问题，需要一种既能利用人类直觉又能保证精确操作的控制框架

Method: 采用领导者-跟随者范式，使用轻量级引导臂实现直观人工控制，结合基于AprilTag检测的自主感知模块进行精确对齐和抓取执行，将人工控制与精细操作解耦

Result: 实验结果表明，KUKAloha降低了操作员工作量，提高了任务完成效率，为建筑环境中的可扩展演示收集和共享人机控制提供了实用解决方案

Conclusion: KUKAloha框架通过解耦人工控制和精细操作，提高了大型机械臂操作的安全性和可重复性，为建筑机器人遥操作提供了有效的共享控制方案
...
TL;DR: AGILE是一个标准化人形机器人强化学习开发流程的端到端工作流，包含环境验证、可重复训练、统一评估和描述符驱动部署四个阶段，旨在解决人形机器人RL从仿真到实物的转移难题。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习在人形机器人仿真中取得了显著进展，但将这些成果转移到新机器人平台仍然困难。主要瓶颈不再是仿真吞吐量或算法设计，而是缺乏将环境验证、训练、评估和部署连接成连贯循环的系统化基础设施。

Method: AGILE工作流包含四个阶段：1) 交互式环境验证；2) 可重复训练；3) 统一评估（支持基于场景的测试和随机化rollout）；4) 通过机器人/任务配置描述符驱动的部署。工作流还包含训练稳定化和算法增强来改善优化稳定性和仿真到实物的转移。

Result: 在Unitree G1和Booster T1两个硬件平台上验证了AGILE，涵盖了行走、恢复、运动模仿和移动操作等五种代表性人形机器人技能，实现了稳定的仿真到实物转移。
...
TL;DR: HortiMulti是一个多模态跨季节数据集，收集于商业草莓和覆盆子塑料大棚，包含多种传感器数据和地面真实轨迹，旨在解决园艺环境中机器人感知和定位算法的挑战。


<details>
  <summary>Details</summary>
Motivation: 农业机器人领域缺乏代表性的真实世界数据集，特别是园艺环境（如塑料大棚）相比城市和林业机器人领域数据集较少。园艺环境存在外观变化大、动态植被、塑料覆盖反光、感知混叠和GNSS不可靠等挑战，这些都会影响现有定位和感知算法的性能。

Method: 在商业草莓和覆盆子塑料大棚中收集整个生长季节的多模态数据，传感器套件包括两个3D激光雷达、四个RGB相机、IMU、GNSS和轮式里程计。地面真实轨迹通过全站仪测量、AprilTag标记和激光雷达惯性里程计组合获得，涵盖密集、稀疏和无标记覆盖。

Result: 发布了时间同步的原始测量数据、校准文件、参考轨迹以及视觉、激光雷达和多传感器SLAM的基准测试。结果显示当前最先进的方法在塑料大棚部署中仍然不足，证实了园艺环境的挑战性。

Conclusion: HortiMulti作为一站式资源，为园艺环境中机器人感知系统的开发和测试提供了重要数据集，填补了该领域代表性数据集的空白，有助于推动农业机器人技术的发展。
...
TL;DR: CRISP框架让机器人通过视觉语言模型作为社交评判者，自主批判和重新规划社交行为，无需特定API即可在不同机器人平台上生成自然的人类化动作。


<details>
  <summary>Details</summary>
Motivation: 传统机器人社交行为生成依赖预定义动作或人工反馈，缺乏灵活性和自主性。需要一种能自主评估和优化社交行为的通用框架。

Method: 提出CRISP框架：1) 分析机器人描述文件提取可动关节和约束；2) 基于情境生成分步行为计划；3) 参考视觉信息生成低级关节控制代码；4) 使用VLM评估社交适当性和自然性；5) 通过奖励搜索迭代优化行为。

Result: 在5种不同类型机器人和20个场景的用户研究中，CRISP方法在偏好度和情境适当性评分上显著优于先前方法，展示了跨平台适用性。

Conclusion: CRISP提供了一个通用框架，最小化人工干预，扩展了机器人的自主交互能力和跨平台适用性，为机器人社交行为生成提供了新思路。
...
TL;DR: IndoorR2X：首个用于室内环境中基于LLM的多机器人任务规划基准框架，通过整合移动机器人和静态IoT设备的感知数据，构建全局语义状态以提升多机器人协作效率。


<details>
  <summary>Details</summary>
Motivation: 传统机器人间通信（R2R）在室内场景理解中存在局限性，无法克服部分可观测性问题，需要大量探索开销或扩大团队规模。而室内环境中已有的低成本IoT传感器（如摄像头）能够提供持续的建筑级上下文信息，超越单个机器人的感知能力。

Method: 提出IndoorR2X基准和仿真框架，整合移动机器人和静态IoT设备的观测数据，构建全局语义状态。采用基于大语言模型（LLM）的任务规划方法，支持可扩展的场景理解，减少冗余探索，实现高层级协调。

Result: IoT增强的世界建模显著提高了多机器人系统的效率和可靠性。在不同设置下的广泛实验验证了该方法的有效性，并识别了基于LLM的机器人与室内IoT传感器协作的关键见解和失败模式。

Conclusion: IndoorR2X为系统评估高层级语义协调策略提供了可配置的仿真环境，展示了IoT感知增强如何改善多机器人协作，为推进机器人与室内IoT传感器之间的LLM基协作提供了重要基础。
