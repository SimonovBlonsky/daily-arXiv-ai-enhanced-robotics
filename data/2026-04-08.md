...
TL;DR: 该论文综述了利用无动作标注的视频数据学习机器人操作控制接口的方法，提出了基于接口构建位置和控制特性的分类法，分析了三类方法并指出机器人集成层是当前主要挑战。


<details>
  <summary>Details</summary>
Motivation: 视频数据包含了丰富的物理动态信息（物体运动、接触展开、场景演化），但缺乏动作监督且与机器人经验存在差异（具身性、视角、物理约束）。如何将视频的时间结构转化为可靠的机器人控制是一个开放挑战。

Method: 提出了"接口中心分类法"，按视频到控制接口的构建位置和控制特性分为三类：1）直接视频-动作策略（接口隐式）；2）潜在动作方法（通过紧凑学习中间层路由时间结构）；3）显式视觉接口（预测可解释目标供下游控制）。分析了每类方法的控制集成特性。

Result: 通过跨家族综合分析发现，最紧迫的开放挑战集中在"机器人集成层"——将视频衍生预测连接到可靠机器人行为的机制。当前方法在闭环控制、执行前验证和故障处理方面存在差异。

Conclusion: 视频作为无动作标注的物理动态观察具有巨大潜力，但实现可靠机器人控制需要解决机器人集成层的问题。未来研究方向应聚焦于弥合视频预测与机器人行为执行之间的差距。
...
TL;DR: COMB是一个紧凑、开源、模块化的机电平台，专为标准观察蜂箱框架内的蜂巢实验设计，集成了XY定位平台、可移动访问窗口、可互换负载模块和嵌入式控制架构。


<details>
  <summary>Details</summary>
Motivation: 真实蜂群实验需要能够在有限空间约束下操作、耐受蜂巢特定污染和环境条件、支持传感和局部驱动而不需要频繁硬件重新设计的机器人系统。

Method: 开发了COMB平台，包含XY定位平台、可移动访问窗口（用于通过蜂巢边界进行密封工具访问）、可互换负载模块和嵌入式控制架构，支持可重复轨迹执行和信号生成。

Result: 通过三个代表性模块展示了平台能力：仿生舞蹈和信号负载、近距离蜂巢扫描仪、用于局部振荡刺激的电磁翅膀驱动器。通过跟踪摇摆轨迹执行、执行多图像拼接进行重复蜂巢马赛克、进行基于视频的翅膀驱动器频谱分析来表征平台性能。

Conclusion: COMB作为一个可重复使用的实验机器人平台，用于受控的蜂巢内传感和驱动，是早期任务特定蜜蜂机器人系统的紧凑、通用化后继者。
...
TL;DR: 本文综述了无人水下航行器（UUV）基于传感器的规划与控制方法，重点探讨了在复杂水下环境中应对传感器噪声、GNSS信号缺失等挑战的局部规划层技术，并对解耦与耦合架构进行了分类比较。


<details>
  <summary>Details</summary>
Motivation: UUV在复杂、不确定的水下环境中面临诸多挑战：传感器测量漂移和噪声、缺乏GNSS信号、水下声学通信带宽低延迟高。这些因素使得UUV需要先进的规划与控制策略来实现有效导航，特别是在动态海洋条件下需要实时适应传感器输入。

Method: 该综述将现有文献分为解耦架构和耦合架构两类。解耦架构顺序处理规划和控制阶段，而耦合架构提供更紧密的反馈回路以实现更快的响应。重点关注基于声纳和IMU等实时传感器输入的自适应局部规划层，以提高定位精度和自主性。

Result: 对耦合规划与控制方法的比较分析显示：PID控制器简单但缺乏复杂机动的预测能力；模型预测控制（MPC）提供优越的路径优化但计算量大；不变集控制器提供强大的安全保证但可能在受限环境中牺牲敏捷性。

Conclusion: 该综述的主要贡献包括：提出了结合规划与控制的架构分类法，重点关注自适应局部规划，并分析了控制器在UUV自主导航集成规划框架中的作用，为未来水下自主系统的发展提供了理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA方法存在架构、代码库和评估协议不兼容的问题，阻碍了系统性比较和可复现性。需要统一的框架来整合感知、语言理解和动作能力。

Method: StarVLA采用模块化的backbone-action-head架构，支持VLM骨干（如Qwen-VL）和世界模型骨干（如Cosmos），提供可复用的训练策略（跨具身学习和多模态协同训练），并通过统一接口集成多个基准测试。

Result: StarVLA的简单训练配方在多个基准测试（LIBERO、SimplerEnv、RoboTwin~2.0、RoboCasa-GR1、BEHAVIOR-1K）上达到或超越了现有方法，支持仿真和真实机器人部署。
...
TL;DR: GaussFly是一个用于自主飞行器视觉运动策略学习的新框架，通过3D高斯重建和对比表示学习，实现了从真实到模拟再到真实的鲁棒策略迁移。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端学习方法直接映射RGB观测到动作命令，存在样本效率低和严重的模拟到真实差距问题，主要原因是模拟与真实域之间的视觉差异。

Method: 采用真实-模拟-真实范式，首先使用带几何约束的3D高斯重建训练场景实现高保真实到模拟转换，然后通过对比表示学习从渲染图像中提取紧凑、抗噪声的潜在特征，最后用预训练编码器为策略提供低维特征输入。

Result: 在模拟和真实环境中的广泛实验表明，GaussFly相比基线方法具有更优的样本效率和渐进性能，能够实现零样本策略迁移到未见过的真实环境，有效弥合模拟到真实差距。

Conclusion: GaussFly通过解耦表示学习和策略优化，显著降低了计算负担并增强了抗视觉噪声能力，为自主飞行器的视觉运动策略学习提供了一个高效且鲁棒的解决方案。
...
TL;DR: 提出了一种用于肌腱驱动可转向导丝的双线性模型预测控制框架，该导丝与标准近距离放射治疗针集成，通过虚拟输入映射到物理可实现的控制，在组织模拟材料中验证了固定目标和移动目标跟踪能力。


<details>
  <summary>Details</summary>
Motivation: 现有可转向针的建模和控制方法主要针对定制针设计，无法直接应用于与商用临床针兼容的导丝。需要开发适用于标准近距离放射治疗针的可转向导丝控制框架，以实现避开敏感解剖结构的弯曲轨迹。

Method: 提出了一个双线性模型预测控制框架，采用几何双线性模型，包含三个虚拟输入（插入速度和两个弯曲率），这些输入被映射到物理可实现的实际输入（插入速度和相应的肌腱张力）。通过图像尖端跟踪在组织模拟材料中进行仿真和物理插入实验验证。

Result: 开环模型验证的估计误差低于2mm（插入针长度的3%），闭环固定目标跟踪误差低至1.45mm（插入长度的1.7%）。但在某些弯曲方向上位置误差较大，达到8.3mm（插入长度的7.8%）。
...
TL;DR: RoboPlayground：一个通过自然语言定义可执行机器人操作任务的框架，支持用户驱动的评估和任务变体生成


<details>
  <summary>Details</summary>
Motivation: 当前机器人操作系统的评估主要依赖少数专家预定义的固定基准，限制了评估的参与者和任务变体的探索，无法充分测试策略对用户定义的任务意图、约束和成功标准的响应能力

Method: 提出RoboPlayground框架，允许用户使用自然语言在结构化物理领域中编写可执行的操作任务。自然语言指令被编译为可复现的任务规范，包含明确的资产定义、初始化分布和成功谓词，每个指令定义了一个相关的结构化任务家族

Result: 用户研究表明，语言驱动界面比基于编程和代码辅助的基线更易用且认知负荷更低；在语言定义的任务家族上评估学习策略揭示了固定基准评估中未显现的泛化失败；任务多样性随贡献者多样性而非任务数量单独扩展

Conclusion: RoboPlayground将评估重构为结构化物理领域上的语言驱动过程，支持通过众包贡献持续扩展评估空间，揭示了当前基准评估的局限性并提供了更全面的策略评估方法
...
TL;DR: 提出一种基于3D打印塑料条的可折叠可展开致动器，通过复合折叠和拉链式结构实现可逆的尺寸和刚度变换，应用于自适应行走机器人。


<details>
  <summary>Details</summary>
Motivation: 现有形状变化系统通常采用定制化、系统特定的机制，难以扩展、重新配置或跨应用移植。需要开发更通用、易于制造的可变形机器人系统。

Method: 使用柔性3D打印塑料条，通过复合折叠和拉链式结构形成方形截面可展开梁，实现紧凑（柔性）和展开（准刚性）状态之间的平滑连续转换。

Result: 致动器的机械性能得到表征，并展示了由四个模块组成的自适应行走机器人集成系统，验证了模块组合实现多样化形状和刚度变换的能力。

Conclusion: 该紧凑、易于制造的可展开致动器为可变形机器人提供了一种通用解决方案，能够实现可逆的尺寸和刚度变换，具有广泛的应用潜力。
...
TL;DR: 该论文提出使用语义嵌入技术来学习分子群的行为范围，为优化此类系统提供更丰富的特征集。研究针对DNA功能化的微管分子群，通过扩展模型和语义分析验证了预期行为。


<details>
  <summary>Details</summary>
Motivation: 当前分子群系统优化缺乏丰富的特征描述，需要更可靠的方法来设计和优化体外系统。通过语义嵌入技术可以更好地理解和解释模拟实验，提高系统的可解释性。

Method: 扩展了标准微管分子群模型，加入DNA功能化控制，应用语义嵌入技术从模拟结果中提取语义原子，分析每帧的分解来描述外部控制参数的影响。

Result: 从模拟结果中提取的语义原子与预期行为相匹配，每帧的分解准确描述了外部控制值的影响，为模拟实验的可解释性提供了相关线索。

Conclusion: 语义嵌入技术能够有效学习分子群的行为范围，提高模拟实验的可解释性，使其更可靠地用于设计和优化体外分子系统。
...
TL;DR: 提出一种集成规划与控制框架，利用实时传感器数据动态引导AUV闭环轨迹，在复杂水下环境中实现鲁棒避障和增强机动性


<details>
  <summary>Details</summary>
Motivation: 自主水下航行器在未知环境中导航面临巨大挑战，包括能见度差、信号传输弱、动态水流等因素，导致全局定位困难、通信不可靠、避障复杂。水下传感器测量固有的噪声引入了不确定性，使规划和控制更加复杂。

Method: 提出集成规划与控制框架，基于预先设计的反馈控制器规划运动，利用实时传感器数据动态引导闭环AUV轨迹。该方法减少在线优化的计算复杂度，提高复杂水下空间的操作安全性。

Result: 通过ROS Gazebo仿真在RexRov AUV上验证了方法的有效性。与基于PID的跟踪方法进行比较评估性能，并量化AUV进入目标通信范围时的航位推算定位误差。

Conclusion: 该框架能够有效应对水下导航挑战，在复杂环境中实现鲁棒避障和增强机动性，同时减少计算负担，提高操作安全性。
...
TL;DR: ExpressMM框架结合高层语言引导规划与底层视觉-语言-动作策略，在移动机械臂协作任务中生成可表达意图的机器人行为，支持用户中断和重定向指令。


<details>
  <summary>Details</summary>
Motivation: 现有表达性机器人行为方法大多未考虑人机交互中用户可能中断、修改或重定向机器人动作的情况，需要开发支持中断交互的表达性行为框架。

Method: 提出ExpressMM框架，集成基于视觉语言模型的高层语言引导规划器（用于感知和对话推理）与底层视觉-语言-动作策略，在协作HRI任务中生成表达性行为，支持可中断交互。

Result: 在移动机械臂辅助人类协作装配场景中演示，观众评估显示ExpressMM的表达性行为帮助观察者清晰理解机器人动作和意图，支持社会适当且可理解的交互，参与者认为机器人有用、可预测且安全。

Conclusion: ExpressMM框架成功实现了在协作人机交互任务中生成表达性机器人行为，支持中断交互，提高了机器人的有用性、安全性和可预测性感知。
...
TL;DR: 提出一个统一框架，将效率与可靠性协同用于连续移动操作，通过可靠性感知轨迹规划和相位依赖切换控制器，在不确定环境中实现高效可靠的任务执行。


<details>
  <summary>Details</summary>
Motivation: 人类能够无缝融合预期规划和即时反馈来执行连续移动操作任务，实现高效可靠。但机器人复制这种行为面临根本挑战：长时域规划与实时反应性冲突，过度追求效率会损害不确定性环境中的可靠性，影响稳定感知和补偿能力。

Method: 1. 可靠性感知轨迹规划器：将可靠执行的关键要素嵌入时空优化，生成高效且可靠性有保证的全局轨迹。2. 相位依赖切换控制器：在全局轨迹跟踪（效率）和任务误差补偿（可靠性）之间无缝切换。3. 分层初始化：促进在线重规划，应对长时域规划问题的复杂性。

Result: 真实世界评估表明，该方法能在不确定性（动态扰动、感知和控制误差）下高效可靠地完成连续任务。框架能泛化到具有不同末端执行器约束的任务。相比最先进基线，始终实现最高效率，同时将任务成功率提高26.67%--81.67%。消融研究验证了各组件贡献。

Conclusion: 提出的统一框架成功协同了效率与可靠性，通过可靠性感知规划、相位依赖控制和分层初始化，使机器人能够在不确定环境中高效可靠地执行连续移动操作任务，显著优于现有方法。
...
TL;DR: CoEnv框架通过整合真实世界与仿真组件，为多机器人协作提供安全策略探索和可靠部署，在复杂多臂操作任务中实现高成功率


<details>
  <summary>Details</summary>
Motivation: 多智能体具身系统在复杂协作操作中面临空间协调、时间推理和共享工作空间感知等关键挑战，需要新的框架来解决这些问题

Method: 提出CoEnv框架，包含三个阶段：真实到仿真的场景重建、基于视觉语言模型的动作合成、以及经过验证的仿真到真实转移与碰撞检测

Result: 在具有挑战性的多臂操作基准测试中，CoEnv实现了高任务成功率和执行效率

Conclusion: CoEnv为多智能体具身AI建立了新范式，通过组合环境概念实现安全策略探索和可靠部署
...
TL;DR: 论文提出了JailWAM框架，这是首个专门针对世界行动模型(WAM)的越狱攻击与评估框架，通过视觉轨迹映射、风险判别器和双路径验证策略，在仿真环境中实现了84.2%的攻击成功率，并建立了安全评估基准。


<details>
  <summary>Details</summary>
Motivation: 世界行动模型(WAM)具有强大的物理交互能力，但现有研究极少关注其安全漏洞，特别是对越狱攻击的脆弱性。这种安全缺口可能直接威胁人身安全、财产安全和环境安全。

Method: 提出了JailWAM框架，包含三个核心组件：1) 视觉轨迹映射，将异构动作空间统一为视觉轨迹表示；2) 风险判别器，作为高召回率筛选工具；3) 双路径验证策略，先通过单图像视频动作生成进行粗筛，再通过完整闭环物理仿真进行验证。同时构建了JailWAM-Bench评估基准。

Result: 在RoboTwin仿真环境中，该框架有效暴露了物理漏洞，在最先进的LingBot-VA模型上实现了84.2%的攻击成功率。基于JailWAM可以构建鲁棒的防御机制。

Conclusion: JailWAM框架填补了WAM安全研究的空白，为设计安全可靠的机器人控制系统提供了有效的技术解决方案，能够系统评估和提升WAM的安全对齐性能。
...
TL;DR: 本文提出了一种模块化空中机器人系统（MARS）的新型控制框架，通过机械设计、虚拟四旋翼抽象和预测分配管道解决了现有方法在规模化时产生的振荡问题，实现了敏捷飞行和稳定对接。


<details>
  <summary>Details</summary>
Motivation: 现有MARS控制算法依赖简化的准静态模型和基于规则的分配，产生不连续和无界的电机指令，导致随着无人机单元数量增加而积累姿态误差，在对接、分离和航点跟踪时产生严重振荡。

Method: 1) 设计紧凑机械系统实现被动对接、无检测被动锁定和磁辅助分离；2) 引入力-力矩等效和多面体约束的虚拟四旋翼模型，显式建模可行力矩集；3) 设计两阶段预测分配管道：约束预测跟踪器计算虚拟输入，动态分配器映射到各模块产生平滑电机指令。

Result: 在超过10种配置的仿真和真实世界实验中展示了稳定对接、锁定和分离，以及有效的控制性能。首次在真实世界中实现MARS的敏捷飞行和运输，峰值俯仰角达40度，平均位置误差0.0896米。

Conclusion: 该研究通过机械创新和先进控制框架解决了MARS规模化时的控制稳定性问题，为模块化空中机器人系统提供了实用的解决方案，实现了前所未有的敏捷飞行性能。
...
Result: 通过仿真和真实机器人实验验证优化轨迹，结果展示强泛化能力，成功解决现有方法难以处理的一系列挑战性任务

Conclusion: 基于物理的仿真模型和进化策略优化方法有效解决了软机器人舀取任务的控制复杂性，实现了自适应舀取性能
...
TL;DR: ReV是一个视觉运动策略框架，通过结合稀疏参考点实现实时轨迹重规划，增强机器人操作在分布外执行错误或动态重路由时的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决机器人操作中视觉运动策略学习的基本问题：如何仅依赖原始专家演示训练，就能增强在分布外执行错误或动态重路由轨迹时的鲁棒性。

Method: 提出Referring-Aware Visuomotor Policy (ReV)闭环框架，使用耦合扩散头：全局扩散头生成全局一致但时间稀疏的动作锚点并定位参考点位置；局部扩散头基于当前时间位置自适应插值相邻锚点。仅通过对专家演示应用针对性扰动进行训练。

Result: 无需额外数据或微调方案，ReV在具有挑战性的模拟和真实世界任务中实现了更高的成功率。

Conclusion: ReV框架通过实时整合稀疏参考点，能够适应意外情况并实现动态轨迹重规划，显著提升了机器人操作的鲁棒性。
...
TL;DR: 开发了一种4自由度柔性腹腔镜器械的控制架构，集成了远端弯曲、独立远端头部旋转、轴旋转和夹持器功能，直径10毫米兼容标准套管，通过Raspberry Pi 5和Motoron控制器实现驱动，并与ATHENA并联机器人集成验证


<details>
  <summary>Details</summary>
Motivation: 微创手术减少患者创伤并缩短恢复时间，但传统腹腔镜器械受限于有限的活动范围，需要开发更灵活的器械来改善手术操作能力

Method: 设计了4自由度柔性腹腔镜器械的控制架构，包括远端弯曲、独立远端头部旋转、轴旋转和夹持器；采用Raspberry Pi 5和Motoron控制器实现驱动单元和SpaceMouse遥操作；推导并参数化了剪刀连杆机构的解析模型

Result: 预测的钳口开度与CAD测量结果一致（MAE 0.13°），与OptiTrack运动捕捉数据一致（MAE 1.43°）；通过与ATHENA并联机器人集成，在模拟胰腺手术程序中验证了系统性能

Conclusion: 成功开发并验证了一种4自由度柔性腹腔镜器械的控制系统，该器械具有10毫米直径兼容标准套管，能够提供更大的操作灵活性，有望改善微创手术的操作能力
...
TL;DR: 提出一种新颖的训练框架，通过对比模型显式地对齐分层视觉-语言-动作模型中的语言描述与动作轨迹，利用离线偏好学习提升多模态对齐效果，在LanguageTable数据集上达到与全监督微调相当的性能。


<details>
  <summary>Details</summary>
Motivation: 机器人透明度对于有效的人机协作至关重要，需要机器人的自然语言通信与其动作一致，并明确基于任务和环境。现有的分层视觉-语言-动作模型虽然能生成语言和低级动作，但在训练过程中没有考虑这些模态之间的显式对齐。

Method: 提出一个新颖的训练框架，通过对比模型评估生成语言与相应动作轨迹之间的对齐程度。该对比模型能够直接基于对齐程度对不同语言-轨迹对进行排序，从而通过离线偏好学习来优化分层视觉-语言-动作模型的基础对齐。

Result: 在LanguageTable数据集（人类语言标注轨迹的基准数据集）上应用该框架，获得了关于多模态基础表示的重要见解，同时建立了一个强大的基线，实现了与完全监督微调相当的性能，并最小化了昂贵的数据标注需求。

Conclusion: 该框架填补了现有分层视觉-语言-动作模型在训练过程中缺乏显式模态对齐的关键空白，通过对比学习和离线偏好学习有效提升了机器人语言通信与动作的一致性，为实现更透明的机器人协作提供了重要方法。
...
### [20] [Leaderless Collective Motion in Affine Formation Control over the Complex Plane](https://arxiv.org/abs/2604.05648)
*Jesus Bautista,Enric Morella,Lili Wang,Hector Garcia de Marina*

Main category: cs.RO

TL;DR: 提出了一种通过修改拉普拉斯矩阵权重实现平面仿射编队集体机动的方法，产生时变仿射变换的集体运动，无需领导者且支持更广泛的运动类型。


<details>
  <summary>Details</summary>
Motivation: 传统领导者-跟随者策略限制了机器人集群的集体运动能力，需要一种无领导者的方法来实现更广泛的仿射变换集体运动，包括平移、旋转、缩放和剪切等线性组合。

Method: 通过修改用于实现静态编队的拉普拉斯矩阵的原始权重，使用复数表示二维仿射编队中的所有相关信息，分析设计定义集体运动特征向量和特征值的解析解。

Result: 该方法能够实现参考配置的时变仿射变换集体运动，支持所有平移、旋转、缩放和剪切的线性组合，通过最多20个智能体的仿真验证了理论结果。

Conclusion: 提出的无领导者仿射编队集体机动方法比传统策略更灵活，能够实现更广泛的集体运动类型，为机器人集群的协调控制提供了新的设计框架。
...
### [21] [A1: A Fully Transparent Open-Source, Adaptive and Efficient Truncated Vision-Language-Action Model](https://arxiv.org/abs/2604.05672)
...
TL;DR: A1是一个开源VLA框架，通过自适应推理方案和层间截断流匹配技术，在保持高性能的同时大幅降低推理成本，实现低成本、高吞吐的机器人操作。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型部署面临高成本问题：十亿级VLM主干和迭代扩散/流式动作头导致高延迟和高计算开销，难以在普通硬件上实现实时控制。

Method: 采用预算感知的自适应推理方案，监控VLM中间层动作一致性以触发提前终止；提出层间截断流匹配技术，跨层预热去噪过程，减少有效去噪迭代次数。

Result: 在仿真基准和真实机器人上达到最先进成功率，同时显著降低推理成本：流匹配推理延迟降低72%，主干计算减少76.6%，性能损失极小；在RoboChallenge上平均成功率29.00%，优于多个基线模型。

Conclusion: A1框架通过优化完整推理流水线，实现了低成本、高吞吐的VLA模型部署，为开放世界机器人操作的实用化提供了可行解决方案。
...
### [22] [Rectified Schrödinger Bridge Matching for Few-Step Visual Navigation](https://arxiv.org/abs/2604.05673)
*Wuyang Luan,Junhui Li,Weiguang Zhao,Wenjian Zhang,Tieru Wu,Rui Ma*

Main category: cs.RO

TL;DR: 提出Rectified Schrödinger Bridge Matching (RSBM)框架，通过共享速度场结构和调整熵正则化参数ε，在3步积分内实现高效视觉导航，平衡多模态覆盖和路径平直度。
...
Method: 提出RSBM框架，利用标准薛定谔桥(ε=1)和确定性最优传输(ε→0)之间的共享速度场结构，通过单一熵正则化参数ε控制。证明条件速度场函数形式在整个ε谱上不变，且减小ε可线性降低条件速度方差。结合学习条件先验缩短传输距离，在中间ε值平衡多模态覆盖和路径平直度。

Result: 标准桥方法需要≥10步收敛，而RSBM仅用3步积分就达到94%余弦相似度和92%成功率，无需蒸馏或多阶段训练，显著缩小高保真生成策略与低延迟需求之间的差距。

Conclusion: RSBM通过速度场结构不变性和降低条件速度方差，实现了高效稳定的视觉导航策略，在极少积分步骤下保持高性能，为具身AI的实时控制提供了可行解决方案。
...
### [23] [Dynamic Control Allocation for Dual-Tilt UAV Platforms](https://arxiv.org/abs/2604.05677)
*Marcello Sorge,Federico Ciresola,Giulia Michieletto,Angelo Cenedese*

Main category: cs.RO

TL;DR: 该论文研究了六旋翼无人机平台在轨迹跟踪任务中的动态控制分配问题，考虑了双倾斜螺旋桨设计和执行器饱和约束。


<details>
  <summary>Details</summary>
Motivation: 研究六旋翼无人机在轨迹跟踪任务中的控制分配问题，特别关注双倾斜螺旋桨平台的设计，这种设计允许每个螺旋桨沿两个正交轴独立倾斜，从而提供更大的控制灵活性。

Method: 采用分层控制结构：高层控制器生成跟踪任务所需的力矩，控制分配法则确保执行器产生这些力矩。分配器对执行器集施加期望的一阶动力学，并利用系统冗余性通过目标函数优化执行器状态。特别考虑了执行器饱和约束的理论建模。

Result: 通过数值仿真验证了控制分配策略的有效性，分析了螺旋桨倾斜角的作用，并通过目标函数中的非对称形状约束来优化系统性能。

Conclusion: 该研究提出了一种考虑执行器饱和约束的六旋翼无人机动态控制分配方法，通过双倾斜螺旋桨设计和分层控制结构提高了轨迹跟踪性能，为无人机控制系统的实际应用提供了理论支持。
...
### [24] [GraspSense: Physically Grounded Grasp and Grip Planning for a Dexterous Robotic Hand via Language-Guided Perception and Force Maps](https://arxiv.org/abs/2604.05697)
*Elizaveta Semenyakina,Ivan Snegirev,Mariya Lezina,Miguel Altamirano Cabrera,Safina Gulyamova,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: 提出一种基于局部可承受接触载荷图的机器人抓取选择和力调节方法，通过物理感知的几何分析计算表面各位置的最大可承受侧向接触力，优化抓取选择并调节手指刚度，确保在结构薄弱区域安全抓取。


<details>
  <summary>Details</summary>
Motivation: 现有抓取规划器通常将物体表面视为结构均匀的，但实际物体具有空间非均匀的机械特性。在薄弱区域接触可能导致物体损坏，即使几何上完美的抓取也无法避免。需要物理基础的接触策略来考虑物体的机械特性。

Method: 1. 识别目标物体并使用SAM3D重建3D几何模型；2. 通过物理感知的几何分析计算力图，编码表面各位置在不发生变形情况下的最大可承受侧向接触力；3. 基于几何有效性和任务目标一致性筛选抓取候选；4. 当多个候选在经典指标下相当时，使用力图感知标准重新排序，优先选择接触点在机械可承受区域的抓取；5. 阻抗控制器根据接触点的局部可承受力缩放每个手指的刚度。

Result: 在纸杯、塑料杯和玻璃杯上的验证表明，该方法能一致地选择结构更强的接触区域，并将抓握力保持在安全范围内。实现了从纯几何问题到物理基础的联合规划问题的转变。

Conclusion: 该工作将灵巧操作从纯几何问题重新定义为物理基础的联合规划问题，结合抓取选择和抓握执行，为未来人形系统提供了更安全可靠的抓取策略。通过考虑物体的机械特性，能够在结构薄弱区域实现安全抓取。
...
### [25] [Hazard Management in Robot-Assisted Mammography Support](https://arxiv.org/abs/2604.05749)
*Ioannis Stefanakos,Roisin Bradley,Radu Calinescu,Beverley Townsend,Tianyuan Wang,Jihong Zhu*

Main category: cs.RO

TL;DR: 该论文提出了一种用于MammoBot辅助机器人系统的危险管理方法，结合利益相关者引导的过程建模、SHARD和STPA分析，识别临床环境中人机交互的安全风险。


<details>
  <summary>Details</summary>
Motivation: 机器人和具身AI系统在临床环境中具有改善护理可及性和质量的潜力，但与脆弱患者的近距离接触引入了重大安全风险，需要从早期开发阶段确保安全性。

Method: 结合利益相关者引导的过程建模、软件危险分析与设计解决方案(SHARD)以及系统理论过程分析(STPA)，与临床医生、机器人专家和患者代表协作定义机器人辅助工作流程。

Result: 分析显示许多危险并非来自组件故障，而是源于时间不匹配、过早行动和系统状态误解，这些危险被转化为细化和额外的安全要求，约束系统行为并减少对正确人类计时或解释的依赖。
...
### [26] [Precise Aggressive Aerial Maneuvers with Sensorimotor Policies](https://arxiv.org/abs/2604.05828)
*Tianyue Wu,Guangtong Xu,Zihan Wang,Junxiao Lin,Tianyang Chen,Yuze Wu,Zhichao Han,Zhiyang Liu,Fei Gao*

Main category: cs.RO

TL;DR: 使用强化学习训练端到端传感器运动策略，使四旋翼无人机能够在仅依赖机载视觉和本体感知的情况下，以倾斜姿态穿越狭窄缝隙，实现厘米级间隙的精确穿越。


<details>
  <summary>Details</summary>
Motivation: 解决四旋翼无人机在仅使用轻量级机载传感器的情况下执行精确激进机动的问题，特别是穿越狭窄缝隙的挑战，这对于扩展无人机在复杂环境中的可达区域至关重要。

Method: 开发传感器运动策略，将机载视觉和本体感知直接映射到低级控制命令。使用强化学习进行端到端策略蒸馏训练，采用基于模型规划器生成的轨迹进行初始化，以克服模型无关强化学习在受限解空间中的探索困难。通过精心设计的仿真到现实迁移方法实现实际部署。

Result: 该方法使四旋翼无人机能够在5厘米间隙下穿越矩形缝隙，倾斜角度可达90度，无需预先知道缝隙的位置或方向。即使未经动态缝隙训练，策略也能反应性地控制无人机穿越移动缝隙。在密集排列的狭窄缝隙轨道上验证了方法的有效性，并展示了策略对几何多样性缝隙的适应性。

Conclusion: 提出的策略学习方法能够使四旋翼无人机仅依赖机载传感器实现精确的激进机动，成功穿越狭窄缝隙，展示了端到端强化学习在无人机复杂机动控制中的潜力，无需依赖手动定义的穿越姿态和视觉特征。
...
### [27] [BiCoord: A Bimanual Manipulation Benchmark towards Long-Horizon Spatial-Temporal Coordination](https://arxiv.org/abs/2604.05831)
*Xingyu Peng,Chen Gao,Liankai Jin,Annan Li,Si Liu*

Main category: cs.RO

TL;DR: BiCoord是一个用于长时程紧密协调的双臂操作基准测试，包含需要连续臂间依赖和动态角色交换的任务，并提出了从时空角度评估协调性的量化指标。


<details>
  <summary>Details</summary>
Motivation: 现有双臂操作基准测试（如RoboTwin和RLBench2）中的任务通常是短时程且协调松散，未能捕捉真实世界双臂行为中固有的时空耦合特性，需要建立能够评估长时程紧密协调的基准。

Method: 提出了BiCoord基准测试，包含多样化的长时程任务，这些任务需要连续的双臂依赖和动态角色交换；同时开发了一套从时间、空间和时空三个维度评估协调性的量化指标。
...
### [28] [Dialogue based Interactive Explanations for Safety Decisions in Human Robot Collaboration](https://arxiv.org/abs/2604.05896)
*Yifan Xu,Xiao Zhan,Akilu Yunusa Kaltungo,Ming Shan Ng,Tsukasa Ishizawa,Kota Fujimoto,Clara Cheung*

Main category: cs.RO

TL;DR: 本文提出了一种基于对话的框架，用于解释人机协作中的安全决策，将解释与基于约束的安全评估紧密结合，支持因果、对比和反事实查询，确保交互探索不放松安全保证。


<details>
  <summary>Details</summary>
Motivation: 随着机器人在共享安全关键环境中操作，仅安全行动已不足够，还需要让人类协作伙伴理解其安全决策。当前人机协作中，停止或切换模式等行为通常由内部安全约束触发，但这些约束对附近工作人员是不透明的。

Method: 提出基于对话的交互式安全决策解释框架，将解释与基于约束的安全评估紧密结合。解释直接来自记录决策轨迹，支持用户提出因果（"为什么？"）、对比（"为什么不？"）和反事实（"如果...会怎样？"）查询。反事实推理在固定认证安全参数下有界评估，确保交互探索不放松操作保证。

Result: 在建筑机器人场景中实例化该框架，提供结构化操作轨迹，展示约束感知对话如何澄清安全干预并支持协调任务恢复。

Conclusion: 通过将解释视为安全控制的操作接口，这项工作为人机协作中交互式、安全感知的自主性设计提供了新视角。
...
### [29] [You're Pushing My Buttons: Instrumented Learning of Gentle Button Presses](https://arxiv.org/abs/2604.05954)
*Raman Talwar,Remko Proesmans,Thomas Lips,Andreas Verleysen,Francis wyffels*

Main category: cs.RO

TL;DR: 研究探讨了在接触丰富的操作任务中，通过训练时仪器化（物体传感器化）来提升策略性能，而不增加部署时依赖。以按钮按压为测试平台，使用指尖麦克风捕捉接触相关音频，通过特权监督训练音频编码器作为接触事件检测器，结合模仿学习策略，最终策略仅使用视觉和音频进行推理。


<details>
  <summary>Details</summary>
Motivation: 仅从摄像头和本体感知学习接触丰富的操作任务很困难，因为接触事件只能被部分观察到。研究旨在探索训练时仪器化是否能提升策略性能，同时避免在部署时产生依赖。

Method: 以按钮按压为测试平台，使用指尖麦克风捕捉接触相关音频。利用仪器化的按钮状态信号作为特权监督，微调音频编码器成为接触事件检测器。结合三种策略将生成的表示与模仿学习结合，最终策略在推理时仅使用视觉和音频。

Result: 按钮按压成功率在不同方法间相似，但仪器化引导的音频表示能持续减少接触力。这表明仪器化作为训练时的辅助目标对学习接触丰富的操作策略具有实际价值。

Conclusion: 仪器化作为训练时的辅助目标，能够有效帮助学习接触丰富的操作策略，提升性能而不增加部署依赖，为机器人操作学习提供了实用方法。
...
### [30] [Automating Manual Tasks through Intuitive Robot Programming and Cognitive Robotics](https://arxiv.org/abs/2604.05978)
*Bijan Kavousian,Petar Tesic,Oliver Petrovic,Christian Brecher*

Main category: cs.RO

TL;DR: 该论文提出了一种基于自然语言和手势的机器人编程新方法，利用大语言模型和计算机视觉技术，通过自然反馈机制确保安全性和用户接受度。


<details>
  <summary>Details</summary>
Motivation: 受人类自然交互启发，旨在开发一种直观的终端用户机器人编程方法，使非专业用户也能轻松编程机器人，提高编程的易用性和可访问性。

Method: 使用大语言模型（LLMs）处理自然语言指令，结合计算机视觉（CV）识别手势，将自然交互转化为机器人程序。系统通过澄清问题和视觉表示提供自然反馈，允许用户审查和调整生成的程序。

Result: 开发了一个能够将自然语言和手势转化为机器人程序的系统，通过自然反馈机制确保程序的安全性和透明度，提高用户接受度。

Conclusion: 该方法为机器人编程提供了一种直观的自然交互方式，通过结合LLMs和CV技术以及自然反馈机制，实现了安全、透明且用户友好的机器人编程体验。
...
### [31] [Force Polytope-Based Cant-Angle Selection for Tilting Hexarotor UAVs](https://arxiv.org/abs/2604.05998)
*Alberto Piccina,Massimiliano Bertoni,Angelo Cenedese,Giulia Michieletto*

Main category: cs.RO

TL;DR: 该研究提出了一种轻量级控制框架，用于星形相互依赖倾斜六旋翼无人机执行交互任务，通过离线计算零力矩力多面体查找表来优化倾斜角度选择，在保证高效和平滑的同时显著减少计算时间。
...
Method: 提出轻量级控制框架，使用离线计算的零力矩力多面体查找表来识别期望控制力下的可行倾斜角度，并通过平衡效率和平滑性选择最优角度。该框架与几何全姿态控制器集成。

Result: 结果显示计算时间显著减少，同时姿态跟踪性能得到改善，执行效率具有竞争力。在Simscape中进行的完整墙壁检查任务的物理仿真进一步证实了该策略在交互场景中的可行性。

Conclusion: 该控制框架为倾斜六旋翼无人机在交互任务中的应用提供了一种高效、轻量级的解决方案，通过优化倾斜角度选择实现了计算效率和性能的平衡。
...
### [32] [A Co-Design Framework for High-Performance Jumping of a Five-Bar Monoped with Actuator Optimization](https://arxiv.org/abs/2604.06025)
*Aastha Mishra,Aman Singh,Shishir Kolathaya*

Main category: cs.RO

TL;DR: 提出一个针对平面五杆闭链单足机器人的协同设计框架，联合优化机械设计、电机齿轮箱参数和控制参数，实现动态跳跃性能最大化


<details>
  <summary>Details</summary>
Motivation: 现有协同设计研究大多关注连杆尺寸和传动比优化，忽略了详细的执行器设计（特别是电机和齿轮箱参数优化），且主要局限于开链机构。需要开发能够同时优化机械设计、执行器参数和控制的综合框架

Method: 采用两阶段优化方法：第一阶段执行器优化生成齿轮比到执行器质量、效率和峰值扭矩的映射；第二阶段使用CMA-ES算法协同优化机器人设计和控制参数

Result: 仿真结果显示，与基准设计相比，跳跃距离提升约42%，机械能耗降低15.8%，证明了该框架在识别高性能、高能效平面跳跃机器人最优参数方面的有效性

Conclusion: 提出的协同设计框架能够有效识别平面闭链跳跃机器人的最优机械设计、执行器参数和控制参数，显著提升跳跃性能并降低能耗
...
### [33] [HiPolicy: Hierarchical Multi-Frequency Action Chunking for Policy Learning](https://arxiv.org/abs/2604.06067)
*Jiyao Zhang,Zimu Han,Junhan Wang,Xionghao Wu,Shihong Lin,Jinzhou Li,Hongwei Fan,Ruihai Wu,Dongjiang Li,Hao Dong*

Main category: cs.RO

TL;DR: HiPolicy：一种分层多频率动作分块框架，通过联合预测不同频率的动作序列来平衡长时程依赖建模与细粒度闭环控制，提升模仿学习性能和执行效率。
...
Method: 提出HiPolicy分层多频率动作分块框架：1）联合预测不同频率的动作序列以捕捉粗粒度高级规划和精确反应性动作；2）从历史观测中提取并融合与各频率对齐的分层特征；3）引入基于熵的引导执行机制，根据动作不确定性自适应平衡长时程规划与细粒度控制。

Result: 在多样化的模拟基准测试和真实世界操作任务上的实验表明，HiPolicy可以无缝集成到现有的2D和3D生成策略中，在显著提升执行效率的同时带来一致的性能改进。

Conclusion: HiPolicy通过分层多频率动作分块有效解决了机器人模仿学习中长时程依赖与细粒度控制之间的权衡问题，为现有策略提供了可扩展的性能提升方案。
...
### [34] [Intuitive Human-Robot Interaction: Development and Evaluation of a Gesture-Based User Interface for Object Selection](https://arxiv.org/abs/2604.06073)
...
<details>
  <summary>Details</summary>
Motivation: 手势是人类之间自然的交流方式，也可以用于人机交互，需要探索手势在机器人交互中的应用

Method: 开发基于手势的用户界面，使用指向和点击手势进行物体选择，并通过20名参与者的实验进行评估

Result: 实验评估了准确性和选择时间，展示了高效协作的潜力

Conclusion: 基于手势的界面在人机交互中具有潜力，能够实现高效的协作
...
### [35] [Learning-Guided Force-Feedback Model Predictive Control with Obstacle Avoidance for Robotic Deburring](https://arxiv.org/abs/2604.06133)
*Krzysztof Wojciechowski,Ege Gursoy,Arthur Haffemayer,Sebastien Kleff,Vincent Bonnet,Florent Lamiraux,Nicolas Mansard*

Main category: cs.RO

TL;DR: 该研究提出了一种将力反馈模型预测控制与扩散运动先验相结合的框架，用于解决工业去毛刺等接触密集型任务中的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统的模型预测控制方法往往忽略实时力反馈，在碰撞约束下的接触密集型工业任务中表现不佳。去毛刺任务需要精确的工具插入、稳定的力调节和无碰撞的圆周运动，这些超出了标准MPC管道的能力。

Method: 提出一个集成力反馈MPC与扩散运动先验的框架。扩散模型作为运动策略的记忆，提供跨多个任务实例的鲁棒初始化和适应，而MPC确保具有显式力跟踪、扭矩可行性和碰撞避免的安全执行。

Result: 在扭矩控制机械臂上进行工业去毛刺任务的实验验证表明，该方法能够实现可靠的工具插入、精确的法向力跟踪，即使在难以到达的配置和障碍约束下也能完成圆周去毛刺运动。

Conclusion: 这是首次将扩散运动先验与力反馈MPC集成，用于碰撞感知的接触密集型工业任务，为解决复杂工业操作提供了有效框架。
