...
Motivation: 传统的手工设计模板模型（如LIP、SLIP）只能近似底层动力学，而数据驱动方法虽然能提取更准确的低维表示，但难以保证潜在空间的稳定性和安全性能够有意义地传递回完整系统。

Method: 使用自编码器从轨迹数据中学习低维潜在状态，结合学习的潜在庞加莱映射捕捉步态到步态的移动动力学，通过解码器将潜在空间的李雅普诺夫分析和吸引域构造提升回完整状态空间。

Result: 在模拟跳跃机器人和全身人形机器人移动实验中，HALO能够产生保留有意义稳定性结构的低维模型，并预测完整系统的吸引域边界。

Conclusion: HALO框架成功桥接了数据驱动方法与稳定性分析之间的差距，为复杂混合系统（如腿式机器人）提供了既能保持准确性又能保证稳定性传递的降阶建模方法。
...
TL;DR: 该研究通过调节Aerobat机器人计算结构中第一半径连杆R1的有效长度来实现推力调节，探索了多种执行方法，最终采用压电滑移执行器实现连杆长度调制，为独立翼推力控制提供了可行方案。


<details>
  <summary>Details</summary>
Motivation: Aerobat蝙蝠仿生扑翼机器人采用单电机驱动的计算结构设计，虽然重量轻但双翼耦合到共享输入电机，无法实现独立推力控制和不对称机动。需要研究推力调节方法来克服这一限制。

Method: 1. 静态实验：使用FDM打印的三种长度R1连杆在不同扑翼频率下测试；2. 弦张力调节机制实验；3. 评估微伺服和压电滑移执行器；4. 设计力放大机制原型，最终采用直接驱动变长机制；5. 进行初步台架测试。

Result: 1. R1长度增加1.5mm使峰值升力提高37%，并改变了下冲程中的峰值力时机；2. 弦张力和微伺服方法因结构柔性和电机脆弱性失败；3. 选择TULA-50压电滑移执行器；4. 最终机制在台架测试中可行，但力输出不足无法进行动态扑翼测试。

Conclusion: 通过嵌入式压电滑移执行器实现连杆长度调制是独立翼推力控制的可行方法，为Aerobat机器人提供了推力调节能力，尽管当前力输出仍需改进。
...
TL;DR: 提出了一种用于人-四旋翼协同负载运输的新型导纳控制器，通过主动控制绞盘实现安全高效的合作运输


<details>
  <summary>Details</summary>
Motivation: 人-机器人协作在物理人机交互应用中至关重要，但现有研究主要集中在机械臂领域，而人-四旋翼协同负载运输研究尚处于起步阶段，需要开发更安全有效的控制方法

Method: 设计了一种新型导纳控制器，用于配备主动控制绞盘的四旋翼无人机，考虑了系统的耦合动力学，使四旋翼和缆绳能够动态适应接触力，提高响应性

Result: 实验验证了控制器在整个协同负载运输过程中的任务自适应能力，包括原地装卸和负载运输任务。与传统方法相比，在系统响应性和运动平滑性方面表现更优

Conclusion: 所提出的方法通过考虑系统耦合动力学，显著提高了人-四旋翼协同负载运输的能力，为这一新兴领域提供了有效的控制解决方案
...
TL;DR: GMP是一种视觉运动策略，通过门控记忆机制和交叉注意力模块，选择性激活历史信息并构建有效记忆表示，解决非马尔可夫任务中的分布偏移和过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 机器人操作任务具有不同的记忆需求，从无需记忆的马尔可夫任务到依赖单次或多次交互历史信息的非马尔可夫任务。简单地扩展视觉运动策略的观测历史通常会导致性能显著下降，主要原因是分布偏移和过拟合。

Method: 提出了门控记忆策略（GMP），包含三个关键技术：1）学习记忆门机制，选择性激活历史上下文；2）轻量级交叉注意力模块，构建有效的潜在记忆表示；3）向历史动作注入扩散噪声，增强对噪声或不准确历史的鲁棒性。

Result: 在提出的非马尔可夫基准MemMimic上，GMP相比长历史基线平均成功率提升30.1%，同时在RoboMimic的马尔可夫任务上保持竞争力。

Conclusion: GMP通过选择性记忆激活和有效记忆表示学习，解决了非马尔可夫任务中的分布偏移和过拟合问题，在保持马尔可夫任务性能的同时显著提升了非马尔可夫任务的性能。
...
TL;DR: RoomRecon：一种用于3D房间模型的交互式实时扫描和纹理处理管道，通过AR引导图像采集和生成式AI提升纹理质量，专注于永久性房间元素以实现可定制性。


<details>
  <summary>Details</summary>
Motivation: 当前RGB-D传感器和3D重建技术存在局限性：无法反映微小场景变化而需要完全重新捕获、缺乏语义场景理解、纹理处理问题影响3D模型视觉质量，这些问题影响了VR体验、室内设计和房地产等应用所需的真实感。

Method: 提出RoomRecon交互式实时扫描和纹理处理管道，采用两阶段纹理处理流程：1）AR引导图像采集进行纹理处理；2）集成生成式AI模型提升纹理质量。专注于墙壁、地板和天花板等永久性房间元素，以实现易于定制的3D模型。

Result: 在多种室内空间进行实验评估纹理质量和速度。定量结果和用户研究表明，RoomRecon在纹理质量和设备计算时间方面优于最先进的方法。

Conclusion: RoomRecon通过创新的两阶段纹理处理管道，结合AR引导和生成式AI，有效解决了当前室内3D重建技术的局限性，在纹理质量和实时性能方面表现出色，为VR、AR、室内设计等应用提供了更好的解决方案。
...
TL;DR: AeroBridge-TTA：一种针对无人机语言引导控制中执行不匹配问题的测试时自适应方法，通过在线更新潜在表示来适应动态变化，在分布外条件下显著提升性能


<details>
  <summary>Details</summary>
Motivation: 语言引导的无人机经常因执行不匹配而失败——即规划轨迹与控制器在实际动态变化（质量变化、阻力变化、执行器延迟、风等）时的跟踪能力之间的差距，现有方法在训练动态与实际动态不同时表现不佳

Method: 提出AeroBridge-TTA语言条件控制管道，包含三个部分：1) 语言编码器将命令映射为子目标；2) 基于子目标和学习潜在表示的自适应策略；3) 测试时自适应模块，从观察到的状态转移中在线更新潜在表示

Result: 在5个语言条件无人机任务和13种不匹配条件下，AeroBridge-TTA在分布内与PPO-MLP基线相当，在5种分布外条件下全部获胜，平均提升22.0个百分点（62.7% vs 40.7%）；仅改变步长α的消融实验显示潜在更新本身带来4.6倍的OOD提升

Conclusion: 测试时自适应通过在线更新潜在表示能有效解决语言引导无人机控制中的执行不匹配问题，在动态变化条件下显著提升性能，且性能提升主要来自分布外条件
...
<details>
  <summary>Details</summary>
Motivation: 现有卫星星座设计方法存在局限性：对称参数化设计（如Walker星座）限制了设计空间，而元启发式方法需要大量计算和迭代。梯度优化因覆盖和重访指标的非可微性而被认为不可行。

Method: 引入了四种连续松弛技术：软sigmoid可见性、noisy-OR多卫星聚合、leaky积分器重访间隔跟踪和LogSumExp软最大值，结合∂SGP4可微分轨道传播器，构建了从轨道参数到任务目标的完全可微分流程。

Result: 该方法能从不规则初始化中恢复Walker-Delta几何结构，仅通过梯度就能发现具有远地点驻留的椭圆Molniya类轨道。相比模拟退火、遗传算法和差分进化基线，梯度方法仅需约750次评估就能恢复Walker等效几何，而三种黑盒基线即使使用四倍评估预算也只能达到明显更差的重访性能。

Conclusion: 通过连续松弛技术使卫星星座设计中的覆盖和重访指标可微分，实现了高效的梯度优化方法，显著减少了计算需求，为星座设计提供了新的优化途径。
...
TL;DR: RoboWM-Bench：首个面向机器人操作任务的视频世界模型基准测试，通过将生成的视频行为转换为可执行动作序列并进行机器人执行验证，系统评估视频世界模型生成物理可执行行为的能力。


<details>
  <summary>Details</summary>
Motivation: 当前大规模视频世界模型虽然能生成逼真的未来预测视频，但视觉真实并不等同于物理可行性。从生成视频推断的行为可能违反物理动力学，在机器人执行时失败。现有基准测试主要关注感知或诊断，缺乏系统评估预测行为能否转化为可执行动作完成目标任务的能力。

Method: 提出RoboWM-Bench基准测试，将人类手部操作和机器人操作视频中生成的行为转换为具身动作序列，并通过机器人执行进行验证。该基准涵盖多样化操作场景，建立统一协议确保评估的一致性和可重复性。

Result: 评估当前最先进的视频世界模型发现，可靠生成物理可执行行为仍是一个开放挑战。常见失败模式包括空间推理错误、接触预测不稳定和非物理形变。虽然在操作数据上进行微调能带来改进，但物理不一致性仍然存在。

Conclusion: RoboWM-Bench为具身评估视频世界模型提供了系统框架，揭示了当前模型在生成物理可行行为方面的局限性，表明需要更物理基础化的视频生成方法以支持机器人学习应用。
...
TL;DR: 本文提出了一种选择性对抗运动先验（AMP）策略，使仿人机器人能够在统一强化学习框架中掌握五种不同步态，针对稳定性和动态性需求采用不同的AMP应用策略。


<details>
  <summary>Details</summary>
Motivation: 在统一强化学习框架中学习仿人机器人的多样化运动技能面临挑战，因为不同步态对稳定性和动态表现力的要求相互冲突。需要一种方法能够同时处理周期性稳定步态和高度动态步态。

Method: 采用选择性AMP策略：对周期性、稳定性关键的步态（行走、正步走、爬楼梯）应用AMP以加速收敛并抑制异常行为；对高度动态步态（跑步、跳跃）则故意省略AMP以避免过度约束运动。使用PPO算法在仿真中训练，通过零样本仿真到真实转移部署到12自由度仿人机器人。

Result: 选择性AMP策略在所有五种步态上都优于统一AMP策略，实现了更快的收敛速度、更低的跟踪误差，在稳定性重点步态上获得更高的成功率，同时不牺牲动态步态所需的敏捷性。

Conclusion: 选择性AMP方法有效解决了统一强化学习框架中多样化步态学习的冲突需求，通过针对不同步态特性调整AMP应用，实现了稳定性和动态表现力的平衡，成功在物理机器人上展示了五种不同步态。
...
TL;DR: 本文提出了一种基于高斯过程的路径规划方法OLAhGP，用于机器人环境探索和监测，能够适应多模态环境传感数据并整合状态和输入约束，在藻华监测场景中表现出优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 高效的路径规划需要整合所有可用信息源，特别是在机器人环境探索和监测任务中，路径规划高度依赖于当前对世界的认知。为了捕捉认知中的不确定性，需要开发能够适应多模态传感数据并考虑约束条件的路径规划方法。

Method: 提出基于高斯过程的路径规划方法OLAhGP，采用滚动时域方式优化未来航点，成本函数基于这些航点的高斯过程后验分布。该方法能够适应多模态环境传感数据并整合状态和输入约束。

Result: 在自主水面舰艇上使用海洋藻华数据进行模拟和实验验证，相比现有方法有显著改进。在相同采样数量下，该方法生成更具信息量的路径，在叶绿素a丰富水域中识别藻华的准确性更高，通过总误分类概率和感兴趣区域二元误分类率衡量。

Conclusion: OLAhGP方法在环境监测路径规划中表现出优越性能，能够有效整合不确定性信息并考虑约束条件，为机器人环境探索提供了更有效的解决方案。
...
TL;DR: ViLiNT：基于多模态注意力机制的机器人目标导航策略，通过融合RGB图像、3D LiDAR点云、目标嵌入和机器人本体描述符，结合扩散模型生成轨迹，并使用路径通过性预测头进行轨迹评分，显著提升零样本迁移的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于监督学习的地面机器人目标导航模型在零样本迁移方面表现良好，但在环境、机器人或传感器配置变化等分布偏移情况下，其避障能力会显著下降。需要开发更鲁棒的导航模型来应对这些挑战。

Method: 提出ViLiNT多模态注意力策略：1）使用transformer架构融合RGB图像、3D LiDAR点云、目标嵌入和机器人本体描述符，捕捉互补的几何和外观特征；2）用transformer输出条件化扩散模型生成可导航轨迹；3）训练路径通过性预测头对扩散模型生成的轨迹进行评分和排序；4）通过机器人本体令牌使模型能根据机器人尺寸生成和选择合适轨迹。

Result: 在三个模拟环境中，ViLiNT相比最先进的纯视觉基线（NoMaD）平均提升166%的成功率。在真实世界障碍场中的部署也证实了性能提升，表明多模态融合与碰撞预测机制的结合能显著提高越野导航的鲁棒性。

Conclusion: ViLiNT通过多模态融合和基于机器人本体的轨迹生成与选择机制，有效提升了目标导航模型在分布偏移情况下的鲁棒性，为机器人越野导航提供了更可靠的解决方案。
...
TL;DR: 人类对机器人团队的感知研究：通过两个实验发现，群体行为变化影响人们对机器人团队的温暖感和能力感感知，这些社会感知比任务表现更能预测团队偏好


<details>
  <summary>Details</summary>
Motivation: 随着机器人团队越来越多地与人类协作，理解人类如何感知机器人团队对于设计有效的人机团队至关重要。虽然先前研究探讨了人类如何解释和评估单个智能体的能力和意图，但对机器人团队的社会感知研究相对不足。

Method: 基于能力-温暖度框架，进行了两项研究：研究1让参与者作为观察者，研究2让参与者作为操作者。通过操纵群体在完成集体搜索任务时的行为（广播时长、分离距离、个体机器人速度），测量参与者对群体行为的社会感知。

Result: 群体行为变化一致影响参与者对温暖度和能力的感知：较长的广播时长增加感知温暖度；较大的分离距离增加感知能力；个体机器人速度对两种感知均无影响。社会感知比任务表现更能预测参与者的团队偏好，参与者更喜欢既温暖又有能力的机器人团队，而不是完成任务最快的团队。

Conclusion: 人机交互动态塑造社会感知，在设计用于有效人机协作的机器人群体时，需要同时整合技术和社会考量。温暖度和能力感知是影响人类对机器人团队偏好的关键因素。
...
TL;DR: 该研究将稀疏门控专家混合架构应用于视觉机器人跑酷，相比传统MLP架构，在相同激活参数下性能显著提升，成功通过障碍的试验次数翻倍。


<details>
  <summary>Details</summary>
Motivation: 机器人跑酷是极具挑战性的运动控制基准，需要处理大高度差等复杂地形。现有方法多使用密集激活的MLP架构，而稀疏门控专家混合架构在大语言模型中已被证明能有效提升可扩展性和性能，本研究探索其在视觉机器人跑酷中的应用价值。

Method: 在控制设置下比较标准MLP和稀疏门控MoE架构的控制策略，确保推理时激活参数数量匹配。在Unitree Go2四足机器人上进行真实环境实验，评估两种架构在通过大型障碍物时的性能表现。

Result: MoE策略在通过大型障碍物时成功试验次数是标准MLP基线的两倍。要达到与MoE相当的性能，标准MLP需要将参数数量扩展到与MoE总参数匹配，但这会导致计算时间增加14.3%。
...
TL;DR: 提出基于虚拟现实的机器人语音交互开发平台，通过中断与纠正、可轮询性、延迟测量优化和时间精确动作重现等特性，改善人机交互的流畅性


<details>
  <summary>Details</summary>
Motivation: 当前基于语音的人机交互体验仍然笨拙且令人沮丧，缺乏合适的开发平台来改进交互流畅性，即使在用于数据收集和原型设计的Wizard-of-Oz操作模式中也是如此

Method: 提出中断与纠正、可轮询性、延迟测量优化和时间精确动作重现作为流畅WoZ系统的关键标准，并开发了一个满足这些标准的虚拟现实移动操作机器人人机交互仿真环境

Result: 开发了一个虚拟现实HRI仿真环境，专门针对移动操作机器人设计，能够满足提出的流畅WoZ系统标准

Conclusion: 通过提出关键标准并开发相应的VR仿真环境，为解决机器人语音交互中的流畅性问题提供了有效的开发平台，特别适用于Wizard-of-Oz模式下的数据收集和原型设计
...
TL;DR: 提出M²GRPO框架，结合Mamba策略与分组相对策略优化，解决仿生水下机器人协同追捕中的长时决策、部分可观测性和多机协调问题，在模拟和真实实验中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统协同追捕策略学习方法在仿生水下机器人中面临挑战：长时决策、部分可观测性、机器人间协调需要表达能力和稳定性。需要同时满足表达性和稳定性的解决方案。

Method: 提出M²GRPO框架，整合选择性状态空间Mamba策略与分组相对策略优化。Mamba策略利用观测历史捕捉长时依赖，基于注意力的关系特征编码智能体间交互，通过归一化高斯采样产生有界连续动作。分组相对优势通过归一化每回合内智能体奖励获得，通过GRPO的多智能体扩展进行优化，减少训练资源需求。

Result: 在团队规模和逃避策略不同的广泛模拟和真实泳池实验中，M²GRPO在追捕成功率和捕获效率上始终优于MAPPO和循环基线方法。框架为仿生机器人系统的协同水下追捕提供了实用且可扩展的解决方案。

Conclusion: M²GRPO框架通过整合Mamba策略和分组相对策略优化，有效解决了仿生水下机器人协同追捕中的关键挑战，在表达能力和稳定性之间取得了良好平衡，为实际应用提供了可行的解决方案。
...
TL;DR: 本文提出了两种处理可变拓扑机构动力学问题的过渡条件方法，分别基于冗余坐标的投影运动方程和最小坐标的Voronets方程，并讨论了它们的计算特性。


<details>
  <summary>Details</summary>
Motivation: 许多机械系统会因运动学拓扑变化而改变其自由度，这些变化可能由理想接触、静摩擦或机构部件的受控锁定引起。在人与机器交互中，机构部件的受控锁定正变得越来越重要。预测可变拓扑机构的动态行为需要解决非光滑动力学问题，核心挑战是在拓扑切换事件中建立物理上有意义的过渡条件。

Method: 提出了两种过渡条件方法：1）使用基于冗余坐标的投影运动方程；2）使用基于最小坐标的Voronets方程。讨论了这两种方法的计算特性，并应用于平面3R机构和6自由度工业机械臂的关节锁定问题。

Result: 成功提出了物理上有意义的拓扑切换过渡条件，两种方法都能有效处理可变拓扑机构的动力学问题。通过平面3R机构和6自由度工业机械臂的关节锁定案例验证了方法的有效性。

Conclusion: 本文为解决可变拓扑机构动力学问题提供了有效的过渡条件方法，两种方法各有特点，能够处理包括关节锁定在内的拓扑变化情况，对于人机交互等应用具有重要意义。
...
TL;DR: 本文提出了一种针对未知负载的力感知导纳控制框架，用于UR5e机器人的拾放操作，通过力/力矩测量来补偿负载质心偏移对柔顺控制的影响。


<details>
  <summary>Details</summary>
Motivation: 未知负载（特别是质心与工具中心点不对齐时）会在机器人手腕产生偏移力矩，这会被柔顺控制器误判为外部交互力，导致非预期的柔顺运动、更大的跟踪误差和降低运输精度。

Method: 提出力感知导纳控制框架：1）使用三轴平移激励项减少运输过程中的负载诱导力效应；2）抓取后先估计负载质量，然后通过平移运动中的手腕力/力矩测量估计负载质心相对于TCP的偏移，用于补偿。

Result: 实验结果表明，相比未校正的放置方法，该方法在保持柔顺运动的同时，显著改善了运输和放置性能。

Conclusion: 该力感知导纳控制框架能有效处理未知负载的质心偏移问题，提高拾放操作的精度和可靠性，同时保持系统的柔顺性。
...
TL;DR: 该研究探索视觉语言模型（VLMs）能否为形态不同于人类的机器人有效推断功能可能性，发现VLMs在非人形机器人上表现出有前景的泛化能力，但在不同物体领域表现不一致，且呈现低假阳性率但高假阴性率的保守预测模式。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在理解人-物交互方面表现出色，但将其应用于具有非人形形态的机器人系统仍未被充分探索。本研究旨在解决这一关键空白，探讨VLMs能否为形态与人类根本不同的机器人有效推断功能可能性。

Method: 引入一种新颖的混合数据集，结合了标注的真实世界机器人功能可能性-物体关系与VLM生成的合成场景。对VLM在多个物体类别和机器人形态上的性能进行实证分析。

Result: 实验表明，VLMs在非人形机器人形式上显示出有前景的泛化能力，但在不同物体领域的表现显著不一致。在所有形态和物体类别中，都观察到低假阳性率但高假阴性率的一致模式，表明VLMs倾向于保守的功能可能性预测。这种模式在新型工具使用场景和非传统物体操作中尤为明显。

Conclusion: 将VLMs有效集成到机器人系统中需要补充方法来缓解过度保守的行为，同时保持低假阳性率带来的固有安全优势。VLMs在非人形机器人功能可能性推断方面显示出潜力，但需要解决其保守预测倾向才能实现更广泛的应用。
...
TL;DR: LiveVLN是一个无需训练即可实现更连续导航的框架，通过多步动作延续来减少等待时间，提升实际部署中的流畅性


<details>
  <summary>Details</summary>
Motivation: 现有导航系统虽然基准测试表现良好，但在实际部署中经常出现走走停停的现象。这是因为感知-推理-执行的循环仍然是阻塞式的：每次新观测后，控制器必须等待感知、传输和推理完成后才能继续运动。仅仅降低动作生成成本并不能消除冗余等待。

Method: LiveVLN通过为预训练的VLM导航器添加多步动作延续功能。它不等待每个完整的感知-推理循环，而是将执行与新到达观测的处理重叠，在当前可执行前缀耗尽之前传递刷新后的未来动作。这种设计在运动过程中保持动作的持续可用性。

Result: 在R2R和RxR基准测试中，LiveVLN保持了基准性能，同时减少了等待时间并提高了动作可用性。在实际部署中，平均等待时间减少了77.7%，StreamVLN的墙上时间缩短了12.6%，NaVIDA缩短了19.6%，实现了更连贯的执行。

Conclusion: LiveVLN是一个运行时框架，可以与兼容的预训练VLM导航器集成，通过多步动作延续实现更连续的导航，显著减少等待时间，提升实际部署的流畅性和效率。
...
TL;DR: 本文提出了一种基于视觉模型预测控制的自主四旋翼无人机管道近距检测框架，通过扩展状态卡尔曼滤波和地形自适应速度设计，在复杂三维场景中实现可靠检测。


<details>
  <summary>Details</summary>
Motivation: 管道检测对能源运输安全至关重要，但面临距离长、地形复杂、人工检测风险高等挑战。无人机虽提供灵活传感平台，但可靠的自主检测仍然困难。

Method: 提出基于图像视觉伺服模型预测控制（VMPC）的自主四旋翼近距管道检测框架，开发了带有图像特征预测的扩展状态卡尔曼滤波（ESKF-PRE），并结合到VMPC中形成ESKF-PRE-VMPC框架，同时引入地形自适应速度设计。

Result: 在真实世界测试中，无风直线管道检测时，管道方向RMSE降低52.63%，图像横向偏差RMSE降低75.04%；在风扰动和弯曲管道任务中，基准方法失败而本方法成功完成。

Conclusion: 提出的ESKF-PRE-VMPC框架结合地形自适应速度设计，能够有效应对视觉更新率低、测量噪声和环境不确定性等挑战，实现可靠的三维场景自主管道检测。
...
TL;DR: 该研究提出了一个基于手势的视觉学习框架，用于与多模态声学机器人平台进行无接触人机交互，通过视觉语言模型实现手势识别并映射到触觉、音频和悬浮三种模态。


<details>
  <summary>Details</summary>
Motivation: 现有的声学机器人虽然能够提供空中触觉、定向音频和声悬浮功能，但依赖脚本命令且缺乏直观的实时人机交互界面，需要更自然的人机交互方式。

Method: 系统结合ESP32-CAM手势捕捉、PhaseSpace运动追踪、集中式处理，以及基于OpenCLIP的视觉学习模型（VLM）和线性探测，将三种手势分类并映射到触觉、音频和悬浮模态。

Result: 验证准确率从约67%（小数据集）提升至近98%（大数据集）；在两个声学机器人的集成实验中，整体手势到模态切换准确率达到87.8%（90次试验），平均端到端延迟为3.95秒。

Conclusion: 该研究证明了基于视觉语言模型的手势界面用于多模态人机交互的可行性，虽然当前系统存在集中处理、静态手势集和受控环境评估等限制，但为更具表现力、可扩展和可访问的群体机器人界面奠定了基础。
...
TL;DR: RAPIDDS框架通过联合建模个体的空间行为（运动路径）和时间行为（任务完成时间），统一了任务级和运动级适应方法，在多人机协作场景中优化任务调度和机器人运动规划。


<details>
  <summary>Details</summary>
Motivation: 现有的人机协作优化方法存在局限性：任务级方法只关注分配和调度，忽略了近距离场景中的空间干扰；运动级方法专注于避碰，却忽略了更广泛的任务上下文。需要一种统一的方法来同时优化任务调度和运动规划。

Method: RAPIDDS框架通过多周期交互学习个体的空间行为（运动路径）和时间行为（任务完成时间），然后联合适应任务调度并引导机器人运动的扩散模型，以最大化效率并最小化接近距离。

Result: 通过仿真消融研究和7自由度机械臂物理实验验证了双重适应的重要性。用户研究（n=32）显示，相比非自适应系统，在效率、接近距离等客观指标以及流畅性和用户偏好等主观指标上均有显著改进。

Conclusion: RAPIDDS通过统一任务级和运动级适应，能够有效优化人机协作计划，提高团队效率和用户体验，为人机协作的实际部署提供了重要框架。
...
TL;DR: 提出MATCH方法，通过混合位置-力控制策略动态选择每个控制维度使用力或位置控制，在极端定位不确定性下解决脆弱插孔任务，相比纯位置控制策略成功率提高10%，破损减少5倍。


<details>
  <summary>Details</summary>
Motivation: 传统基于姿态的强化学习控制策略在需要力约束的任务（如插入精密连接器）中存在局限性，主要依赖精心调校的低级控制器来避免损坏操作，缺乏对力的显式控制。

Method: 提出混合位置-力控制策略，学习动态选择每个控制维度使用力或位置控制；引入MATCH（Mode-Aware Training for Contact Handling）方法，调整策略动作概率以显式反映混合控制中的模式选择行为。

Result: 在极端定位不确定性的脆弱插孔任务中，MATCH显著优于姿态控制策略：成功率提高10%，破损减少5倍；在1600多次仿真到真实实验中，高噪声环境下成功率是姿态策略的2倍（68% vs 33%），相比可变阻抗策略平均施加力减少30%。

Conclusion: MATCH方法通过混合位置-力控制策略有效解决了精密装配任务中的力约束问题，在保持数据效率的同时显著提高了任务成功率和安全性，为机器人精细操作提供了新解决方案。
...
TL;DR: MWM（Mask World Model）通过预测语义掩码而非像素来学习机器人策略，利用几何信息瓶颈过滤视觉噪声，提升泛化能力和鲁棒性
...
Method: 提出Mask World Model（MWM），利用视频扩散架构预测语义掩码的演化而非像素，通过几何信息瓶颈强制模型捕捉关键物理动力学和接触关系，同时过滤视觉噪声；将掩码动力学主干与基于扩散的策略头无缝集成，实现端到端鲁棒控制

Result: 在LIBERO和RLBench仿真基准测试中显著优于最先进的RGB世界模型；真实世界实验和鲁棒性评估（通过随机标记剪枝）显示MWM具有优越的泛化能力和对纹理信息丢失的鲁棒性

Conclusion: 通过预测语义掩码而非像素，MWM能够更好地捕捉物理本质，过滤视觉噪声，从而产生更可靠、泛化能力更强的机器人控制策略
...
TL;DR: UniT框架通过视觉锚定建立统一物理语言，解决人形机器人数据稀缺问题，实现人类到人形机器人的动作迁移，在策略学习和世界建模中均表现出色。


<details>
  <summary>Details</summary>
Motivation: 人形机器人基础模型的扩展受到机器人数据稀缺的限制，而大量的人类自我中心数据提供了一个可扩展的替代方案，但跨身体形态的差异（运动学不匹配）仍然是根本性挑战。

Method: UniT采用三分支交叉重建机制：动作预测视觉以将运动学锚定到物理结果，视觉重建动作以过滤无关视觉干扰，融合分支将这些纯化模态协同到共享的离散潜在空间中，形成身体形态无关的物理意图表示。

Result: 在策略学习（VLA-UniT）中，通过预测统一标记有效利用多样化人类数据，在人形机器人仿真基准和真实世界部署中实现了最先进的数据效率和鲁棒的分布外泛化，展示了零样本任务迁移能力。在世界建模（WM-UniT）中，通过统一标记作为条件对齐跨身体形态动态，实现了直接的人类到人形机器人动作迁移。

Conclusion: UniT通过诱导高度对齐的跨身体形态表示，为将大量人类知识提炼为通用人形机器人能力提供了可扩展的路径，t-SNE可视化证实了人类和人形机器人特征收敛到共享流形中。
