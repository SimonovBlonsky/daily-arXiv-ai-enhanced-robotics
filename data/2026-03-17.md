...
TL;DR: 论文提出人类主导的自动驾驶卡车编队作为实用中间路径，通过分析事故数据和行业现状，论证人机协同编队能提供冗余安全、适应性判断和可扩展验证框架，为大规模自动驾驶货运部署提供技术和社会桥梁。


<details>
  <summary>Details</summary>
Motivation: 虽然自动驾驶卡车技术快速发展，但完全无人驾驶在多样化气候、基础设施和监管环境下仍面临技术和社交挑战。人类相关因素仍是严重事故的主要原因，同时行业在长尾边缘案例、冬季运营、偏远地区物流和大规模安全验证方面存在持续限制。

Method: 1. 分析北美53起重大卡车事故（2021-2026年）数据，识别事故主导因素；2. 回顾行业最新发展，识别技术限制；3. 提出人类在环（HiL）编队架构作为中间解决方案。

Result: 人类相关因素仍是严重卡车事故的主要贡献者，验证了高级辅助/自动驾驶系统的必要性。人机协同编队架构能提供分层冗余、不确定条件下的适应性判断和可扩展验证框架，跟随车辆的双重使用能力支持从协调编队到独立自主运营的渐进过渡。

Conclusion: 人类主导的自动驾驶卡车编队不是妥协，而是基于技术基础和社会协调的桥梁，为大规模自动驾驶货运部署提供实用中间路径，平衡技术可行性和社会接受度。
...
TL;DR: MRPoS：一种结合空间指向和LLM语音交互的混合现实机器人导航界面，替代传统手势操作，显著提升效率和易用性


<details>
  <summary>Details</summary>
Motivation: 当前混合现实机器人导航界面主要依赖手动"空中点击"手势进行目标放置，这种操作重复性强且对初学者体力要求高，需要更自然、易用的交互方式

Method: 提出MRPoS框架，采用多模态界面结合空间指向和基于大语言模型的语音交互，通过整合两种信息将用户语音意图转化为导航目标，并由混合现实技术可视化

Result: 综合实验表明，与传统手势系统相比，MRPoS显著减少了任务完成时间和工作负荷，提供了更易访问和高效的界面

Conclusion: MRPoS通过自然的多模态交互替代复杂手势，为混合现实机器人导航提供了更直观、高效的解决方案
...
TL;DR: Bi-HIL是一个基于双边控制的多模态分层模仿学习框架，用于解决长时程接触丰富的机器人操作任务，通过整合关键帧记忆和子任务进度率来稳定分层协调。


<details>
  <summary>Details</summary>
Motivation: 长时程接触丰富的机器人操作面临部分可观测性和接触不确定性下不稳定子任务转换的挑战。现有方法通常依赖平面策略，难以处理长时程协调问题。

Method: 提出Bi-HIL框架，通过整合关键帧记忆和子任务级进度率来稳定分层协调。进度率建模活动子任务内的阶段进展，并同时调节高层和低层策略。

Result: 在单臂和双臂真实机器人任务上评估Bi-HIL，相比平面策略和消融变体显示出持续改进。结果证明了显式建模子任务进展与力感知控制相结合的重要性。

Conclusion: Bi-HIL通过显式建模子任务进展和力感知控制，为鲁棒的长时程操作提供了有效解决方案，在分层协调和接触不确定性处理方面表现出优势。
...
TL;DR: STL-SVPIO：一种结合信号时序逻辑与Stein变分路径积分优化的方法，用于解决机器人任务规划中复杂时空约束的长时域连续控制轨迹合成问题，显著提升了鲁棒性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂STL规范的长时域连续控制轨迹合成时面临挑战：基于求解器的方法（如MILP）存在指数级扩展问题，而采样方法（如MPPI）难以处理稀疏的长时域成本。需要一种能够有效处理嵌套STL鲁棒性目标结构的方法。

Method: 提出STL-SVPIO方法，将STL重构为全局信息丰富的可微分奖励塑造机制。利用Stein变分梯度下降和可微分物理引擎，将相互排斥的控制粒子群传输到高鲁棒性区域。将稀疏的逻辑满足问题转化为可处理的变分推断，缓解了标准基于梯度方法的严重局部极小值陷阱。

Result: STL-SVPIO在传统STL任务中显著优于现有方法，在鲁棒性和效率方面都有明显提升。能够解决复杂的长时域任务，包括多智能体协调中的同步和排队问题，而基线方法要么无法找到可行解，要么计算上不可行。在非线性动力学的敏捷机器人运动规划任务（如7自由度操作和猎豹后空翻）中也展示了良好的泛化能力。

Conclusion: STL-SVPIO通过将STL与变分推断相结合，成功解决了复杂STL规范的长时域连续控制轨迹合成问题，为机器人任务规划提供了一种高效、鲁棒且可泛化的解决方案。
...
TL;DR: 论文提出了GroundedPlanBench基准和V2GP框架，用于评估和改进视觉语言模型在机器人操作中的空间可执行长时程规划能力。


<details>
  <summary>Details</summary>
Motivation: 当前机器人操作研究虽然利用视觉语言模型进行高层推理和任务分解，但现有基准无法评估生成的行动计划是否在空间上可执行，特别是缺乏对具体交互位置的指定，这限制了真实世界操作能力的评估。

Method: 1) 定义了空间接地规划新任务；2) 创建了GroundedPlanBench基准，联合评估分层子动作规划和空间动作接地；3) 提出了V2GP框架，利用真实世界机器人视频演示自动生成数据来改进空间接地长时程规划。

Result: 评估显示空间接地长时程规划仍是当前视觉语言模型的主要瓶颈。V2GP框架在基准测试和真实世界机器人操作实验中均能显著提升动作规划和空间接地性能。

Conclusion: 该研究填补了机器人操作中空间可执行规划评估的空白，提出的基准和框架为改进视觉语言模型的空间接地规划能力提供了有效途径，推动了向空间可执行规划的进展。
...
TL;DR: 该论文提出了一种面向无线连接机器人系统的安全保证目标导向语义通信框架，在保证安全性的前提下提升任务有效性。


<details>
  <summary>Details</summary>
Motivation: 现有目标导向语义通信方法主要关注任务有效性优化，但忽视了机器人系统中的安全要求。在现实机器人应用中，安全性应作为最高优先级考虑。

Method: 首先总结典型机器人任务的安全要求和有效性指标，系统分析无线连接机器人系统在感知、通信和控制方面面临的安全与有效性挑战，然后提出安全保证的目标导向感知、通信和控制解决方案。

Result: 通过无人机目标跟踪案例研究验证，提出的GSC解决方案能将安全率和跟踪成功率分别提高2倍和4.5倍以上。

Conclusion: 该研究填补了现有目标导向语义通信方法在安全性方面的空白，为无线连接机器人系统提供了兼顾安全性和任务有效性的通信框架。
...
TL;DR: Dream2Fix框架通过生成对抗性失败回放数据，实现机器人执行错误的自主恢复，将纠正准确率从19.7%提升至81.3%


<details>
  <summary>Details</summary>
Motivation: 当前基础模型在机器人操作中仍难以自主恢复执行错误，现有失败学习范式依赖昂贵且不安全的真实数据收集或存在严重模拟到现实差距的模拟器扰动，现有视觉分析器主要输出粗糙的二元诊断而非可执行的轨迹级纠正

Method: 引入Dream2Fix框架，通过生成世界模型中的动作扰动，直接从成功的真实世界演示中合成逼真的反事实失败回放，创建配对失败-纠正数据而不依赖模拟器；实施结构化验证机制，严格筛选回放的任务有效性、视觉一致性和运动学安全性；使用超过12万配对样本的高保真数据集微调视觉语言模型，联合预测失败类型和精确恢复轨迹

Result: 在真实世界机器人实验中，该方法实现了最先进的纠正准确率，从先前基线的19.7%提升至81.3%，并成功实现了物理部署中的零样本闭环失败恢复

Conclusion: Dream2Fix通过生成对抗性失败数据和结构化验证，有效弥合了失败诊断与可执行恢复之间的差距，显著提升了机器人自主错误恢复能力
...
TL;DR: 提出一种基于织物气动人工肌肉的头部颈部支撑外骨骼服设计，通过物理模型计算重力补偿所需压力，分析运动范围和扭矩输出，验证了软体驱动在头部颈部辅助中的潜力。


<details>
  <summary>Details</summary>
Motivation: 传统刚性外骨骼虽然能实现头部颈部活动，但笨重且结构限制性强，需要更轻便、柔性的驱动方式。针对某些神经系统疾病患者需要头部颈部支撑的需求，开发基于软体驱动的解决方案。

Method: 设计基于织物气动人工肌肉的外骨骼服原型，建立物理模型推导补偿重力负载所需的气压。进行运动范围和工作空间分析，引入颈部压缩力作为舒适度相关指标。比较不同执行器布局配置的扭矩输出，选择最优设计。

Result: 模型显示执行器长度限制导致83%工作空间覆盖，重力补偿限制导致43%工作空间覆盖。模型正确预测了测量数据的趋势，其中执行器环绕颈部不是显著影响因素。测试假人和真人演示证实外骨骼服能提供功能性头部支撑和轨迹跟踪。

Conclusion: 基于人工肌肉的软体驱动外骨骼服设计能够有效提供头部颈部支撑和运动辅助，展示了软体驱动技术在头部颈部活动辅助方面的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有水下干预感知系统依赖光学摄像头，在能见度差或光照不足时性能受限；现有光声融合方法无法实现实时密集3D重建，且缺乏浑浊环境下的验证

Method: 使用MASt3R从光学相机数据实时提取密集对应关系，结合声呐3D重建的几何线索，确保在浑浊条件下的鲁棒性

Result: 在浑浊度值从<0.5到>12 NTU范围内记录的数据实验中，该方法相对于基线方法在浑浊条件下的鲁棒性显著提升

Conclusion: Sonar-MASt3R方法通过光声融合实现了水下浑浊环境中实时密集3D重建，提高了水下干预系统的感知能力
...
TL;DR: 提出基于安全、随时有效推断(SAVI)的机器人策略比较框架，实现样本高效、统计严谨的评估，支持多种评估指标，相比传统方法可减少50-70%的评估负担。


<details>
  <summary>Details</summary>
Motivation: 通用机器人操作策略能力不断提升，但真实世界硬件测试资源受限，需要更信息化的性能度量和可靠高效的评估程序来准确评估模型能力和领域进展。

Method: 基于安全、随时有效推断(SAVI)的测试程序，采用顺序测试方法，允许评估者在积累足够统计证据时提前停止，支持从离散部分任务进展到连续奖励或轨迹平滑度等多种指标。

Result: 在模拟和真实世界评估数据上验证，相比标准批量方法减少70%评估负担，相比最先进的二元结果顺序程序减少50%负担，且不损失统计严谨性。使用细粒度任务进展指标比二元成功指标能更快区分竞争策略。

Conclusion: 该框架为机器人策略比较提供了样本高效、统计严谨的统一方法，支持多种实用评估指标，显著减少评估资源需求，有助于更有效地评估和推进机器人操作领域进展。
...
TL;DR: SAATT框架使用LLM实现社交感知的轮椅自主导航，通过预测他人意图和透明化决策，在社交场景中优于传统导航方法


<details>
  <summary>Details</summary>
Motivation: 电动轮椅虽然减少体力消耗，但需要高认知负荷；现有自主系统缺乏社交意识和决策透明度，导致用户和他人感知安全性和信任度降低

Method: 提出SAATT导航框架，使用大型语言模型（LLM）作为决策器，了解用户意图并预测他人意图，在局部控制器中实现社交情境检测和导航，同时通过文本方式透明展示推理过程

Result: 在三个不同社交水平的模拟环境中，与标准全局规划器、代表性竞争社交导航模型和消融研究相比，SAATT在大多数社交情境中表现优异，在安全、社交合规、效率和舒适度八个指标上等效或仅略差

Conclusion: SAATT展示了社交感知和透明自主导航系统在辅助轮椅用户方面的潜力，能够更好地处理社交情境并提高用户信任度
...
TL;DR: REFINE-DP：通过强化学习微调扩散策略的层次框架，解决人形机器人运动规划与控制脱节问题，提升长时程操作任务成功率


<details>
  <summary>Details</summary>
Motivation: 人形机器人运动操作需要高层运动规划与低层全身控制的协调，但现有扩散策略在离线训练后与低层控制器脱节，导致指令跟踪差、分布偏移累积和任务失败。为高维人形系统大规模收集演示数据成本过高。

Method: 提出REFINE-DP层次框架：联合优化扩散策略高层规划器和基于强化学习的低层运动操作控制器。通过PPO-based扩散策略梯度微调扩散策略提升任务成功率，同时更新控制器以准确跟踪规划器演化的指令分布，减少分布不匹配。

Result: 在仿真中人形机器人执行门穿越和长时程物体运输等任务，REFINE-DP达到超过90%的成功率，即使在预训练数据未见过的分布外情况下也能成功。在真实世界动态环境中实现平滑自主任务执行。

Conclusion: REFINE-DP显著优于预训练扩散策略基线，证明强化学习微调是实现可靠人形机器人运动操作的关键。该方法解决了规划与控制脱节问题，提升了系统在复杂动态环境中的鲁棒性。
...
TL;DR: 本文提出了一种用于高速赛车横向控制的线性参数变化模型预测控制器（LPV-MPC），在IAC AV-24赛车上实现了超过160 mph（71.5 m/s）的稳定控制性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶赛车领域面临控制器选择的挑战，需要在车载系统计算能力有限、赛道时间有限和高成本等约束条件下，选择既能满足实时性要求又能保证高性能的控制器。

Method: 采用线性参数变化模型预测控制器（LPV-MPC）进行横向控制，详细描述了控制器设计、模型参数提取方法以及系统级和实现方面的关键考虑因素。

Result: 在IAC AV-24赛车上实现了超过160 mph（71.5 m/s）的稳定控制性能，并在最终比赛运行中获得了验证，提供了车辆动力学和控制器性能的全面分析。

Conclusion: LPV-MPC控制器在高速自动驾驶赛车中表现出色，能够满足实时计算要求和性能约束，为类似应用提供了有效的解决方案。
...
Method: 采用隐式最大似然估计（IMLE）作为生成建模方法进行规划，该方法提供强模态覆盖同时实现快速推理

Result: IMLE在标准离线强化学习基准测试中达到与扩散模型相当的性能，在开环和闭环设置中显著提高规划速度，并在实时人类导航场景中验证了其适应性

Conclusion: IMLE是轨迹规划中扩散模型的有效替代方案，特别适合需要快速推理的实时MPC应用，在保持性能的同时大幅提升速度
...
TL;DR: 多机器人在未知环境下通过协调观察推断上下文，然后基于推断的上下文优先级执行任务规划的两阶段方法


<details>
  <summary>Details</summary>
Motivation: 现实世界中机器人操作时，目标优先级取决于操作上下文。当上下文未知时，机器人需要协调收集信息来推断上下文，以避免基于错误上下文导致的不对齐和不安全行为

Method: 提出MR-CUSSP问题形式化，采用两阶段解决方案：1) CIMOP计算引导机器人到信息地标以高效推断上下文的规划；2) LCBS用于基于上下文诱导的词典序目标偏好的无碰撞多机器人路径规划

Result: 在三个模拟领域评估算法，并在五个移动机器人的salp领域设置中展示实际应用性

Conclusion: 该研究提出了一种有效的多机器人上下文推断和基于优先级任务规划的方法，能够处理未知上下文环境下的协调决策问题
...
TL;DR: ExBot框架通过引入瓶颈状态和双重动作原语，解决了可变形物体操作中的分布外状态问题，实现了数据效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在机器人操作中表现出色，但在分布外状态下会失败。可变形物体操作具有近乎无限的配置可能性，使得全面数据收集不可行。现有方法需要大量数据或高精度感知，这在可变形物体操作中不切实际，特别是存在自遮挡等复杂情况。

Method: 提出ExBot框架，包含两个关键创新：1) 引入瓶颈状态作为任务执行的标准化起点，将分布外问题转化为将不同初始状态转移到瓶颈状态的问题；2) 基于可识别性对分布外状态空间进行划分，使用双重动作原语，即使对于不可识别状态也能操作而无需精确感知。

Result: 在绳索和布料操作的真实世界实验中，ExBot能够从各种分布外状态（包括严重的自遮挡情况）成功完成任务。

Conclusion: ExBot通过围绕瓶颈状态集中演示和利用探索改变感知条件，实现了数据效率和鲁棒性，成功解决了可变形物体操作中的分布外挑战。
...
TL;DR: KoopmanFlow是一种参数高效的生成控制策略，通过Koopman结构归纳偏置将全局稳定运动与高频局部修正解耦，在实时控制中实现更好的接触丰富任务性能。


<details>
  <summary>Details</summary>
Motivation: 生成控制策略在机器人操作中前景广阔，但难以同时建模稳定的全局运动和高频局部修正。现有方法使用均匀时间积分方案，在压缩为实时控制单步时平滑了稀疏高频瞬态与低频稳态的耦合动态。

Method: 提出KoopmanFlow，在统一多模态潜在空间中通过Koopman结构归纳偏置在终端阶段分叉生成：宏观分支通过单步一致性训练锚定慢变轨迹，瞬态分支使用流匹配隔离由视觉线索刺激的高频残差，通过非对称一致性目标优化。

Result: 实验表明KoopmanFlow在需要敏捷扰动抑制的接触丰富任务中显著优于现有基线方法，通过用延迟缓冲换取更丰富的结构先验，在实时部署限制内实现了优越的控制保真度和参数效率。

Conclusion: KoopmanFlow通过解耦多尺度动态并避免多阶段误差累积，为生成控制策略提供了更有效的架构，在实时机器人操作中实现了更好的性能表现。
...
TL;DR: 通过监控VLA模型中少数几个"导航头"的注意力信号，无需训练即可实时检测视觉推理幻觉导致的路径偏差，检测到偏差后触发轻量RL策略进行最短路径回滚
...
Method: 发现冻结VLA模型中少数几个"导航头"能准确检测路径偏差，提出无需训练的异常检测框架，实时监控这些头的信号。检测到偏差后触发轻量RL策略执行最短路径回滚

Result: 仅使用3个注意力头就能达到44.6%的偏差检测率，假阳性率仅11.7%。将整个检测到恢复的流程集成到物理机器人上，展示了实际鲁棒性

Conclusion: 通过监控VLA模型内部的导航头，无需额外训练即可有效检测视觉推理幻觉，结合轻量RL策略实现实时偏差纠正，为机器人导航提供实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 布料解缠任务需要适应变化的接触和张力条件，但大规模真实世界训练不现实（布料损坏和硬件磨损）。模拟到现实迁移是可行方案，但布料操作对交互动力学高度敏感，依赖精确力值的策略常因现实差距而失败。研究发现解缠主要特征是定性张力转变而非精确力值，直接最小化原始力测量的模拟现实差距不一定符合任务结构。

Method: 提出自适应力差量化方法，通过将力输入表示为离散化的时间差异，并自适应学习状态依赖的量化阈值来降低观测分辨率。这种表示减轻了对环境特定力特征的过拟合，促进了直接的模拟到现实迁移。

Result: 在模拟和真实世界布料解缠实验中，ADQ方法比使用原始力输入的策略实现了更高的成功率和更强的模拟到现实迁移鲁棒性。

Conclusion: 强调粗糙力变化模式同时抑制精细环境依赖变化可以改善模拟到现实迁移的鲁棒性。自适应力差量化方法通过离散化力变化表示有效解决了布料操作中的现实差距问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有层次化VLA框架通常使用2D表示连接高级推理与低级控制，但缺乏深度感知和时间一致性，限制了在复杂3D场景中的鲁棒性。需要一种能够统一处理语义、几何和长期动作动态的表示方法。

Method: 提出ST-VLA框架，使用统一3D-4D表示桥接感知与动作。将2D指导转换为3D轨迹，生成平滑空间掩码捕获4D时空上下文。引入ST-Human大规模人类操作数据集（14个任务，30万片段），通过半自动管道标注2D、3D和4D监督。训练ST-VLM时空视觉语言模型，生成空间基础和时序一致的3D表示来指导策略执行。

Result: 在RLBench和真实世界操作任务上的实验表明，ST-VLA显著优于现有最先进基线，零样本成功率分别提升44.6%和30.3%。平滑空间掩码聚焦任务相关几何并稳定潜在表示，支持在线重规划和长期推理。

Conclusion: 将时空推理卸载到具有统一3D-4D表示的VLM中，能够显著提高开放世界机器人操作的鲁棒性和泛化能力。统一的时空表示为语义推理和连续控制提供了稳定接口。
...
TL;DR: 提出基于显式世界模型的开放世界物体操纵框架，通过构建环境的物理基础数字孪生实现零样本泛化，无需特定任务的动作演示。


<details>
  <summary>Details</summary>
Motivation: 开放世界物体操纵是机器人学的基本挑战。现有的视觉-语言-动作模型严重依赖大规模机器人动作演示数据，这些数据收集成本高且可能阻碍分布外泛化能力。

Method: 提出基于显式世界模型的框架，通过开放集感知、数字孪生重建、交互策略采样与评估来构建环境的物理基础数字孪生。在物理模拟器中高效探索和评估操纵策略，然后将选定策略可靠部署到现实世界。

Result: 该框架能够在没有任何任务特定动作演示的情况下执行多个开放集操纵任务，在任务和物体层面都证明了强大的零样本泛化能力。

Conclusion: 通过构建物理基础的数字孪生环境，提出的框架实现了开放世界操纵的零样本泛化，避免了传统方法对大规模动作演示数据的依赖。
...
TL;DR: 提出GraspADMM框架，通过ADMM方法解耦目标接触点与实际接触位置，在保持抓取多样性的同时提升运动学可行性和动态稳定性


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：Dexonomy框架依赖固定接触点限制了手部可达性且无法优化动态稳定性；纯梯度优化器能最大化动态稳定性但使用简化接触近似导致物理穿透。需要同时解决多样性、运动学可行性和动态稳定性问题

Method: 使用交替方向乘子法（ADMM）将精炼阶段公式化，解耦物体上的目标接触点与手上的实际接触位置。该方法交替更新目标物体点以直接最大化动态抓取指标，同时调整手部姿势以物理到达这些目标点并严格尊重碰撞边界

Result: GraspADMM显著优于现有基准方法，在类型无关合成中实现近15%绝对抓取成功率提升，在类型感知合成中实现约100%相对改进。即使在极端低摩擦条件下也能保持稳健、物理合理的抓取生成

Conclusion: GraspADMM框架成功解决了同时保持抓取多样性、运动学可行性和动态稳定性的挑战，通过ADMM分解实现了目标接触点优化与物理可达性的有效平衡
...
TL;DR: ImagiNav：一种新的模块化视觉语言导航范式，通过解耦视觉规划和机器人执行，利用野外导航视频实现零样本机器人导航


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言导航依赖昂贵的机器人特定数据训练端到端策略，而基于模拟数据的基础模型受限于场景多样性和视觉保真度，难以实现规模化泛化

Method: 提出模块化分层框架：1）视觉语言模型将指令分解为文本子目标；2）微调生成视频模型想象实现子目标的未来视频轨迹；3）逆动力学模型从想象视频中提取轨迹，由底层控制器跟踪执行。同时开发了通过逆动力学和预训练视觉语言模型自动标注的野外导航视频数据管道

Result: ImagiNav展示了强大的零样本迁移能力，无需机器人演示即可实现机器人导航，为直接从无标注的开放世界数据学习导航的通用机器人铺平道路

Conclusion: 通过解耦视觉规划和机器人执行，ImagiNav能够直接利用多样化的野外导航视频，克服了传统方法对昂贵机器人数据和模拟数据局限性的依赖，为实现通用机器人导航提供了新范式
...
TL;DR: PaIR-Drive提出了一种并行的模仿学习与强化学习协作框架，用于端到端自动驾驶，避免了传统顺序微调带来的策略漂移问题，在NAVSIM基准上取得了91.2 PDMS和87.9 EPDMS的竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 传统端到端自动驾驶基于模仿学习，性能受限于人类演示质量。现有的强化学习顺序微调方法存在策略漂移问题，且性能受限于预训练的模仿学习策略。需要一种能够实现模仿学习和强化学习完全协作优化的新框架。

Method: 提出PaIR-Drive并行框架：1）将模仿学习和强化学习分离为两个并行分支，避免训练目标冲突；2）推理时强化学习利用模仿学习策略进一步优化最终规划；3）在强化学习分支引入树状轨迹神经采样器和分组相对策略优化（GRPO）增强探索能力。
...
Conclusion: PaIR-Drive通过并行协作框架解决了传统顺序微调的问题，实现了模仿学习和强化学习的协同优化，在自动驾驶任务中展现出超越人类演示的性能，并能有效探索和生成高质量轨迹。
...
### [25] [TransDex: Pre-training Visuo-Tactile Policy with Point Cloud Reconstruction for Dexterous Manipulation of Transparent Objects](https://arxiv.org/abs/2603.13869)
...
Conclusion: TransDex通过点云重建预训练和分层注意力融合，有效解决了透明物体灵巧操作中的感知挑战，展示了在实际机器人系统中的优越性能。
...
### [26] [Path-conditioned Reinforcement Learning-based Local Planning for Long-Range Navigation](https://arxiv.org/abs/2603.13888)
*Mateo Haro,Julia Richter,Fan Yang,Cesar Cadena,Marco Hutter*

Main category: cs.RO

TL;DR: 提出一种基于强化学习的局部导航策略，利用路径信息作为上下文指导，无需显式路径跟随奖励，在高质量路径可用时提升导航效率，在路径信息退化时保持基准性能


<details>
  <summary>Details</summary>
Motivation: 传统分层导航管道对全局路径质量敏感，不准确的遥感数据可能导致局部不可行的航点，同时局部规划器有限的全局上下文限制了长距离导航效率

Method: 提出基于强化学习的局部导航策略，以参考路径观测为条件，主要基于目标到达目标进行训练，没有显式的路径跟随奖励，通过隐式条件学习机会性地利用路径信息

Result: 实验结果显示，当高质量路径可用时显著提高导航效率，当路径观测严重退化甚至不存在时仍保持基准水平性能
...
### [27] [LineMaster Pro: A Low-Cost Intelligent Line Following Robot with PID Control and Ultrasonic Obstacle Avoidance for Educational Robotics](https://arxiv.org/abs/2603.13907)
*Jeni Shahi,Abhishek Shah,A. S. M. Ahsanul Sarkar Akib*

Main category: cs.RO

TL;DR: LineMaster Pro是一款基于Arduino Nano的低成本循线机器人，集成了红外传感器、超声波避障、PID控制和有限状态机，成本仅28.5美元，比商业方案便宜94%，适合资源受限的教育环境。


<details>
  <summary>Details</summary>
Motivation: 商业循线机器人价格昂贵（150-300美元）且缺乏集成障碍物检测功能，限制了在真实场景中的应用和资源受限环境中的普及。

Method: 采用Arduino Nano平台，集成双TCRT5000红外传感器进行精确循线，HC-SR04超声波传感器实时障碍物检测，Ziegler-Nichols优化的数字PID控制器，以及分层有限状态机实现鲁棒避障。包含四阶段传感器校准方法确保在不同光照和表面条件下的可靠运行。

Result: 200次控制试验和72小时连续运行验证：平均循线精度1.18厘米（速度0.4米/秒），障碍物检测可靠性96.7%（10-40厘米范围），误报率0.7%，路径偏差恢复成功率94%。PID控制比传统开关控制提升43%（p<0.001）。总硬件成本28.5美元，比商业方案降低94%。

Conclusion: LineMaster Pro以极低成本实现了高性能循线机器人，集成了障碍物检测功能，为资源受限环境中的机器人教育建立了实用基准，显著降低了教育成本门槛。
...
### [28] [Data-Driven Autoregressive Power Prediction for GTernal Robots in the Robotarium](https://arxiv.org/abs/2603.13908)
*Yassin Abdelmeguid,Ammar Hasan*

Main category: cs.RO

TL;DR: 本文提出了一种轻量级自回归预测器，用于GTernal移动机器人平台的能耗预测，通过分析48,000个样本发现能耗具有强时间自相关性，7,041参数的MLP模型在未见运动模式上达到R²=0.90，并在七个机器人的碰撞避免场景中实现零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统的能耗感知算法需要准确的功耗模型，但现有方法依赖运动学近似，无法捕捉真实硬件的复杂动态特性。需要开发更准确的能耗预测模型来支持多机器人算法开发。

Method: 提出轻量级自回归预测器，基于GTernal移动机器人平台在Georgia Tech Robotarium中收集的48,000个样本数据。使用多层感知机（MLP）模型，参数为7,041个，通过条件化近期功耗历史来预测能耗。发现功耗具有强时间自相关性（ρ₁=0.95），这主导了运动学效应。

Result: 模型在未见运动模式上达到R²=0.90，接近测量噪声限制的理论预测上限。在七个机器人的物理验证碰撞避免场景中，平均R²=0.87，展示了零样本迁移到未见机器人和行为的能力。预测器单次推理仅需224μs，支持150倍于平台30Hz控制速率的实时部署。

Conclusion: 提出的轻量级自回归预测器能够准确预测机器人能耗，克服了传统运动学近似的局限性。模型具有高效的计算性能和良好的泛化能力，可用于支持能耗感知的多机器人算法开发。作者发布了训练好的模型和数据集供研究社区使用。
...
### [29] [SmoothVLA: Aligning Vision-Language-Action Models with Physical Constraints via Intrinsic Smoothness Optimization](https://arxiv.org/abs/2603.13925)
*Jiashun Li,Xiaoyu Shi,Hong Xie,Mingsheng Shang,Yun Lu*

Main category: cs.RO

TL;DR: SmoothVLA是一个新颖的强化学习微调框架，通过结合任务奖励和轨迹平滑度优化，解决了VLA模型在稳定性和探索性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型的后训练方法面临稳定性和探索性的两难：监督微调受限于演示质量且缺乏泛化能力，而强化学习虽然提升探索性但常产生违反物理约束的抖动轨迹。

Method: 提出SmoothVLA框架，采用物理信息混合奖励函数，将稀疏任务奖励与基于轨迹加速度变化率（jerk）的连续密集奖励相结合，通过Group Relative Policy Optimization优化策略。

Result: 在LIBERO基准测试中，SmoothVLA在平滑度上比标准RL提升13.8%，在多样化任务上的泛化能力显著超越监督微调方法。

Conclusion: SmoothVLA通过内在奖励优化为VLA模型与物理世界约束的对齐提供了可扩展的方法，实现了任务性能和运动平滑度的协同优化。
...
### [30] [ToMPC: Task-oriented Model Predictive Control via ADMM for Safe Robotic Manipulation](https://arxiv.org/abs/2603.13944)
*Xinyu Jia,Wenxin Wang,Jun Yang,Yongping Pan,Haoyong Yu*

Main category: cs.RO

TL;DR: 提出任务导向模型预测控制框架，用于开放工作空间中安全高效的机器人操作，统一了避障运动与机器人-环境交互，并通过ADMM算法实时求解复杂优化问题。


<details>
  <summary>Details</summary>
Motivation: 在开放工作空间中，机器人需要同时处理避障运动和与环境交互的复杂任务，现有方法难以统一处理这些多样化场景，需要一种既能保证安全又能提高操作效率的框架。

Method: 提出任务导向模型预测控制框架，包含任务导向避障功能，利用运动学冗余提高操作效率。采用ADMM算法将复杂优化问题分解为两个子问题，分别用DDP和QP求解。

Result: 在Franka Panda机器人上的仿真和硬件实验验证了框架有效性，能够实时规划运动/力轨迹，最大化操作范围同时避障，并严格遵守安全相关的硬约束。

Conclusion: 该框架成功统一了避障运动与机器人-环境交互，通过任务导向避障和ADMM求解策略，实现了开放工作空间中安全高效的机器人操作。
...
### [31] [Vision-guided Autonomous Dual-arm Extraction Robot for Bell Pepper Harvesting](https://arxiv.org/abs/2603.13987)
*Kshitij Madhav Bhat,Tom Gao,Abhishek Mathur,Rohit Satishkumar,Francisco Yandun,Dominik Bauer,Nancy Pollard*

Main category: cs.RO

TL;DR: VADER是一个用于户外甜椒自主采摘的双臂移动机器人系统，结合了鲁棒的感知管道和双臂协调规划，在真实环境中实现了超过60%的采摘成功率和低于100秒的单果采摘周期。


<details>
  <summary>Details</summary>
Motivation: 农业机器人是解决劳动力短缺和采摘成本上升的关键方案。甜椒采摘是劳动密集型任务，占总生产成本的50%。虽然自动化解决方案在受控温室环境中显示出潜力，但由于环境多变性和遮挡问题，户外非结构化农场的采摘仍然是一个开放挑战。

Method: 提出了VADER系统，这是一个专门为户外环境设计的双臂移动操作系统。系统集成了鲁棒的感知管道和双臂规划框架，协调抓取臂和切割臂进行采摘操作。系统还基于GELLO远程操作框架设计了远程操作故障安全机制。为了支持鲁棒感知，贡献了一个包含3200多张图像的分层结构数据集，涵盖室内和室外场景，配对广角场景图像和特写甜椒图像，实现了从果实检测到高精度姿态估计的从粗到精训练策略。

Result: 在各种真实条件下验证了系统，展示了超过60%的采摘成功率和低于100秒的单果采摘周期。系统还具备基于GELLO框架的远程操作故障安全机制以确保鲁棒性。

Conclusion: VADER系统为户外甜椒自主采摘提供了一个有效的解决方案，通过结合鲁棒感知、双臂协调规划和故障安全机制，在非结构化环境中实现了有前景的性能。公开的代码和数据集将促进该领域的进一步发展。
...
### [32] [URDF-Anything+: Autoregressive Articulated 3D Models Generation for Physical Simulation](https://arxiv.org/abs/2603.14010)
*Zhuangzhe Wu,Yue Xin,Chengkai Hou,Minghao Chen,Yaoxu Lyu,Jieyu Zhang,Shanghang Zhang*

Main category: cs.RO

TL;DR: 本文提出了一种端到端自回归框架，能够从视觉观察直接生成可执行的铰接物体模型，支持从仿真到真实机器人的策略迁移。


<details>
  <summary>Details</summary>
Motivation: 铰接物体在机器人学、物理仿真和虚拟环境中至关重要，但从视觉输入重建它们仍然具有挑战性，需要同时推断部件几何和运动学结构。

Method: 提出端到端自回归框架，给定图像和物体级3D线索，顺序生成部件几何及其关联的关节参数，产生完整的URDF模型，无需多阶段流水线。

Result: 在大规模铰接物体基准测试和真实世界机器人任务中，该方法在几何重建质量、关节参数准确性和物理可执行性方面优于先前方法。

Conclusion: 该方法实现了"真实-跟随-仿真"新范式，通过视觉观察构建的高保真数字孪生体，使在纯仿真中训练和测试的策略能够直接迁移到真实机器人而无需在线适应。
...
### [33] [Amortizing Trajectory Diffusion with Keyed Drift Fields](https://arxiv.org/abs/2603.14056)
*Gokul Puthumanaillam,Melkior Ornik*

Main category: cs.RO

TL;DR: 提出KDP方法，通过一步推理实现类似扩散模型的轨迹规划效果，大幅降低推理时间成本


<details>
  <summary>Details</summary>
Motivation: 扩散模型轨迹规划器能生成丰富的多模态动作序列，但迭代去噪过程导致推理时间成本高，在严格计算预算下闭环规划速度慢

Method: 引入关键漂移策略(KDP)，使用漂移场目标训练一步轨迹生成器，通过条件感知的邻域概念，在关键空间计算距离，在完整轨迹空间应用更新

Result: 在标准RL基准测试和实时硬件部署中，KDP以一步推理实现强性能，规划延迟显著低于扩散采样

Conclusion: KDP通过一步推理实现扩散式轨迹规划行为，同时保持采样多样化候选计划的能力，适合实时控制应用
...
### [34] [Stiffness Copilot: An Impedance Policy for Contact-Rich Teleoperation](https://arxiv.org/abs/2603.14068)
*Yeping Wang,Zhengtong Xu,Pornthep Preechayasomboon,Ben Abbatematteo,Amirhossein H. Memar,Nick Colonnese,Sonny Chan*

Main category: cs.RO

TL;DR: Stiffness Copilot：基于视觉的共享控制遥操作系统，操作者控制机器人位姿，AI策略在线调整机器人阻抗，实现安全与效率的平衡


<details>
  <summary>Details</summary>
Motivation: 在接触丰富的遥操作任务中，机器人阻抗选择很困难：需要柔顺以避免损坏环境，又需要刚硬以保持响应性和施加必要力。传统方法难以在线实时调整阻抗参数。

Method: 1. 在仿真中使用特权接触信息推断方向依赖的刚度矩阵；2. 用这些矩阵监督轻量级视觉策略，从腕部摄像头图像预测机器人刚度；3. 策略在运行时零样本迁移到真实图像

Result: 在人体实验中，Stiffness Copilot实现了与恒定低刚度相当的安全性，同时达到了恒定高刚度的效率水平

Conclusion: Stiffness Copilot通过视觉策略在线调整机器人阻抗，成功解决了接触丰富遥操作任务中安全性与效率的平衡问题，实现了零样本迁移到真实环境
...
### [35] [GelSphere: An Omnidirectional Rolling Vision-Based Tactile Sensor for Online 3D Reconstruction and Normal Force Estimation](https://arxiv.org/abs/2603.14104)
*Seoyeon Lee,Mohammad Amin Mirzaee,Wenzhen Yuan*

Main category: cs.RO

TL;DR: GelSphere是一种球形视觉触觉传感器，可实现实时连续表面扫描，通过内部钢球轴承层实现全向滚动，支持在线大表面重建


<details>
  <summary>Details</summary>
Motivation: 传统视觉触觉传感器只能局部感知且在表面滑动时会损坏，圆柱形触觉传感器只能沿固定方向滚动，需要一种能够全向滚动进行连续表面扫描的传感器

Method: 设计球形视觉触觉传感器，内部使用钢球在凝胶和刚性外壳之间形成轴承层，实现全轴滚动运动，通过Wi-Fi流式传输触觉图像，具备在线大表面重建能力

Result: 传感器在多向滚动下保持几何保真度和高重建精度，能够实现不间断表面扫描，重建准确性和图像融合性能均表现良好

Conclusion: GelSphere传感器成功实现了全向滚动连续表面扫描，克服了传统触觉传感器的局限性，为实时大表面重建提供了有效解决方案
...
### [36] [H-RINS: Hierarchical Tightly-coupled Radar-Inertial Navigation via Smoothing and Mapping](https://arxiv.org/abs/2603.14109)
*Ali Alridha Abdulkarim,Mikhail Litvinov,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: 提出分层雷达-惯性因子图框架，通过重置图处理高频里程计，持久图维护完整状态信息，实现长期偏差可观测性和漂移抑制。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达在视觉退化环境中提供鲁棒感知，但雷达-惯性状态估计容易漂移。雷达仅提供稀疏的体坐标系速度测量，对绝对方向约束较弱，导致IMU偏差在滑动窗口滤波器典型的时间范围内可观测性差。

Method: 提出紧耦合分层雷达-惯性因子图框架，将估计问题解耦为高频重置图和持久全局图。重置图融合IMU预积分、雷达速度和自适应零速度更新；持久图维护完整状态信息，融合惯性数据与关键帧几何建图和闭环检测，使用增量平滑与建图技术。

Result: 系统实现高精度和漂移减少的估计，达到27倍实时执行速度。通过持久图向重置图持续注入完全可观测的优化偏差及其精确协方差，有效锚定高频估计器对抗积分漂移。

Conclusion: 提出的分层框架解决了雷达-惯性状态估计中的基本可观测性挑战，通过紧耦合机制实现长期偏差可观测性，在保持实时性能的同时显著减少漂移。
...
### [37] [See, Learn, Assist: Safe and Self-Paced Robotic Rehabilitation via Video-Based Learning from Demonstration](https://arxiv.org/abs/2603.14160)
*Ali Alabbas,Camillo Murgia,Joanne Regan,Philip Long*

Main category: cs.RO

TL;DR: 提出一个远程康复治疗框架，通过RGB-D视频让治疗师远程教学，机器人使用6自由度轨迹编码和混合控制架构，支持三种康复模式，并具备安全保护机制。


<details>
  <summary>Details</summary>
Motivation: 解决远程康复治疗中机器人如何准确复现治疗师示范动作、适应不同患者解剖结构差异、确保安全交互的问题。

Method: 使用Cartesian DMPs编码6自由度身体中心轨迹，采用解耦混合控制架构构建空间合规虚拟隧道和基于努力的时域扩展机制，支持三种康复模式，并利用GMR模型实时学习患者肢体特征进行异常检测。

Result: 系统精度达到平均轨迹复现误差3.7cm和关节活动度误差5.5度，动态交互试验证实控制器能有效执行基于努力的进度控制，同时保持严格空间路径跟随。

Conclusion: 该框架成功实现了远程康复治疗的精确、安全和适应性控制，为机器人辅助康复提供了有效的解决方案。
...
### [38] [Towards Equitable Robotic Furnishing Agents for Aging-in-Place: ADL-Grounded Design Exploration](https://arxiv.org/abs/2603.14182)
*Hansoo Lee,Changhee Seo,Subin Park,Sonya S. Kwak*

Main category: cs.RO

TL;DR: 该立场论文主张老年护理机器人应从"便利功能"转向关注公平、正义和责任，通过访谈识别日常活动挑战，提出基于计算机视觉和生成式AI的机器人概念，并通过后续访谈验证用户偏好。


<details>
  <summary>Details</summary>
Motivation: 在居家养老环境中，日常活动中的小困难会累积影响幸福感，导致疲劳、焦虑、自主性降低和安全风险。当前机器人技术过于关注"便利功能"，而忽视了公平、正义和责任等核心伦理问题。

Method: 1. 对4位70-80岁老年人进行基于日常活动的半结构化访谈，识别常见挑战；2. 提出基于计算机视觉、生成式AI和LLM的室内机器人概念，具备自然语言交互、情境感知提醒、安全执行和用户中心透明度功能；3. 进行视频刺激的后续访谈验证设计概念。

Result: 识别出三个主要挑战：寻找/整理物品、服药、物品运输。用户偏好包括：执行前确认、可预测性、可调节速度/自主性、多模态反馈。同时揭示了与公平相关的担忧，如技术可及性和包容性。

Conclusion: 机器人技术应超越便利功能，关注公平、正义和责任。提出了基于访谈需求的机器人概念，并通过用户反馈验证了设计方向。最后提出了在真实家庭中部署公平机器人福祉系统的开放性问题。
...
### [39] [Navigation beyond Wayfinding: Robots Collaborating with Visually Impaired Users for Environmental Interactions](https://arxiv.org/abs/2603.14216)
*Shaojun Cai,Nuwan Janaka,Ashwin Ram,Janidu Shehan,Yingjia Wan,Kotaro Hara,David Hsu*

Main category: cs.RO

TL;DR: 该研究提出了一种协作式人机导航系统，结合机器人导盲犬的精确感知定位能力和用户的物理操作能力，通过"引导模式"和"适应模式"的切换，帮助视障人士完成需要环境交互的复杂导航任务。


<details>
  <summary>Details</summary>
Motivation: 现有机器人引导系统主要关注无障碍路径导航，但忽略了视障人士导航中需要与环境交互的关键环节（如按电梯按钮、开门、拉椅子等）。这些交互需要精确的目标定位和操作，以及用户与机器人之间的动态协调，而现有系统无法满足这些需求。

Method: 开发了一个协作式人机导航系统，采用两种模式交替工作：1) 引导模式：机器人检测并引导用户到达交互目标位置；2) 适应模式：当用户与环境交互时（如开门），机器人调整自身运动以适应用户的动作。系统结合了机器人导盲犬的精确感知定位能力和用户的物理操作能力。

Result: 评估结果显示，该系统比传统白手杖和非自适应引导系统更安全、更流畅、更高效。随着任务对交互目标定位精度的要求提高，性能优势更加明显。系统能够有效支持需要环境交互的复杂导航任务。

Conclusion: 人机协作方法在辅助技术领域具有广阔前景，能够为视障人士提供更通用、更真实的导航支持。通过结合机器人的精确感知能力和用户的物理操作能力，可以解决传统导航系统无法处理的环境交互挑战。
...
### [40] [AeroGen: Agentic Drone Autonomy through Single-Shot Structured Prompting & Drone SDK](https://arxiv.org/abs/2603.14236)
*Kautuk Astu,Yogesh Simmhan*

Main category: cs.RO

TL;DR: AeroGen是一个开环框架，通过结构化护栏提示和AeroDaaS无人机SDK集成，使通用LLM能够从用户提示生成约束感知的无人机控制代码，提高AI生成无人机自主程序的可靠性。
...
Result: 在20个导航任务和5个无人机任务（城市、农场和检查环境）的多样化基准测试中，AeroGen每个任务生成约40行AeroDaaS Python代码，耗时约20秒，在真实世界和模拟环境中都表现出色。结构化提示与明确定义的SDK提高了LLM生成无人机自主程序的鲁棒性、正确性和可部署性。

Conclusion: 通过结构化护栏提示和与明确定义的无人机SDK集成，可以显著提高LLM生成无人机自主程序的可靠性、正确性和可部署性，为安全关键无人机应用提供可行的AI代码生成解决方案。
...
### [41] [Design of a Bio-Inspired Miniature Submarine for Low-Cost Water Quality Monitoring](https://arxiv.org/abs/2603.14244)
*Quang Huy Vu,Quan Le,Manh Duong Phung*

Main category: cs.RO

TL;DR: 本文设计并验证了一种受鱿鱼启发的微型潜水器，用于低成本水质监测，硬件成本约122.5美元，通过喷射推进和泵控浮力实现稳定导航、深度控制和水样采集。


<details>
  <summary>Details</summary>
Motivation: 水质监测对保护水生生态系统和检测环境污染至关重要。当前需要一种低成本、紧凑且稳定的解决方案，适用于教育和小规模环境监测应用。

Method: 设计受鱿鱼喷射推进机制启发的微型潜水器，采用泵驱动水射流进行推进和转向，结合泵控浮力机制实现深度调节和水样采集。系统集成ESP32微控制器、IMU、压力传感器、GPS接收器和LoRa通信模块等低成本商用组件。

Result: 实验验证显示：360度旋转测试中横滚和俯仰偏差分别保持在±2度和±1.5度内；转向实验显示航向阶跃响应上升时间约2秒，稳定时间约5秒；深度控制实验达到2.5米目标深度，稳态误差在±0.1米内；野外实验证明可靠导航和成功水样采集。

Conclusion: 该平台提供了一个紧凑、稳定且经济高效的小规模水生环境监测解决方案，硬件成本约122.5美元，适合教育和环境监测应用。
...
### [42] [Load-Aware Locomotion Control for Humanoid Robots in Industrial Transportation Tasks](https://arxiv.org/abs/2603.14308)
*Lequn Fu,Yijun Zhong,Xiao Li,Yibin Liu,Zhiyuan Xu,Jian Tang,Shiqi Li*

Main category: cs.RO

TL;DR: 提出了一种基于解耦协调架构的负载感知人形机器人运动框架，通过强化学习策略控制下半身运动，结合基于历史的估计器推断负载扰动，实现稳定负载搬运


<details>
  <summary>Details</summary>
Motivation: 工业环境中的人形机器人需要执行负载搬运任务，但在变化载荷和上半身运动下实现稳定运动具有挑战性，主要由于动态耦合和部分可观测性

Method: 采用解耦但协调的定位-操作架构：下半身运动通过强化学习策略在运动学推导的名义配置上产生残差关节动作；使用基于运动学的运动参考和基于历史的状态估计器推断基础线速度和高度，并将负载和操作引起的扰动编码为紧凑的潜在表示

Result: 框架完全在仿真中训练并在全尺寸人形机器人上部署无需微调，实验显示训练速度更快、高度跟踪准确、定位-操作稳定

Conclusion: 提出的负载感知运动框架能够有效处理工业人形机器人的负载搬运任务，在变化载荷和上半身运动下实现稳定鲁棒的运动控制
...
### [43] [OmniClone: Engineering a Robust, All-Rounder Whole-Body Humanoid Teleoperation System](https://arxiv.org/abs/2603.14327)
*Yixuan Li,Le Ma,Yutang Lin,Yushi Du,Mengya Liu,Kaizhe Hu,Jieming Cui,Yixin Zhu,Wei Liang,Baoxiong Jia,Siyuan Huang*

Main category: cs.RO

TL;DR: OmniClone是一个全身人形机器人遥操作系统，在单个消费级GPU上实现高保真多技能控制，通过OmniBench诊断基准评估性能，相比现有系统减少66%关节位置误差并大幅降低计算需求。


<details>
  <summary>Details</summary>
Motivation: 现有全身人形遥操作系统存在三个主要问题：1）使用聚合指标评估性能，掩盖了不同运动模式下的关键故障；2）系统配置紧密耦合且劳动密集；3）缺乏同时具备鲁棒性、多功能性和实用性的系统。需要建立一个既实用又可靠的遥操作系统。

Method: 提出了OmniClone系统，核心包括：1）OmniBench诊断基准，按运动类别和难度分层评估策略；2）优化的训练数据配方；3）系统级改进：主体无关的重定向和鲁棒通信；4）控制源无关的统一策略，支持实时遥操作、生成运动回放和VLA模型。

Result: 系统将平均每关节位置误差（MPJPE）降低了66%以上，同时计算资源需求比同类方法低几个数量级。系统能泛化到身体比例差异巨大的操作者，并在单个消费级GPU上实现高保真多技能控制。

Conclusion: OmniClone通过诊断评估与实用工程相结合，为可扩展的人形机器人遥操作和自主学习提供了可访问的基础。系统具有控制源无关性、高效性和实用性，有望推动人形机器人遥操作的实际应用。
...
### [44] [VIP-Loco: A Visually Guided Infinite Horizon Planning Framework for Legged Locomotion](https://arxiv.org/abs/2603.14345)
*Aditya Shirwatkar,Satyam Gupta,Shishir Kolathaya*

Main category: cs.RO

TL;DR: VIP-Loco框架通过将视觉场景理解与强化学习和规划相结合，为腿式机器人提供感知性运动控制，在复杂环境中实现鲁棒、可解释的运动。


<details>
  <summary>Details</summary>
Motivation: 传统MPC在处理高维感知输入和快速变化地形时存在困难，而模型无关RL虽然能适应视觉挑战场景但缺乏规划能力。需要一种能结合两者优势的方法。

Method: 提出VIP-Loco框架：训练时，内部模型将本体感知状态和深度图像映射为紧凑的动力学特征供RL策略使用；部署时，将学习到的模型用于无限时域MPC公式，结合适应性与结构化规划。

Result: 在模拟环境中验证了VIP-Loco在多种挑战性运动任务上的性能，包括斜坡、楼梯、爬行、倾斜、跳跃间隙和攀爬，覆盖三种机器人形态。通过消融实验和与SOTA方法比较，证明了其统一规划和感知的能力。

Conclusion: VIP-Loco成功地将视觉场景理解与RL和规划相结合，实现了在各种环境中鲁棒且可解释的腿式机器人运动控制，为感知性运动提供了有效解决方案。
...
### [45] [From Scanning Guidelines to Action: A Robotic Ultrasound Agent with LLM-Based Reasoning](https://arxiv.org/abs/2603.14393)
*Yuan Bi,Yiping Zhou,Pei Liu,Feng Li,Zhongliang Jiang,Nassir Navab*

Main category: cs.RO

TL;DR: 提出基于大语言模型（LLM）的机器人超声扫描统一框架，通过LLM智能体解读扫描指南并动态调用软件工具执行扫描，实现自适应、可解释的自主超声扫描系统。


<details>
  <summary>Details</summary>
Motivation: 传统机器人超声扫描通常采用固定的程序和特定任务模型，难以适应新的扫描任务和复杂的工作流程。临床超声扫描高度依赖操作者的经验和情境判断，需要能够处理可变、决策依赖的工作流程的系统。

Method: 提出基于LLM的智能体框架，从扫描手册中检索和推理指南步骤，基于观察和当前扫描状态自适应规划决策。使用基于强化学习（RL）的策略对LLM智能体进行微调，提高推理质量和工具选择/参数化的正确性。

Result: 在10个超声扫描指南上进行语言执行验证，评估推理能力和工具选择/参数化，显示RL微调的优势。在真实机器人扫描中成功应用于胆囊、脊柱和肾脏扫描，系统能够遵循多样化的指南并在统一系统中实现可靠的自主扫描。

Conclusion: 该框架通过LLM智能体实现了自适应、可解释的机器人超声扫描，能够处理可变的工作流程，在不同解剖目标上实现可靠的自主扫描，为临床超声扫描的自动化提供了统一解决方案。
...
### [46] [eNavi: Event-based Imitation Policies for Low-Light Indoor Mobile Robot Navigation](https://arxiv.org/abs/2603.14397)
*Prithvi Jai Ramesh,Kaustav Chanda,Krishna Vinod,Joseph Raj Vishal,Yezhou Yang,Bharatesh Chakravarthi*

Main category: cs.RO

TL;DR: 提出首个真实世界室内跟随数据集eNavi，包含同步的事件流、RGB帧和专家控制动作，并开发了基于行为克隆的RGB-Event融合导航策略，在低光条件下表现优于纯RGB模型
...
Method: 1) 收集真实世界室内跟随数据集，包含同步的事件流、RGB帧和专家控制动作；2) 构建多模态数据预处理流程，对齐事件和RGB观测，从里程计重建地面真实动作；3) 提出基于双MobileNet编码器和transformer融合模块的RGB-Event导航策略，通过行为克隆训练

Result: 在12种训练变体（从单路径模仿到多路径模仿）的系统评估中，融合事件数据的策略，特别是RGB-Event融合模型，在未见过的低光条件下表现出更好的鲁棒性和更低的动作预测误差，而纯RGB模型在此条件下失效

Conclusion: 事件数据显著提升了室内导航策略的鲁棒性，特别是在低光条件下。提出的RGB-Event融合模型优于单一模态方法，证明了事件相机在真实世界机器人导航中的价值。数据集、同步流程和训练模型已开源
...
### [47] [OCRA: Object-Centric Learning with 3D and Tactile Priors for Human-to-Robot Action Transfer](https://arxiv.org/abs/2603.14401)
*Kuanning Wang,Ke Fan,Yuqian Fu,Siyu Lin,Hu Luo,Daniel Seita,Yanwei Fu,Yu-Gang Jiang,Xiangyang Xue*

Main category: cs.RO

TL;DR: OCRA是一个基于视频的人类到机器人动作迁移的对象中心框架，通过人类演示视频直接学习，实现鲁棒的操作。它利用多视角RGB视频、3D基础模型VGGT以及检测分割模型重建对象中心的3D点云，并结合触觉先验，通过扩散策略生成操作动作。


<details>
  <summary>Details</summary>
Motivation: 当前机器人学习需要从人类演示中直接学习操作技能，但现有方法往往难以处理复杂背景和对象交互。对象中心学习能够强调任务相关对象及其交互，过滤无关背景，为机器人教学提供自然且可扩展的方式。

Method: OCRA采用多视角RGB视频输入，使用VGGT 3D基础模型结合先进检测和分割模型重建对象中心的3D点云。为处理视觉难以感知的属性，整合了包含超过100万张触觉图像的大规模数据集。通过多模态模块ResFiLM融合3D和触觉先验，最后使用扩散策略生成操作动作。

Result: 在纯视觉和视觉-触觉任务上的大量实验表明，OCRA显著优于现有基线和消融实验，证明了其从人类演示视频中学习的有效性。

Conclusion: OCRA通过对象中心学习框架，结合3D重建和触觉先验，成功实现了从人类演示视频到机器人操作的直接迁移，为机器人学习提供了鲁棒且可扩展的解决方案。
...
### [48] [Towards Versatile Opti-Acoustic Sensor Fusion and Volumetric Mapping](https://arxiv.org/abs/2603.14457)
*Ivana Collado-Gonzalez,John McConnell,Brendan Englot*

Main category: cs.RO

TL;DR: 本文提出了一种融合立体声纳对和单目相机的体积映射框架，用于水下机器人在不同能见度条件下的安全导航，通过置信度加权点云融合解决声纳高程模糊问题。


<details>
  <summary>Details</summary>
Motivation: 水下机器人在障碍物丰富的环境中需要精确的3D体积映射。基于视觉的感知在浑浊条件下失效，而声纳虽然对光照和浑浊度鲁棒，但分辨率低且存在高程模糊问题。需要一种融合两种传感器的解决方案。

Method: 提出体积映射框架，融合立体声纳对和单目相机：1）重叠声纳视场解决高程模糊，生成完全定义的3D点云；2）识别相机图像中的感兴趣区域，与声纳回波关联；3）结合声纳距离和相机高程线索生成额外3D点；4）为每个3D点分配置信度值；5）使用高斯过程体积映射框架融合置信度加权点，优先考虑最可靠的测量。

Result: 与其他光声和声纳方法进行实验比较，并在码头环境中进行现场测试，证明该方法在清晰和浑浊条件下都能有效捕获复杂几何形状，并为机器人导航保留关键信息。代码已开源。

Conclusion: 该融合框架通过解决声纳高程模糊问题，结合视觉和声纳的优势，实现了在不同能见度条件下的可靠3D体积映射，为水下自主导航提供了有效解决方案。
...
### [49] [Physics-Informed Policy Optimization via Analytic Dynamics Regularization](https://arxiv.org/abs/2603.14469)
*Namai Chandra,Liu Mohan,Zhihao Gu,Lin Wang*

Main category: cs.RO

TL;DR: PIPER：一种新型物理信息强化学习框架，通过可微拉格朗日残差作为正则化项，将物理约束直接集成到神经策略优化中，提高机器人控制的学习效率、稳定性和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的策略学习方法（如演员-评论家方法）在机器人控制中存在高样本复杂性和产生物理不一致动作的问题。尽管模拟器中已有准确的动力学模型，但神经策略仍需要从数据中重新发现复杂物理规律。

Method: 提出PIPER框架，通过分析软物理约束将物理约束直接集成到神经策略优化中。核心方法是在演员目标函数中集成可微拉格朗日残差作为正则化项，该残差从机器人模拟器描述中提取，微妙地偏向于动态一致的解决方案。这种物理集成通过策略优化期间的额外损失项实现，无需修改现有模拟器或核心RL算法。

Result: 大量实验表明，该方法显著提高了学习效率、稳定性和控制精度，为高效且物理一致的机器人控制建立了新范式。

Conclusion: PIPER框架成功地将物理约束集成到强化学习中，通过可微拉格朗日残差正则化实现了更高效、更稳定、更物理一致的机器人控制，为物理信息强化学习开辟了新方向。
...
TL;DR: 该研究为小型自由飞行器在轨道前哨站（如月球门户）附近执行检查任务开发了一个自主性架构框架，将任务需求转化为具体的规划和控制决策。


<details>
  <summary>Details</summary>
Motivation: 小型自由飞行器可为未来轨道前哨站提供重要的舱外活动服务（如检查和维修），但需要在精密空间站和微重力目标附近安全运行，因此需要建立正式的自主性框架来描述这类检查任务所需的自主能力。

Method: 研究将通用任务需求转化为具体的规划和控制自主架构决策，考虑速度限制、指向要求、保持/禁止区域等独特约束，并设计任务回退技术以提供分层安全保障。在SmallSatSim仿真环境中实现了整个自主性框架。

Result: 提出了一个完整的自由飞行器检查自主性架构，包括规划和控制堆栈的典型输入输出、求解时间和推进剂使用等系统需求，为未来小型卫星在关键任务硬件附近自主检查提供了蓝图。

Conclusion: 该自主性架构超越了现有文献，既提供了自主检查任务的现实系统需求，又将这些需求转化为检查规划和控制的自主性设计决策，为未来小型卫星在关键硬件附近自主检查实施提供了参考框架。
...
TL;DR: 该论文提出了一种基于项目的学习方法，通过敏捷的学期项目教授学生机器人技术，结合理论和实践教学，使用ROS系统开发硬件机器人拆卸软件生态系统。


<details>
  <summary>Details</summary>
Motivation: 传统计算机科学教育过于理论化，缺乏实践经验，无法满足行业需求。需要创新教学方法，将理论知识与实践技能相结合，帮助学生为机器人技术等现代技术趋势做好准备。

Method: 采用基于项目的学习方法，在数字技术学位课程中设置学期跨度的敏捷项目。通过"学校"模式（讲座与练习的互动混合）确保基础资格，然后学生团队使用ROS系统开发硬件机器人拆卸软件生态系统，逐步从基础知识过渡到具体用例实践。

Result: 论文提出了一个完整的教学框架，包括具体的项目示例（积木组件自动化拆卸系统）。该方法通过渐进式学习路径，从基础技术教学到团队协作解决实际问题，实现了理论与实践的有效结合。

Conclusion: 基于项目的敏捷学习方法能够有效弥补传统教育的不足，通过结合理论教学和实践操作，帮助学生获得机器人技术领域的实际工作技能，为行业需求做好准备。
...
### [52] [SmallSatSim: A High-Fidelity Simulation and Training Toolkit for Microgravity Robotic Close Proximity Operations](https://arxiv.org/abs/2603.14598)
*David Schwartz,Alexander Hansson,Sabrina Bodmer,David Sternberg,Oliver Jia-Richards,Keenan Albee*

Main category: cs.RO

TL;DR: SmallSatSim是一个基于MuJoCo物理引擎的高保真仿真工具包，用于模拟微重力环境下小型卫星的交会对接和近距离操作，支持GPU加速、可配置故障模型和先进控制技术。


<details>
  <summary>Details</summary>
Motivation: 微重力环境下的交会对接和近距离操作在太空装配制造、轨道碎片清理和小天体探测等领域应用广泛，但存在独特的机器人控制和规划挑战。现有工具难以满足新型敏捷任务场景（如自由漂浮操作、故障下规划、翻滚体高保真动力学估计）的开发和测试需求。

Method: 基于MuJoCo物理引擎构建高保真仿真框架，准确模拟微重力环境下小型卫星的RPO动力学。采用MuJoCo MJX和JAX实现GPU加速管道，支持基于采样和学习的仿真用例。提供可配置故障模型、先进自由飞行器控制技术、可视化、日志记录和GPU并行化功能。

Result: 开发出SmallSatSim开源工具包，具备高保真RPO动力学模拟、GPU加速、故障模型配置、先进控制技术集成等能力。该工具包已成功应用于机器人RPO规划和控制的演示验证。

Conclusion: SmallSatSim为自主、敏捷的机器人小型卫星操作研究提供了强大的仿真测试平台，能够加速新型RPO算法的开发和验证，推动微重力环境下机器人控制技术的发展。
...
### [53] [Latent Dynamics-Aware OOD Monitoring for Trajectory Prediction with Provable Guarantees](https://arxiv.org/abs/2603.14603)
*Tongfei Guo,Lili Su*

Main category: cs.RO

TL;DR: 该论文将轨迹预测中的OOD监测建模为快速变点检测问题，利用HMM建模预测误差演化，扩展累积最大均值差异方法实现无需后变分布知识的检测，在真实驾驶数据集上验证了延迟减少和对重尾误差的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在安全关键CPS中，轨迹预测的准确性对下游规划控制至关重要，但深度学习模型在真实部署中遇到OOD场景时可靠性下降。由于交通条件动态变化和交互模式改变，OOD事件检测具有挑战性，而安全关键应用需要检测延迟和误报率的正式保证。

Method: 将OOD监测建模为快速变点检测问题，利用HMM对ID条件下的预测误差演化进行建模，扩展累积最大均值差异方法，使其能够在无需后变分布知识的情况下实现检测，同时保持延迟和误报率的可证明保证。

Result: 在三个真实世界驾驶数据集上的实验表明，该方法减少了检测延迟，对重尾误差具有鲁棒性，并且能够处理未知的后变条件。

Conclusion: 通过将OOD监测形式化为快速变点检测问题，并利用HMM结构扩展累积最大均值差异方法，为安全关键CPS中的轨迹预测提供了具有正式保证的可靠OOD检测方案。
...
TL;DR: TacFiLM是一种轻量级的多模态融合方法，通过特征线性调制将视觉-触觉信号整合到视觉-语言-动作模型中，提升接触丰富的操作任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型主要依赖视觉感知，但视觉无法捕捉接触丰富操作中的复杂交互动态（如接触力、表面摩擦、柔顺性等）。虽然已有尝试将触觉信号整合到VLA模型中，但通常通过令牌拼接或大规模预训练增加了复杂性，而行为模型的高计算需求需要更轻量级的融合策略。

Method: TacFiLM采用后训练微调方法，使用特征线性调制（FiLM）将预训练的触觉表示条件化到中间视觉特征上，实现视觉-触觉信号的轻量级融合。

Result: 在插入任务上的实验结果显示，该方法在分布内和分布外任务上都取得了成功率的持续提升、直接插入性能改善、完成时间减少和力稳定性提高。

Conclusion: TacFiLM是将触觉信号整合到VLA模型中的有效方法，能够显著改善接触丰富的操作行为，同时保持轻量级的设计优势。
...
TL;DR: CyboRacket：一种用于人形机器人球拍运动的层次化感知-动作框架，整合了机载视觉感知、基于物理的轨迹预测和大规模预训练全身控制，在网球击球任务中实现了实时视觉跟踪和成功击球。


<details>
  <summary>Details</summary>
Motivation: 动态球类交互任务对机器人具有挑战性，需要有限反应时间下的紧密感知-动作耦合。现有机器人球拍运动系统通常依赖外部运动捕捉或需要针对特定任务重新训练的低级控制器，缺乏通用性和自主性。

Method: 提出CyboRacket层次化框架：1）使用机载摄像头跟踪来球；2）基于物理预测未来轨迹；3）将估计的拦截状态转换为末端执行器和基座运动指令；4）通过SONIC在Unitree G1人形机器人上执行全身控制。

Result: 在基于视觉的人形网球击球任务中，实验结果显示能够实现实时视觉跟踪、轨迹预测，并仅使用机载传感成功完成击球动作。

Conclusion: CyboRacket框架成功整合了机载视觉感知、物理预测和预训练全身控制，为人形机器人球拍运动提供了一种通用且自主的解决方案，实现了紧密的感知-动作耦合。
...
TL;DR: PBD-R：一种改进的位置动力学方法，通过动量守恒约束和修改的速度更新实现物理精确的刚体动力学，在保持计算效率的同时显著提升物理准确性


<details>
  <summary>Details</summary>
Motivation: 机器人学需要能够处理从刚体到可变形物体和流体等多种物理交互的仿真器。当前仿真器采用多个子求解器拼接的架构，使物理推理复杂化。基于粒子的仿真器提供统一表示，但流行的位置动力学方法缺乏物理准确性，限制了在机器人领域的应用。

Method: 提出PBD-R（修订版位置动力学），通过引入动量守恒约束和修改的速度更新来强制执行物理精确的刚体动力学。同时创建了一个求解器无关的基准测试，包含解析解来评估物理准确性。

Result: PBD-R在物理准确性方面显著优于传统PBD，与MuJoCo达到竞争性精度，同时计算需求更低。基准测试验证了其物理准确性的改进。

Conclusion: PBD-R结合了基于粒子求解器的优势（统一表示、跨材料交互）和机器人学对物理保真度的要求，为机器人仿真提供了高效且物理准确的解决方案。
...
TL;DR: 提出了一种坐标无关的机器人模型辨识方法，通过黎曼度量诱导的对偶度量对逆动力学残差进行加权，消除坐标选择带来的偏差


<details>
  <summary>Details</summary>
Motivation: 现有最小二乘回归方法在逆动力学中直接测量坐标力空间的残差，因此依赖于坐标图、单位和缩放的选择，存在坐标诱导的偏差
...
Conclusion: 提出的坐标无关辨识方法通过物理意义归一化消除了坐标偏差，保持了凸性，并与物理一致性约束和几何正则化兼容，提高了机器人模型辨识的准确性
...
TL;DR: 本文提出了一种基于对偶四元数重置映射的无人机碰撞建模方法，在SE(3)流形上直接处理刚体碰撞，解决了传统方法中线性与角冲量解耦导致的流形不一致问题。


<details>
  <summary>Details</summary>
Motivation: 无人机在复杂环境中运行时需要精确的碰撞建模来保持稳定性。传统接触模型将线性冲量和角冲量解耦处理，在快速状态转换时存在流形不一致的风险，这影响了碰撞建模的准确性和稳定性。

Method: 1. 提出对偶四元数重置映射方法，直接在SE(3)流形上解析刚体碰撞
2. 基于统一的空间扭曲（将线速度和角速度作为单一对偶实体）进行操作
3. 构建混合恢复控制器，耦合线性动量和角动量以确保碰撞过程中的严格能量耗散

Result: 1. 硬件在环基准测试显示，与优化的基于矩阵的实现相比，执行延迟减少了24%
2. 高保真MuJoCo仿真验证了控制器对复杂接触动力学的鲁棒性
3. 与解耦恢复方法相比，碰撞后均方根误差(RMSE)减少了56.6%，峰值动能减少了41.2%

Conclusion: 该方法在代数上等效于经典牛顿冲量模型，同时在离散状态跳跃期间保持流形一致性。通过耦合线性动量和角动量的混合恢复控制器，实现了碰撞过程中的严格能量耗散，显著提高了无人机在复杂环境中的碰撞建模精度和稳定性。
...
TL;DR: LiDAR-EVS：用于自动驾驶中鲁棒外推视图LiDAR模拟的轻量级框架，解决3DGS在训练轨迹外视图模拟的过拟合问题


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术在自动驾驶模拟中表现出色，但在训练轨迹外的外推视图LiDAR模拟存在挑战。现有方法通常在单次遍历传感器扫描上训练，存在严重过拟合问题，对新的车辆路径泛化能力差。

Method: 提出LiDAR-EVS框架，包含两个关键组件：1）通过多帧LiDAR融合、视图变换、遮挡剔除和强度调整实现伪外推视图点云监督；2）空间约束的dropout正则化，增强对真实驾驶中各种轨迹变化的鲁棒性。

Result: 在三个数据集上的广泛实验表明，LiDAR-EVS在外推视图LiDAR合成方面达到了最先进的性能。

Conclusion: LiDAR-EVS是一个有前景的工具，可用于自动驾驶系统中的数据驱动模拟、闭环评估和合成数据生成，具有即插即用特性，可轻松扩展到不同的LiDAR传感器和神经渲染基线。
...
TL;DR: CORAL框架通过将高级语义推理与低级反应控制解耦，解决了现有VLM驱动水下航行器在珊瑚礁监测中的三个关键限制：频繁推理等待、无法建模机器人动力学、以及小偏差累积成大路径误差。


<details>
  <summary>Details</summary>
Motivation: 珊瑚礁作为关键生态系统物种持续衰退，需要定期水下监测评估健康状况，但人工潜水监测成本高、危险且有限。自主水下航行器（AUVs）是有前景的替代方案，但现有AUVs依赖几何导航无法解释场景语义。虽然视觉语言模型（VLMs）能实现语义推理，但现有端到端VLM驱动系统存在三个关键限制：需要VLM生成每个导航决策导致频繁等待推理、无法建模机器人动力学导致碰撞、以及有限的自校正能力导致小偏差累积成大路径误差。

Method: 提出CORAL框架，将高级语义推理与低级反应控制解耦。VLM通过选择航点提供高级探索指导，基于动力学的规划器处理低级无碰撞执行。几何验证模块验证航点并在需要时触发重新规划。

Result: 与先前最先进方法相比，CORAL将覆盖率提高了14.28个百分点（相对提高17.85%），碰撞减少了100%，并且需要57%更少的VLM调用。

Conclusion: CORAL框架通过解耦语义推理和反应控制，显著提高了自主水下航行器在珊瑚礁监测中的性能，减少了计算开销和碰撞风险，同时提高了覆盖效率。
...
### [61] [Exploring the dynamic properties and motion reproducibility of a small upper-body humanoid robot with 13-DOF pneumatic actuation for data-driven control](https://arxiv.org/abs/2603.14787)
*Hiroshi Atsuta,Hisashi Ishihara,Minoru Asada*

Main category: cs.RO

TL;DR: 开发了一个紧凑型13自由度上身人形机器人，利用多层感知器实现数据驱动控制，相比传统PID控制器在轨迹跟踪方面表现更优


<details>
  <summary>Details</summary>
Motivation: 气动驱动的人形机器人具有高自由度，在物理人机交互方面潜力巨大，但气动执行器的非线性特性使其精确控制具有挑战性

Method: 首先研究系统的关键动态特性（如执行时间延迟），确认系统具有高度可重复性；基于此实现了一个基于多层感知器的数据驱动控制器，具有显式时间延迟补偿，在4自由度手臂子系统上进行训练

Result: 与传统PID控制器相比，数据驱动控制器在轨迹跟踪方面表现出更优越的性能，验证了数据驱动方法在控制复杂高自由度气动机器人方面的潜力

Conclusion: 数据驱动方法能够有效控制具有非线性特性的高自由度气动机器人，为物理人机交互应用提供了有前景的解决方案
...
TL;DR: GraspALL模型通过将连续光照变化编码为量化参考，指导RGB与非RGB模态的自适应特征融合，在动态光照下提升衣物抓取准确性


<details>
  <summary>Details</summary>
Motivation: 服务机器人全天候操作需要准确衣物抓取，但低光照场景会严重降低衣物结构特征，导致抓取鲁棒性显著下降。现有方法利用非RGB模态的光照不变性增强RGB特征，但忽视了不同光照条件下对非RGB特征的依赖变化，可能引入错位的非RGB线索，从而削弱模型利用多模态信息适应光照变化的能力。

Method: 提出GraspALL模型，将连续光照变化编码为量化参考，根据不同的光照强度指导RGB和非RGB模态之间的自适应特征融合，从而生成光照一致的抓取表示。

Result: 在自建的衣物抓取数据集上的实验表明，GraspALL在不同光照条件下比基线方法提高了32-44%的抓取准确率。

Conclusion: 通过将光照变化量化为参考并指导自适应特征融合，GraspALL能够有效生成光照一致的抓取表示，显著提升动态光照条件下的衣物抓取性能。
...
Method: 基于指数积公式构建统一误差模型，将所有参数整合到李代数框架中；推导闭式解析雅可比矩阵；开发基于半定松弛的可证明正确初始化算法

Result: 实验验证了该方法在相同视觉测量条件下优于现有基线方法；可证明初始化算法在坐标标定方面优于多个基线方法

Conclusion: 提出的统一标定框架能够有效减少误差传播，通过李代数公式实现坐标变换和运动学参数的联合优化，提高了双机械臂系统的标定精度
...
TL;DR: 论文提出了Ego-to-World（E2W）基准和CoRL框架，用于解决多智能体系统中从分布式局部视角理解世界的挑战，通过强化学习结合跨视图空间奖励提升视觉语言模型的跨视图融合能力。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体系统中从分布式、局部视角理解世界的基本挑战。每个智能体通过自我中心视角感知环境，这种视角通常受到遮挡和模糊性的限制。需要研究如何融合异构视角来获得全局场景理解。

Method: 提出了CoRL框架，采用两阶段方法：1）思维链监督微调；2）使用Group-Relative Policy Optimization的强化学习。核心组件是跨视图空间奖励（CVSR），通过将推理步骤与视觉证据链接，提供密集的任务对齐反馈，确保连贯的跨视图实体解析，并引导模型做出正确的最终预测。

Result: 在E2W基准上的实验表明，CoRL在推理和感知接地指标上持续超越强大的专有和开源基线。消融实验进一步证实了每个CVSR组件的必要性。此外，CoRL能够泛化到外部空间推理基准，并在现实世界多机器人操作中实现有效的跨视图定位和抓取放置执行。

Conclusion: E2W和CoRL为从分布式自我中心观察中学习世界中心场景理解提供了原则性基础，推动了协作式具身AI的发展。该方法在跨视图融合、空间推理和实际机器人操作方面表现出色。
...
TL;DR: 提出在黎曼流形上通过关节空间路径规划解决机器人手术中角度限制和计算复杂性问题的方法


<details>
  <summary>Details</summary>
Motivation: 机器人微创手术中，机器人围绕固定插入端口运动常遇到角度限制，且腹腔表面非凹面特性使路径规划计算成本高昂

Method: 通过将位置空间转换为黎曼流形，在关节空间进行路径规划，定义边缘成本函数来搜索期望路径并减少关节运动范围

Result: 实验结果表明，相比位置空间计算，该方法能有效减少关节角度运动范围

Conclusion: 提出的黎曼流形关节空间路径规划方法能有效解决机器人手术中的角度限制和计算复杂性问题
...
TL;DR: 提出ViSA方法，通过访问状态增强和一致性嵌入空间学习，改进对比强化学习中的目标空间泛化能力


<details>
  <summary>Details</summary>
Motivation: 传统对比强化学习(CRL)在目标条件强化学习中，由于将访问状态作为伪目标，只能准确估计有限目标的价值函数，存在目标空间泛化能力不足的问题

Method: 提出ViSA方法，包含两个核心组件：1) 生成增强状态样本，在策略探索过程中增强难以访问的状态样本；2) 学习一致性嵌入空间，使用增强状态作为辅助信息，基于互信息重构嵌入空间的目标函数进行正则化

Result: 在仿真和真实机器人任务中评估ViSA，显示其改进了目标空间泛化能力，能够准确估计难以访问目标的价值函数

Conclusion: ViSA通过数据增强方法有效解决了CRL中目标空间泛化受限的问题，提高了对难以访问目标的准确价值估计能力
...
TL;DR: 本文提出了一个基于离散微分几何的统一力学框架，将刚性折叠、弹性变形和稳定性驱动的转变整合到单一变分公式中，为智能折纸机器人提供预测建模和力学引导的驱动平台。


<details>
  <summary>Details</summary>
Motivation: 折纸启发架构为轻量化、可重构和可编程机器人系统提供了强大途径，但目前缺乏一个统一的力学框架来无缝桥接刚性折叠、弹性变形和稳定性驱动的转变。

Method: 引入基于离散微分几何的几何一致建模框架，将面板弹性和折痕旋转嵌入到单一变分公式中。通过将折痕-面板耦合直接嵌入到中边几何离散化中，框架自然捕获刚性折叠极限、分布弯曲、多稳定性和非线性动态跳跃。采用包含重力、接触、摩擦和磁驱动的隐式动态公式。

Result: 该框架能够实现刚性和柔性区域中稳定性和变形的可编程控制，使折纸结构从静态折叠机制转变为主动机器人模块。通过代表性示例（单折分叉、可展开Miura膜、双稳态Waterbomb模块和基于Kresling的爬行机器人）展示了几何驱动力学如何直接影响机器人功能。

Conclusion: 这项工作确立了离散微分几何作为智能折纸机器人的基础设计语言，在统一的计算平台内实现了预测建模、稳定性编程和力学引导的机器人驱动。
...
<details>
  <summary>Details</summary>
Motivation: 基于模仿学习的端到端自动驾驶策略在闭环执行中表现不佳，因为开环训练目标与真实驾驶要求不匹配。虽然强化学习可以通过奖励信号直接优化驾驶目标，但基于渲染的训练环境存在渲染差距且计算成本高。

Method: 1. 基于离线数据集构建向量空间的伪模拟环境，实现高效无渲染的试错训练；2. 引入预测世界模型，根据自车计划生成反应性智能体轨迹，弥合静态数据集与动态闭环环境之间的差距；3. 采用分层解耦规划器，结合模仿学习进行横向路径生成和强化学习进行纵向速度优化。

Result: 在Bench2Drive基准测试中达到最先进性能，比之前的端到端强化学习方法提升了10.29%的驾驶分数，且无需昂贵的在线交互。在DOS基准测试的进一步评估证实了其在处理安全关键遮挡场景中的可靠性。

Conclusion: PerlAD通过伪模拟强化学习方法有效解决了端到端自动驾驶中的闭环执行问题，实现了高效、无渲染的训练，在多个基准测试中表现出优越性能，为自动驾驶策略训练提供了新的有效途径。
...
TL;DR: TakeVLA是一个创新的视觉-语言-动作后训练框架，通过预接管语言监督和场景梦想强化微调，解决了现有VLA自动驾驶系统在分布偏移下的安全边界有限和被动优化问题。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶的视觉-语言-动作范式依赖静态数据集离线训练，容易受到分布偏移影响。现有的后训练方法虽然使用接管数据来缓解这一问题，但存在两个关键限制：1）监督仅限于接管时刻之后，导致策略安全边界有限；2）被动偏好优化缺乏主动探索以获得最优性能。

Method: 提出TakeVLA框架，包含两个互补创新：1）预接管语言监督，让VLA模型能够主动从错误中学习，通过明确教导模型在易出错情况下应该做什么，培养预防性思维，提前预测危险并大幅扩大安全边界；2）场景梦想，一种在重构的接管场景中进行强化微调的范式，鼓励主动探索超越简单的偏好拟合。

Result: 在Bench2Drive基准测试中，TakeVLA实现了最先进的闭环性能，超越了强大的VLA基线SimLingo，驾驶分数提高了4.93，同时安全边界得到增强，平均TTC（碰撞时间）增加了11.76%。

Conclusion: TakeVLA通过预接管语言监督和场景梦想强化微调，有效解决了现有VLA后训练方法的局限性，显著提升了自动驾驶系统的安全性和性能，为端到端自动驾驶提供了更鲁棒的解决方案。
...
TL;DR: ReMAP-DP是一个结合标准化透视重投影与结构感知双流扩散策略的机器人框架，通过融合2D语义特征和3D几何描述符，解决了通用机器人策略在3D空间感知不足的问题，在多种操作任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 基于2D视觉表示的通用机器人策略擅长语义推理，但缺乏执行高精度任务所需的显式3D空间感知。现有的3D集成方法由于稀疏点云的结构不规则性和多视图正交渲染引入的几何失真，难以有效解决这一问题。

Method: 提出ReMAP-DP框架，结合标准化透视重投影与结构感知双流扩散策略。通过将重投影视图与像素对齐的PointMaps耦合，双流架构利用可学习的模态嵌入来融合冻结的语义特征和显式几何描述符，确保精确的隐式补丁级对齐。

Result: 在仿真和真实环境中的广泛实验表明ReMAP-DP在多样化操作任务中表现优异：在RoboTwin 2.0上达到59.3%的平均成功率，比DP3基线提高6.6%；在ManiSkill 3的几何挑战性Stack Cube任务上比DP3提升28%。同时展现出卓越的真实世界鲁棒性，仅需少量演示就能高效执行高精度动态操作。

Conclusion: ReMAP-DP通过有效融合2D语义特征和3D几何信息，成功解决了通用机器人策略在3D空间感知方面的局限性，为高精度机器人操作任务提供了有效的解决方案，在仿真和真实环境中都表现出优异的性能和数据效率。
...
### [71] [CycleRL: Sim-to-Real Deep Reinforcement Learning for Robust Autonomous Bicycle Control](https://arxiv.org/abs/2603.15013)
*Gelu Liu,Teng Wang,Zhijie Wu,Junliang Wu,Songyuan Li,Xiangwei Zhu*

Main category: cs.RO

TL;DR: CycleRL：首个基于深度强化学习的自主自行车控制框架，通过模拟到真实迁移实现鲁棒控制


<details>
  <summary>Details</summary>
Motivation: 传统控制策略难以处理自主自行车的欠驱动非线性动力学，对模型失配敏感且适应现实世界不确定性的能力有限

Method: 使用PPO算法在高保真NVIDIA Isaac Sim环境中训练端到端神经控制策略，采用复合奖励函数平衡姿态、速度和转向控制，并通过系统域随机化实现模拟到真实迁移

Result: 模拟环境中达到99.90%平衡成功率、1.15°转向跟踪误差和0.18 m/s速度跟踪误差，成功实现硬件迁移

Conclusion: 深度强化学习为自主自行车控制提供了有效范式，相比传统方法具有更好的适应性
...
### [72] [AnoleVLA: Lightweight Vision-Language-Action Model with Deep State Space Models for Mobile Manipulation](https://arxiv.org/abs/2603.15046)
...
<details>
  <summary>Details</summary>
Motivation: 语言引导的机器人操作任务对服务机器人至关重要，需要安全、高效和任务级通用性。现有视觉-语言-动作模型（VLAs）虽然性能强大，但标准Transformer架构计算成本高，在资源受限环境中部署困难。

Method: 提出AnoleVLA轻量级VLA模型，采用深度状态空间模型高效处理多模态序列。利用轻量级快速序列状态建模处理视觉和文本输入，使机器人能够高效生成轨迹。

Result: 在仿真和物理实验中评估该方法。在真实世界评估中，AnoleVLA在任务成功率上比代表性大型VLA高出21点，同时推理速度约快3倍。

Conclusion: AnoleVLA通过深度状态空间模型实现了高效的视觉-语言-动作建模，在保持高性能的同时显著降低了计算成本，适合资源受限环境中的机器人部署。
...
### [73] [Multi-Mode Pneumatic Artificial Muscles Driven by Hybrid Positive-Negative Pressure](https://arxiv.org/abs/2603.15066)
...
<details>
  <summary>Details</summary>
Motivation: 仿生肌肉是实现类人机器人运动的关键技术，现有技术需要更灵活、轻便、可编程的解决方案来满足柔性可穿戴设备和紧凑软体机器人系统的需求。

Method: 采用低成本热封片材制造，通过热压和热封工艺制作包含可充气骨架和外皮的IN-FOAM结构，骨架图案可编程，支持正负压混合驱动，并提出多层骨架结构和多通道骨架方法。

Result: 实验、理论和数值研究表明，IN-FOAM的输出力和收缩可通过正负压混合操作模式调节，多层结构可进一步提高收缩比，多通道方法可在单个IN-FOAM中集成多种运动模式。

Conclusion: IN-FOAM在柔性可穿戴设备和紧凑软体机器人系统方面具有巨大应用潜力，为仿生肌肉技术提供了创新解决方案。
...
### [74] [HALO:Closing Sim-to-Real Gap for Heavy-loaded Humanoid Agile Motion Skills via Differentiable Simulation](https://arxiv.org/abs/2603.15084)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中部署的人形机器人经常需要携带未知负载，这会引入显著的模型不匹配，降低仿真到现实强化学习方法的有效性

Method: 基于可微分模拟器MuJoCo XLA的两阶段梯度系统辨识框架：第一阶段校准标称机器人模型以减少内在仿真到现实差异；第二阶段进一步识别未知负载的质量分布

Result: 在大量仿真和真实世界实验中，相比现有基线方法，实现了更精确的参数识别、改进的运动跟踪精度，以及显著增强的敏捷性和鲁棒性

Conclusion: 通过显式减少策略训练前的结构化模型偏差，该方法能够在重载条件下实现强化学习策略到硬件的零样本迁移
...
### [75] [BodyGuards: Escorting by Multiple Robots in Unknown Environment under Limited Communication](https://arxiv.org/abs/2603.15108)
...
<details>
  <summary>Details</summary>
Motivation: 多机器人系统在侦察、灾难响应等高风险任务中部署日益增多，但在通信受限的未知对抗环境中保护人类操作员仍是关键挑战。现有协作探索方法追求完全覆盖，而本研究专注于任务导向探索，以最小化操作员到达目标的导航时间并确保安全

Method: 提出BodyGuards护送框架，包含三个核心组件：(I)操作员动态移动策略，维护带有风险区域的局部地图进行主动路径规划；(II)双模式机器人策略，结合前沿探索与优化返回事件，平衡探索、威胁检测和间歇通信；(III)多机器人协调协议，联合规划探索和信息共享以实现高效护送

Result: 广泛的人机交互仿真和硬件实验表明，该方法在对抗性和受限环境中显著降低了操作员风险和任务时间，性能优于基线方法

Conclusion: BodyGuards框架通过任务导向探索、动态风险管理和多机器人协调，有效解决了对抗性环境中保护人类操作员的挑战，为高风险任务中的多机器人系统提供了实用的护送解决方案
...
### [76] [A Novel Camera-to-Robot Calibration Method for Vision-Based Floor Measurements](https://arxiv.org/abs/2603.15126)
*Jan Andre Rudolph,Dennis Haitz,Markus Ulrich*

Main category: cs.RO

TL;DR: 提出了一种用于地面观测移动机器人的新型手眼标定方法，通过设计参考板结合激光跟踪器3D测量和相机2D成像，实现机器人-相机变换的精确标定。
...
Method: 设计了一个参考板，包含用于激光跟踪器姿态获取的反射器巢和用于机器人安装相机观测的相机标定目标。标定过程包括估计板姿态、板-相机姿态和机器人姿态，然后计算机器人-相机变换。

Result: 实验表明该方法具有亚毫米级的重复精度。

Conclusion: 该方法成功地将激光跟踪器3D测量与相机2D成像相结合，为地面观测移动机器人提供了一种精确的手眼标定解决方案。
...
### [77] [Confusion-Aware In-Context-Learning for Vision-Language Models in Robotic Manipulation](https://arxiv.org/abs/2603.15134)
*Yayun He,Zuheng Kang,Botao Zhao,Zhouyin Wu,Junqing Peng,Jianzong Wang*

Main category: cs.RO

TL;DR: CAICL方法通过混淆感知的上下文学习解决VLM在机器人操作中的捷径学习问题，在VIMA-Bench上达到85.5%成功率


<details>
  <summary>Details</summary>
Motivation: 基于视觉语言模型的机器人操作系统在混淆物体场景中缺乏鲁棒性，主要原因是VLM固有的捷径学习问题限制了其准确区分混淆特征的能力

Method: 提出混淆感知上下文学习(CAICL)：首先进行混淆定位和分析，识别潜在混淆源；然后将这些信息作为提示，引导VLM关注最可能导致误识别的特征

Result: 在VIMA-Bench上的大量实验表明，CAICL有效解决了捷径学习问题，达到85.5%的成功率，在不同泛化程度的任务中表现出良好的稳定性

Conclusion: CAICL方法通过混淆感知的上下文学习显著提升了VLM在机器人操作中的鲁棒性和性能，特别是在混淆物体场景中
...
### [78] [Master Micro Residual Correction with Adaptive Tactile Fusion and Force-Mixed Control for Contact-Rich Manipulation](https://arxiv.org/abs/2603.15152)
*Xingting Li,Yifan Xie,Han Liu,Wei Hou,Guangyu Chen,Shoujie Li,Wenbo Ding*

Main category: cs.RO

TL;DR: M2-ResiPolicy：一种主-微残差控制架构，通过融合高频触觉反馈和视觉规划，解决了机器人精细操作中全局任务一致性与局部反应控制的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前视觉模仿学习方法在长时程规划方面表现良好，但难以感知关键交互线索（如摩擦变化、初始滑动），且难以平衡全局任务一致性与局部反应反馈，导致在接触丰富和精细操作任务中表现不佳。

Method: 提出主-微残差控制架构：1）主引导策略（MGP，10Hz），使用扩散模型生成时间一致的动作块，通过触觉强度驱动的自适应融合机制动态调节视觉与触觉的感知权重；2）微残差校正器（MRC，60Hz），使用轻量级GRU基于TCP力矩反馈提供实时动作补偿；3）与力混合PBIC执行层集成，有效调节接触力确保交互安全。

Result: 在多个要求苛刻的任务（包括易碎物体抓取和精密插入）中，M2-ResiPolicy显著优于标准扩散策略（DP）和最先进的反应扩散策略（RDP），在芯片抓取任务中实现了93%的无损伤成功率，并表现出优越的力调节稳定性。

Conclusion: M2-ResiPolicy通过分层控制架构成功解决了机器人精细操作中的多时间尺度控制挑战，实现了全局任务规划与局部反应控制的协同，为接触丰富和精细操作任务提供了有效的解决方案。
...
### [79] [ForceVLA2: Unleashing Hybrid Force-Position Control with Force Awareness for Contact-Rich Manipulation](https://arxiv.org/abs/2603.15169)
...
TL;DR: ForceVLA2是一个端到端的视觉-语言-动作框架，为机器人提供混合力-位置控制和显式力感知，显著提升接触丰富操作任务的成功率和可靠性


<details>
  <summary>Details</summary>
Motivation: 现有的具身智能主要依赖位置控制，缺乏对交互力的显式感知和调节，限制了在真实世界任务中的稳定性、精度和鲁棒性

Method: 提出ForceVLA2框架：1) 在VLM专家中引入基于力的提示来构建跨阶段的力感知任务概念；2) 在动作专家中采用跨尺度专家混合机制，自适应融合这些概念与实时交互力进行闭环混合力-位置调节；3) 构建包含1000条轨迹的ForceVLA2-Dataset数据集

Result: ForceVLA2在5个接触丰富任务中显著提升成功率和可靠性，相比pi0和pi0.5分别提高48.0%和35.0%，有效缓解了机械臂过载和不稳定接触等常见故障模式

Conclusion: ForceVLA2通过显式力感知和混合力-位置控制，显著推进了VLA中的力感知交互物理智能，为接触丰富的机器人操作提供了更稳定、精确和鲁棒的解决方案
...
### [80] [What Matters for Scalable and Robust Learning in End-to-End Driving Planners?](https://arxiv.org/abs/2603.15185)
*David Holtz,Niklas Hanselmann,Simon Doll,Marius Cordts,Bernt Schiele*

Main category: cs.RO

TL;DR: 本文系统性地重新审视了端到端自动驾驶架构中常见模式对闭环性能的影响，提出了轻量级可扩展的BevAD架构，在Bench2Drive基准上取得了72.7%的成功率。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶架构通常在开环数据集上开发，但那些在开环评估中表现优异的架构改进往往无法转化为可扩展的稳健闭环驾驶学习。本文旨在重新评估常见架构模式对闭环性能的实际影响。

Method: 系统性地研究了三种常见架构模式对闭环性能的影响：(1)高分辨率感知表示，(2)解耦的轨迹表示，(3)生成式规划。基于这些分析结果，提出了BevAD这一轻量级、高度可扩展的端到端驾驶架构。

Result: BevAD在Bench2Drive基准上取得了72.7%的成功率，并展示了使用纯模仿学习的强大数据扩展能力。分析揭示了这些架构模式之间的意外限制和未被充分探索的协同效应。

Conclusion: 通过对常见架构模式的系统性重新评估，发现了开环评估与闭环性能之间的差距，并提出了一种更有效的端到端自动驾驶架构，为未来的架构设计提供了重要见解。
...
### [81] [NavGSim: High-Fidelity Gaussian Splatting Simulator for Large-Scale Navigation](https://arxiv.org/abs/2603.15186)
*Jiahang Liu,Yuanxing Duan,Jiazhao Zhang,Minghan Li,Shaoan Wang,Zhizheng Zhang,He Wang*

Main category: cs.RO

TL;DR: NavGSim是一个基于高斯泼溅的大规模导航环境仿真器，能够生成高保真、大尺度的导航场景，支持VLA模型训练，显著提升导航策略的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 机器人学习中的环境仿真是一个关键挑战，特别是在渲染和物理仿真方面。导航任务中轨迹经常跨越多个房间或整个楼层，需要大规模、高保真的仿真环境。

Method: 基于分层3D高斯泼溅框架构建NavGSim，实现数百平方米大场景的光照真实渲染。引入基于高斯泼溅的切片技术直接从重建的高斯模型中提取可导航区域。提供完整的API支持多GPU开发，包括自定义场景重建、机器人配置、策略训练和评估工具。

Result: 使用NavGSim收集的轨迹训练Vision-Language-Action模型，在仿真和真实环境中评估性能。结果表明NavGSim显著增强了VLA模型的场景理解能力，使策略能够有效处理多样化的导航查询。

Conclusion: NavGSim为机器人导航学习提供了高质量的大规模仿真环境，通过高斯泼溅技术实现了高保真渲染和可导航区域提取，有效支持VLA模型的训练和评估。
...
### [82] [Coupled Particle Filters for Robust Affordance Estimation](https://arxiv.org/abs/2603.15223)
*Patrick Lowin,Vito Mengers,Oliver Brock*

Main category: cs.RO

TL;DR: 提出一种通过耦合两个递归估计器（可抓取区域和可移动区域）来消除机器人感知中视觉、几何和语义歧义的方法，显著提高了感知精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 机器人感知中的视觉、几何和语义歧义使得感知任务具有挑战性，需要一种能够有效消除这些歧义的方法来提高感知精度。

Method: 使用两个耦合的递归估计器分别估计可抓取区域和可移动区域，每个估计器编码特定属性的规律性来减少不确定性，通过双向信息交换聚焦于两个估计器一致的区域。

Result: 在真实世界数据集上评估，相比Where2Act、Hands-as-Probes和HRP方法，精度分别提高了308%、245%和257%，在低光照或杂乱环境下保持鲁棒性，真实世界评估成功率达到70%。

Conclusion: 耦合互补的估计器能够产生精确、鲁棒且适合具体实现的感知预测，为解决机器人感知中的歧义问题提供了有效方案。
...
### [83] [A Methodology for Dynamic Parameters Identification of 3-DOF Parallel Robots in Terms of Relevant Parameters](https://arxiv.org/abs/2603.15254)
*Miguel Díaz-Rodríguez,Vicente Mata,Angel Valera,Alvaro Page*

Main category: cs.RO

TL;DR: 提出一种针对全并联机器人动态参数识别的策略，通过选择相关参数子集、模型简化和加权最小二乘法，在满足物理可行性条件下获得可识别参数


<details>
  <summary>Details</summary>
Motivation: 机械系统动态参数识别对基于模型的控制和真实动态仿真很重要，但传统方法只能识别部分基参数，且某些参数由于对机器人动力学贡献小、测量噪声和建模差异而难以准确识别

Method: 从完整动态模型出发，考虑每个连杆的几何形状和全并联机器人腿的对称性进行简化，使用加权最小二乘法进行识别，通过统计考虑减少模型直到满足物理可行性条件

Result: 该方法在两种不同配置的实际3-DOF并联机器人上进行了实验测试，识别模型的逆动力学和正动力学响应与实验一致

Conclusion: 提出的相关参数识别策略有效解决了并联机器人动态参数识别问题，并提出了基于相关参数的正动力学计算方法
...
### [84] [HapticVLA: Contact-Rich Manipulation via Vision-Language-Action Model without Inference-Time Tactile Sensing](https://arxiv.org/abs/2603.15257)
...
TL;DR: HapticVLA：一种无需部署触觉传感器的触觉感知VLA架构，通过安全感知奖励加权流匹配和触觉蒸馏两阶段方法，实现离线学习触觉感知能力并在推理时仅使用视觉和状态信息生成触觉感知动作。
...
Method: 提出HapticVLA两阶段方法：1) 安全感知奖励加权流匹配(SA-RWFM)：训练包含预计算安全感知触觉奖励的流匹配动作专家，惩罚过度抓握力和次优抓握轨迹；2) 触觉蒸馏(TD)：从SA-RWFM教师模型中蒸馏出紧凑触觉令牌，训练学生VLA从视觉和状态模态预测该令牌，实现推理时无需触觉传感器的触觉感知动作生成。

Result: 在真实世界实验中，HapticVLA达到86.7%的平均成功率，持续优于基线VLA（包括推理时提供直接触觉反馈的版本）。

Conclusion: HapticVLA能够在VLA中保留接触丰富的触觉感知推理能力，同时消除部署时对板上触觉传感器的需求，为触觉感知操作提供了一种成本效益高且可复现的解决方案。
...
### [85] [MoE-ACT: Scaling Multi-Task Bimanual Manipulation with Sparse Language-Conditioned Mixture-of-Experts Transformers](https://arxiv.org/abs/2603.15265)
*Kangjun Guo,Haichao Liu,Yanji Sun,Ruhan Zhao,Jinni Zhou,Jun Ma*

Main category: cs.RO

TL;DR: MoE-ACT：一个轻量级多任务模仿学习框架，通过集成稀疏专家混合模块到ACT Transformer编码器中，显著提升双臂操作的多任务性能，平均成功率比原始ACT提高33%


<details>
  <summary>Details</summary>
Motivation: 机器人在实际家庭和工业应用中需要处理多个任务，但任务间的分布外变化常导致严重的任务干扰和负迁移问题，影响通用机器人策略的训练效果

Method: 提出MoE-ACT框架：1）在ACT Transformer编码器中集成稀疏专家混合模块，将统一任务策略分解为独立调用的专家组件；2）通过自适应激活在潜在空间解耦多任务动作分布；3）解码时使用FiLM动态调制动作令牌；4）多尺度交叉注意力同时关注低级和高级语义特征；5）整合文本信息，从纯视觉模型转变为视觉中心、语言条件的动作生成系统

Result: 在仿真和真实世界双臂设置中的实验验证表明，MoE-ACT显著提升多任务性能，平均成功率比原始ACT提高33%，展现出更强的鲁棒性和泛化能力

Conclusion: MoE-ACT通过专家混合模块有效缓解多任务学习中的任务干扰问题，为复杂双臂操作环境提供了更强大的多任务策略学习框架，具有实际部署价值
...
### [86] [GNIO: Gated Neural Inertial Odometry](https://arxiv.org/abs/2603.15281)
*Dapeng Feng,Yizhen Yin,Zhiqiang Chen,Yuhua Qi,Hongbo Chen*

Main category: cs.RO

TL;DR: GNIO是一种基于学习的惯性导航框架，通过可学习的运动库和门控预测头来减少MEMS传感器漂移，在静止期抑制噪声，在动态运动时缩放预测。
...
Method: 提出GNIO框架，包含两个关键创新：1) 可学习的运动库，从全局运动模式字典查询语义上下文；2) 门控预测头，将位移分解为幅度和方向，作为软可微的零速度更新机制。

Result: 在四个公开基准测试中，GNIO显著减少了位置漂移，在OxIOD数据集上轨迹误差降低60.21%，在频繁停止和不规则运动速度的挑战性场景中表现出优越的泛化能力。

Conclusion: GNIO通过显式建模运动有效性和上下文，有效解决了MEMS惯性导航中的漂移问题，特别是在静止期噪声抑制和动态运动预测方面表现优异。
...
### [87] [User-Tailored Learning to Forecast Walking Modes for Exosuits](https://arxiv.org/abs/2603.15329)
*Gabriele Abbate,Enrica Tricomi,Nathalie Gierden,Alessandro Giusti,Lorenzo Masia,Antonio Paolillo*

Main category: cs.RO

TL;DR: 基于机器学习的感知模块，仅使用两个惯性传感器数据，实时识别穿戴者三种步行模式（上楼梯、下楼梯、平地行走），支持在线自适应和闭环控制。


<details>
  <summary>Details</summary>
Motivation: 辅助机器人设备（如下肢软体外骨骼）需要适应不同用户，但现有系统传感器配置有限，难以实现高级感知功能。需要开发轻量化的感知系统来识别用户步行模式。

Method: 提出基于机器学习的感知模块，仅使用两个惯性传感器数据，能够同时估计未来和过去的步行模式。支持在线数据标注和模型自适应，实现用户定制化感知。

Result: 在真实数据集上的分析表明该感知模块有效。通过单受试者在线实验验证了系统与外骨骼闭环控制的性能。

Conclusion: 该轻量化感知系统仅需两个传感器即可准确识别三种步行模式，支持在线自适应和闭环控制，为软体外骨骼提供了实用的用户定制化感知解决方案。
...
### [88] [NavThinker: Action-Conditioned World Models for Coupled Prediction and Planning in Social Navigation](https://arxiv.org/abs/2603.15359)
*Tianshuai Hu,Zeying Gong,Lingdong Kong,XiaoDong Mei,Yiyi Ding,Qi Zeng,Ao Liang,Rong Li,Yangyi Zhong,Junwei Liang*

Main category: cs.RO

TL;DR: NavThinker是一个未来感知的社交导航框架，通过动作条件世界模型与强化学习耦合，实现机器人对场景和行人未来演变的预测规划，在多个数据集和真实机器人上表现出色。
...
Method: 提出NavThinker框架，将动作条件世界模型与在线策略强化学习（DD-PPO）耦合。世界模型在Depth Anything V2补丁特征空间中进行自回归预测，预测未来场景几何和人类运动；多头解码器生成未来深度图和人类轨迹。通过两种方式注入世界模型的前瞻信号：(i) 动作条件未来特征融合到当前观察嵌入中，(ii) 基于预测人类轨迹的社交奖励塑造。

Result: 在单机器人和多机器人Social-HM3D实验中实现了最先进的导航成功率，能够零样本迁移到Social-MP3D数据集，并在Unitree Go2机器人上进行了真实世界部署，验证了泛化能力和实际应用性。

Conclusion: NavThinker通过耦合动作条件世界模型和强化学习，有效解决了社交导航中的耦合预测-规划挑战，实现了对未来场景演变的推理，在模拟和真实环境中都表现出优异的性能和泛化能力。
...
### [89] [MA-VLCM: A Vision Language Critic Model for Value Estimation of Policies in Multi-Agent Team Settings](https://arxiv.org/abs/2603.15418)
*Shahil Shaik,Aditya Parameshwaran,Anshul Nayak,Jonathon M. Smereka,Yue Wang*

Main category: cs.RO

TL;DR: MA-VLCM框架使用预训练的视觉语言模型替代MARL中的中心化评论家，通过自然语言任务描述、视觉轨迹观察和多智能体状态信息进行条件评估，显著提高样本效率并生成适合资源受限机器人的紧凑执行策略。


<details>
  <summary>Details</summary>
Motivation: 传统多智能体强化学习中的中心化评论家从头学习效率低下且泛化能力差，而大型视觉语言动作模型虽然具有强大的多模态推理和零样本泛化能力，但直接部署到异构多机器人系统上计算成本过高。

Method: 提出MA-VLCM框架，用经过微调的预训练视觉语言模型替代MARL中的学习型中心化评论家，该模型基于自然语言任务描述、视觉轨迹观察和结构化多智能体状态信息来评估多智能体行为。

Result: 该方法在策略优化期间消除了评论家学习过程，显著提高了样本效率，同时在多智能体团队设置中，不同VLM骨干模型在分布内和分布外场景下都表现出良好的零样本回报估计能力。

Conclusion: MA-VLCM框架成功地将预训练视觉语言模型的强大泛化能力与多智能体强化学习相结合，为资源受限的异构多机器人系统提供了一种高效且可部署的解决方案。
...
### [90] [Zero-Shot Generalization from Motion Demonstrations to New Tasks](https://arxiv.org/abs/2603.15445)
*Kilian Freitag,Alvin Combrink,Nadia Figueroa*

Main category: cs.RO

TL;DR: 该论文提出了一种将孤立演示组合成共享工作空间的方法，通过引入高斯图将运动基元重新解释为离散顶点，实现连续控制与离散图搜索的桥接，从而能够泛化到未见过的任务。


<details>
  <summary>Details</summary>
Motivation: 从专家演示中学习运动策略是现代机器人学的重要范式。端到端模型需要大数据集和计算密集型推理，而现有的动态系统学习方法通常只能建模孤立任务，难以重用演示来学习新行为。需要一种方法能够组合孤立演示以实现任务泛化。

Method: 提出了高斯图的概念，将学习到的运动基元的空间组件重新解释为离散顶点及其连接。基于此图提出了两种框架：Stitching用于构建时间不变的动态系统，Chaining用于生成序列化动态系统以处理复杂运动，同时保留收敛保证。

Result: 仿真和真实机器人实验表明，这些方法能够成功泛化到新的任务，而基线方法在这些任务上失败。

Conclusion: 通过将连续控制与离散图搜索相结合，提出的高斯图方法能够有效组合孤立演示，实现任务泛化，为从少量演示中学习可泛化的动态系统提供了新思路。
...
### [91] [RoCo Challenge at AAAI 2026: Benchmarking Robotic Collaborative Manipulation for Assembly Towards Industrial Automation](https://arxiv.org/abs/2603.15469)
...
TL;DR: RoCo挑战赛旨在推动工业机器人装配的具身人工智能发展，通过行星齿轮箱装配任务评估仿真和真实世界中的机器人协作装配能力。


<details>
  <summary>Details</summary>
Motivation: 具身人工智能正从孤立感知转向集成连续行动，这对工业机器人操作具有重要意义。需要建立基准来评估和推进机器人在复杂装配任务中的协作能力，特别是面向人本制造的高精度装配场景。

Method: 开发了RoCo挑战赛及数据集，包含仿真和真实世界两阶段评估。仿真轮次在Isaac Sim中定义细粒度任务阶段进行逐步评分；真实世界轮次使用物理齿轮箱组件和高质量遥操作数据集。核心任务是组装行星齿轮箱，包括安装三个行星齿轮、太阳齿轮和齿圈。

Result: 吸引了来自10多个国家的60多个团队和170多名参与者。最有效的解决方案是ARC-VLA和RoboCola。结果表明，用于长视野多任务学习的双模型框架非常有效，战略性地利用从故障恢复的课程数据是成功部署的关键洞察。

Conclusion: RoCo挑战赛为工业具身人工智能提供了有价值的基准，展示了双模型框架和故障恢复数据策略的有效性。该竞赛为未来工业机器人协作装配研究指明了方向，相关数据集和代码已开源。
...
### [92] [On the Derivation of Tightly-Coupled LiDAR-Inertial Odometry with VoxelMap](https://arxiv.org/abs/2603.15471)
*Zhihao Zhan*

Main category: cs.RO

TL;DR: 该论文提供了紧密耦合的激光雷达-惯性里程计在迭代误差状态卡尔曼滤波器框架下的数学公式推导，使用VoxelMap表示，旨在统一几何建模和概率状态估计


<details>
  <summary>Details</summary>
Motivation: 为紧密耦合的激光雷达-惯性里程计系统提供一个清晰、自包含的数学推导，统一几何建模和概率状态估计，作为技术参考和入门指南

Method: 在迭代误差状态卡尔曼滤波器框架内，使用VoxelMap表示，提供紧密耦合的LiDAR-惯性里程计的数学公式推导，包括一致的符号表示和明确的公式

Result: 提供了一个清晰的数学推导框架，将几何建模和概率状态估计统一起来，为理解和实现紧密耦合的LiDAR-惯性里程计系统提供了技术参考

Conclusion: 该论文不是提出新算法，而是提供了一个清晰、自包含的推导，旨在作为紧密耦合LiDAR-惯性里程计系统的技术参考和入门指南，帮助理解系统架构和估计原理
...
### [93] [Optimal control of differentially flat underactuated planar robots in the perspective of oscillation mitigation](https://arxiv.org/abs/2603.15528)
*Stefano Lovato,Michele Tonan,Matteo Bottin,Matteo Massaro,Alberto Doria,Giulio Rosati*

Main category: cs.RO

TL;DR: 本文研究将最优控制与微分平坦控制结合，以改善欠驱动机器人的轨迹跟踪性能，减少末端执行器的残余振荡。


<details>
  <summary>Details</summary>
Motivation: 欠驱动机器人具有轻量化和高灵活性的优势，但被动关节的控制需要精确的动态建模。传统方法中忽略摩擦简化了模型，但会导致末端执行器在目标位置产生残余振荡，影响跟踪精度。

Method: 结合最优控制与微分平坦控制，通过形式化分析和数值仿真验证。使用不同的二次性能指标（控制力矩和欠驱动关节的势能）来规划平坦变量，设计对刚度和阻尼变化鲁棒的运动规律。

Result: 最优控制可用于规划平坦变量，考虑不同的性能指标。最小化势能可以设计对欠驱动关节刚度和阻尼变化鲁棒的运动规律，从而在刚度和阻尼不匹配情况下减少振荡。

Conclusion: 最优控制与微分平坦控制的结合能有效改善欠驱动机器人的轨迹跟踪性能，通过优化设计减少末端执行器的残余振荡，提高系统对参数变化的鲁棒性。
...
### [94] [From Passive Observer to Active Critic: Reinforcement Learning Elicits Process Reasoning for Robotic Manipulation](https://arxiv.org/abs/2603.15600)
*Yibin Liu,Yaxing Lyu,Daqi Gao,Zhixuan Liang,Weiliang Tang,Shilong Mu,Xiaokang Yang,Yao Mu*

Main category: cs.RO

TL;DR: PRIMO R1是一个7B参数框架，通过强化学习将视频MLLMs从被动"观察者"转变为主动"批评者"，用于长时程机器人操作的过程监督，显著提升进度估计准确性和失败检测能力。


<details>
  <summary>Details</summary>
Motivation: 当前视频MLLMs主要基于监督微调训练，只能作为被动"观察者"识别正在进行的事件，而无法评估当前状态相对于最终任务目标的进展。准确的过程监督对于长时程机器人操作仍然是一个关键挑战。

Method: 提出PRIMO R1框架，利用基于结果的强化学习激励显式的思维链生成来进行进度估计。架构上通过明确锚定初始状态和当前状态图像来构建结构化时序输入。支持PRIMO数据集和基准测试。

Result: 在多样化的域内环境和域外真实世界人形机器人场景中，PRIMO R1达到最先进性能。7B模型将专门推理基线的平均绝对误差降低50%，相对准确率显著优于72B规模的通用MLLMs。在RoboFail基准上以67.0%准确率创下新记录，超越OpenAI o1等闭源模型6.0%。

Conclusion: PRIMO R1成功将视频MLLMs从被动观察者转变为主动批评者，通过强化学习驱动的思维链生成显著提升了长时程机器人操作的过程监督能力，展现出强大的零样本泛化能力。
...
### [95] [EAAE: Energy-Aware Autonomous Exploration for UAVs in Unknown 3D Environments](https://arxiv.org/abs/2603.15604)
*Jacob Elskamp,Moji Shi,Leonard Bauersfeld,Davide Scaramuzza,Marija Popović*

Main category: cs.RO

TL;DR: EAAE是一个面向多旋翼无人机的能量感知自主探索框架，通过将能量作为显式决策变量，在保持探索效率的同时显著降低能耗。


<details>
  <summary>Details</summary>
Motivation: 传统基于覆盖或时间的探索策略往往产生高能耗的机动轨迹，而电池供电的多旋翼无人机任务性能常受能量而非几何因素限制，因此需要能量感知的自主探索方法。

Method: 提出EAAE模块化前沿框架：1) 将前沿聚类为视觉一致区域；2) 规划动态可行的候选轨迹到信息量最大的聚类；3) 通过离线功率估计循环预测执行能量；4) 采用双层规划架构，在最小化轨迹能量的同时保持探索进度。

Result: 在模拟3D环境中测试，与基于距离和基于信息增益的前沿基线相比，EAAE在保持竞争性探索时间和可比地图质量的同时，持续降低总能耗。

Conclusion: EAAE为前沿探索提供了一个实用的即插即用能量感知层，能够在保持探索性能的同时显著降低多旋翼无人机的能量消耗。
...
### [96] [Perception-Aware Autonomous Exploration in Feature-Limited Environments](https://arxiv.org/abs/2603.15605)
*Moji Shi,Rajitha de Silva,Hang Yu,Riccardo Polvara,Marija Popović*

Main category: cs.RO

TL;DR: 提出了一种感知感知的层次化探索框架，通过结合特征可观测性优化探索策略，提高无人机在特征稀疏环境中的定位稳定性


<details>
  <summary>Details</summary>
Motivation: 现有自主探索方法主要关注覆盖效率，但忽略了视觉惯性里程计性能依赖于鲁棒视觉特征的可用性。这可能导致机器人进入特征稀疏区域，造成里程计漂移、地图损坏和任务失败。

Method: 提出层次化感知感知探索框架：1) 使用全局特征地图评估候选边界的预期特征质量，优先选择视觉信息丰富的子目标；2) 沿规划运动优化连续偏航轨迹以保持稳定的特征跟踪。

Result: 在具有不同纹理水平的环境中进行仿真评估，并在具有大面积无纹理墙壁的真实室内环境中实验。相比忽略特征质量和/或不优化连续偏航的基线方法，本方法能保持更可靠的特征跟踪，减少里程计漂移，在里程计误差超过指定阈值前平均实现30%更高的覆盖率。

Conclusion: 通过显式耦合探索进度与特征可观测性，提出的感知感知探索框架能显著提高无人机在特征稀疏环境中的定位稳定性和探索效率。
