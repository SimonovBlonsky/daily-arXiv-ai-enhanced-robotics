...
TL;DR: VLLR框架结合LLM/VLM外部奖励和策略自确定性内在奖励，无需人工奖励工程即可微调机器人基础策略，在CHORES基准上显著提升任务成功率。
...
Method: 提出VLLR密集奖励框架：1) 使用LLM分解任务为可验证子任务，VLM估计进度初始化价值函数用于热身阶段；2) 基于策略自确定性的内在奖励在PPO微调中提供每步指导。

Result: 在CHORES基准（移动操作和导航）上，VLLR相比预训练策略绝对成功率提升达56%，相比最先进RL微调方法在分布内任务提升5%，在分布外任务提升10%，无需人工奖励工程。

Conclusion: VLLR框架通过结合LLM/VLM外部奖励和策略自确定性内在奖励，有效解决了机器人基础策略微调中的奖励工程问题，显著提升了任务完成效率和成功率。
...
TL;DR: MRReP：基于混合现实的界面，允许用户通过手势在物理地面上绘制手绘参考路径，将人类空间意图融入机器人导航


<details>
  <summary>Details</summary>
Motivation: 传统路径规划器主要优化几何成本，对人类操作者明确指定路径的支持有限，而自主移动机器人在共享环境中需要反映人类空间意图的路径

Method: 开发MRReP混合现实界面，用户通过手势在物理地面上绘制手绘参考路径，通过自定义的手绘参考路径规划器将用户指定的点序列转换为全局路径
...
TL;DR: 提出分层规划控制框架，结合MILP-MPC长时域引导和基于精确符号距离的CBF安全滤波，实现多面体机器人在多面体障碍环境中的几何感知安全导航
...
Method: 分层框架：高层使用MILP嵌入MPC生成点质量机器人的名义轨迹；低层使用基于Minkowski差空间精确符号距离的CBF作为安全滤波器，并扩展到高阶CBF，强制执行机器人几何约束

Result: 在U形和迷宫环境中对单积分器和双积分器动力学进行测试，结果表明该框架减轻了纯反应式CBF导航的拓扑诱导局部极小值行为，实现了安全、实时、几何感知的导航

Conclusion: 提出的分层规划控制框架成功结合了长时域引导和几何感知安全保证，为多面体机器人在多面体障碍环境中的自主导航提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 开发稳健的仿人机器人自主运动操作技能是机器人学中的开放问题。虽然强化学习在腿部运动控制方面取得了成功，但将其应用于复杂、交互丰富的操作任务面临长期规划挑战。现有的DreamControl方法使用现成的人类运动扩散模型作为生成先验来指导强化学习策略，但存在局限性。

Method: 提出改进框架：1）直接在仿人机器人运动空间训练引导扩散模型；2）将多样化的人类和机器人数据集聚合到统一的体现空间中；3）建立更自动化的流程，无需手动过滤干预；4）通过扩展参考轨迹生成来获得更稳健的下游强化学习策略。

Result: 该方法能够捕捉更广泛的技能（得益于更大的训练数据混合），建立了更自动化的流程，并通过扩展参考轨迹生成实现了更稳健的下游强化学习策略。在仿真和真实Unitree-G1机器人上进行了广泛实验验证。

Conclusion: 提出的改进框架通过直接在机器人运动空间训练扩散模型并整合多源数据，显著提升了仿人机器人运动操作技能的学习效果和自动化程度，为复杂机器人任务提供了更有效的解决方案。
...
TL;DR: REMind是一个教育机器人介导的角色扮演游戏，旨在通过机器人角色扮演支持儿童的反欺凌旁观者干预训练。
...
Method: 设计REMind游戏系统，让玩家观察社交机器人表演的欺凌场景，反思角色视角，并通过操控机器人化身来练习防御策略。通过混合方法对18名9-10岁儿童进行游戏测试研究。

Result: 研究发现该体验支持了关键学习目标，包括自我效能感、视角采择、理解防御结果和干预策略。结果突显了机器人介导应用戏剧作为支持社会情感学习的新教学框架的潜力。

Conclusion: REMind展示了机器人介导应用戏剧作为支持儿童反欺凌旁观者干预的有效教学框架，为社交情感学习提供了创新方法。
...
TL;DR: 提出分层规划控制框架，在未知非线性动力学下在线学习系统特性并导航，通过自适应状态空间分区和可达性验证实现探索与利用的平衡


<details>
  <summary>Details</summary>
Motivation: 在未知非线性动力学下进行自主运动规划需要同时学习系统特性和导航到目标，现有方法通常需要大量先验知识或计算复杂

Method: 1) 将状态空间划分为多面体，使用分段仿射模型近似未知非线性系统；2) 引入非均匀自适应分区策略，仅在任务相关区域细化分区；3) 将PWA系统抽象为有向加权图，使用可达控制理论和预测可达条件增量验证边；4) 在线更新图并进行高层图搜索规划，低层合成仿射反馈控制器执行计划；5) 引入松弛可达条件扩展框架到欠驱动系统

Result: 仿真结果表明该方法能有效平衡探索与利用，同时提供形式化的可达性保证，特别是在欠驱动系统设置下也能工作

Conclusion: 提出的分层规划控制框架能够在有限先验知识下实现在线运动合成，通过自适应分区和可达性验证提供形式化保证，扩展了可达控制理论在欠驱动系统中的应用
...
TL;DR: 该论文提出了一种让空中机器人能够使用机载传感器和嵌入式飞行硬件实时推理周围风场的方法，核心创新是将距离测量与稀疏现场风测量融合来预测周围流场。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决两个基本问题：1）地形数据在密集城市环境中对准确风预测是否足够；2）学习到的风模型在运动规划中的实用性，重点关注能量效率和障碍物避让。通过让机器人能够快速评估局部风况而无需先验环境知识，加速空中机器人进入更具挑战性的环境。

Method: 结合深度学习、流体力学和最优控制工具，建立了一个使用导航LiDAR进行局部风预测的框架，然后将局部风模型先验整合到后退时域最优控制器中，研究局部风知识如何影响自主导航中的能量使用和鲁棒性。
...
TL;DR: 使用Webots机器人仿真平台模拟了Braitenberg机器人的群体捕食者规避和围攻行为，研究了围攻呼叫范围和群体规模对围攻成功率的影响。


<details>
  <summary>Details</summary>
Motivation: 模仿自然界中动物通过围攻呼叫协调群体攻击捕食者的行为机制，探索人工生命系统中行动选择的仿真方法，为自主智能体控制架构设计提供参考。

Method: 使用Webots机器人仿真平台，模拟Braitenberg机器人群体面对光源（代表无生命捕食者）时的行为：当遇到光源时，机器人会发出围攻呼叫召集同伴进行围攻，否则选择逃离。研究了围攻呼叫范围（无限范围、中范围、低范围）和群体规模（10个机器人vs 3个机器人）两个变量。

Result: 结果表明，围攻呼叫范围和群体规模都对围攻成功率有显著影响。较大的群体规模和较广的呼叫范围能提高围攻行为的整体成功率。

Conclusion: 这项研究对人工生命系统中的行动选择仿真具有重要意义，并为自主智能体的控制架构设计提供了有价值的参考，展示了生物启发机制在机器人群体协调中的应用潜力。
...
TL;DR: 该论文提出了一种基于热红外和深度信息(TIR-D)的鲁棒人员跟踪架构，用于解决RGB-D跟踪在恶劣光照条件下的性能退化问题，通过顺序知识转移策略和细粒度差分学习率策略实现了优越的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 自主移动机器人在多样化和不可预测环境中需要鲁棒的人员跟踪能力。虽然RGB-D跟踪具有高精度，但在完全黑暗或强烈背光等挑战性光照条件下性能严重下降。为了在所有天气条件下实现鲁棒性，需要利用机器人标准传感器套件（LiDAR和热红外相机）开发新的跟踪方法。

Method: 提出热红外-深度(TIR-D)跟踪架构，采用顺序知识转移策略，将大规模热红外训练模型的结构先验演化到TIR-D领域。使用细粒度差分学习率策略，在保留预训练特征提取能力的同时，快速适应几何深度线索。

Result: 实验结果显示，提出的TIR-D跟踪器实现了优越性能，平均重叠度(AO)达到0.700，成功率(SR)达到58.7%，显著优于传统的RGB迁移和单模态基线方法。

Conclusion: 该方法为全天候机器人应用中鲁棒的人员跟随提供了实用且资源高效的解决方案，通过利用标准传感器套件和创新的训练策略，有效解决了恶劣光照条件下的跟踪挑战。
...
TL;DR: 本文提出了一种在不确定环境中集成任务和运动规划的方法，使用LTLf规范处理部分可观测环境中的非线性机器人动力学问题。


<details>
  <summary>Details</summary>
Motivation: 解决在不确定环境中机器人同时进行任务规划和运动规划的问题，特别是环境语义标签的不确定性，需要处理非线性动力学和部分可观测性。

Method: 将问题建模为部分可观测随机混合系统，提出了一种利用混合系统结构的随时算法，结合决策技术和基于采样的运动规划。

Result: 算法在不确定环境中表现出高效性，始终优于基线方法，并证明了算法的正确性和渐近最优性。

Conclusion: 该方法成功解决了不确定环境中的集成任务和运动规划问题，为具有非线性动力学和部分可观测性的机器人系统提供了有效的解决方案。
...
TL;DR: EgoNav系统仅用5小时人类行走数据训练，无需机器人数据或微调，就能让仿人机器人在未见过的多样化环境中导航。系统通过扩散模型预测未来轨迹分布，结合360度视觉记忆和DINOv3特征，实现实时推理和避障。


<details>
  <summary>Details</summary>
Motivation: 当前机器人导航系统通常需要大量机器人数据或特定环境训练，限制了在多样化、未见环境中的泛化能力。EgoNav旨在仅用少量人类行走数据，实现仿人机器人在各种未知环境中的零样本导航。

Method: 1) 使用扩散模型预测未来轨迹分布，条件包括：过去轨迹、融合颜色/深度/语义的360度视觉记忆、以及DINOv3提取的视频特征；2) 混合采样方案实现10步去噪的实时推理；3) 滚动时域控制器从预测分布中选择路径。

Result: 离线评估显示EgoNav在避撞和多模态覆盖方面优于基线方法。在Unitree G1仿人机器人上的零样本部署中，系统能在未见过的室内外环境中自然涌现出等待开门、绕开人群、避开玻璃墙等行为。

Conclusion: EgoNav证明了仅用少量人类行走数据就能训练出在多样化未见环境中导航的仿人机器人系统，无需机器人数据或微调，展现了从学习先验中自然涌现复杂导航行为的能力。
...
TL;DR: CDCBS通过引入证书轨迹和舰队预算机制，改进了ACCBS算法，在密集场景下提供更一致的解质量，同时通过预算限制因子化减少有效组大小。


<details>
  <summary>Details</summary>
Motivation: 现有闭环MAPF算法（如ACCBS）在有限计算预算下可能因短视的有限视野规划而终止于短活动前缀，特别是在密集场景中，难以保持全局保证和利用组合结构。

Method: 引入证书轨迹及其关联的舰队预算作为过滤闭环更新的通用机制。证书提供无冲突回退计划和剩余成本的单调上界；仅接受证书改进的更新可确保完整性。预算信息诱导预算限制因子化，实现跨时间步的全局可继承分解。将该框架实例化到ACCBS上得到CDCBS。

Result: 在基准地图上的实验表明，CDCBS比ACCBS获得更一致的解质量，特别是在密集设置中，同时提出的因子化减少了有效组大小。

Conclusion: 证书轨迹和舰队预算机制为闭环MAPF算法提供了有效的全局保证和组合结构利用方法，CDCBS在密集场景下表现出更好的性能一致性。
...
TL;DR: 提出一种用于双积分器系统的路径跟踪方法，通过离线速度缩放和在线QP跟踪相结合，确保在加速度限制下安全跟踪包含急转弯的高曲率路径。


<details>
  <summary>Details</summary>
Motivation: 基于采样的规划器生成的路径可能包含急转弯和高曲率区域，即使在几何上无碰撞，但在加速度限制下的一步可达性变得至关重要。需要确保系统能在物理约束下安全跟踪这些路径。

Method: 采用纯追踪风格、可达性引导的二次规划跟踪器，具有一步加速度裕度。离线阶段：沿样条路径评估加速度裕度，更新标量速度缩放剖面，使所需一步加速度保持在可用界限内。在线阶段：使用相同的向前看跟踪结构跟踪缩放后的参考路径。

Result: 该方法能够确保双积分器系统在速度和加速度限制下，安全跟踪包含急转弯和高曲率区域的路径，通过速度缩放优化了跟踪性能。

Conclusion: 通过离线速度缩放和在线QP跟踪相结合的方法，有效解决了在物理约束下跟踪复杂路径的问题，为基于采样的规划器生成的路径提供了可靠的跟踪解决方案。
...
TL;DR: 通过多视角相机采集专家演示，无需额外人工成本即可增强模仿学习的泛化能力，提出多视角动作聚合方法提升数据效率和泛化性能
...
Method: 提出一个实用框架：在演示收集过程中扩展相机视角，使用多个同步相机视角从每个专家轨迹生成伪演示，丰富训练分布并提升视觉表示中的视角不变性。分析不同动作空间与视角扩展的交互，引入多视角动作聚合方法，使单视角策略在部署时能从多个相机中受益。

Result: 在仿真和真实世界操作任务中的大量实验表明，相比单视角基线，该方法在数据效率和泛化方面取得显著提升。相机空间表示进一步增强了多样性。
...
<details>
  <summary>Details</summary>
Motivation: 动物通过脑体协同进化，利用内在身体动力学实现节能运动，而机器人通常采用集中式控制器抑制而非利用身体动力学。需要一种方法让机器人身体也能利用被动动力学实现节能

Method: 提出物理模仿学习（PIL），将强化学习控制策略蒸馏到被动并联弹性关节（PEJ）中，让身体模仿部分控制行为，剩余部分由电机执行。这样将部分控制策略卸载到PEJ，实现脑体协同设计

Result: 在模拟四足机器人中，PIL方法在平坦地形上可将87%的机械功率卸载到PEJ，在崎岖地形上可卸载18%。整体能耗降低，且不增加搜索空间复杂度

Conclusion: PIL提供了一种计算高效的脑体协同设计方法，通过将控制策略蒸馏到身体设计中而非联合优化，实现了任务特定的具身物理智能，适用于广泛的关节型机器人形态
...
TL;DR: StretchBot：一种结合多模态感知与知识图谱增强LLM推理的混合神经符号机器人教练，用于自适应辅助拉伸指导，相比脚本化系统能更好地适应用户状态和环境上下文。


<details>
  <summary>Details</summary>
Motivation: 现有辅助机器人系统大多采用脚本化方式，缺乏根据用户状态、环境上下文和交互动态进行自适应调整的能力，限制了其在家庭和医疗场景中的实际应用效果。

Method: 开发了StretchBot混合神经符号机器人教练系统，结合多模态感知与基于知识图谱的大型语言模型推理，支持在保持结构化常规的同时进行上下文感知的调整。
...
TL;DR: LiPS是一种轻量级全景分割方法，在保持查询式解码的同时简化特征提取和融合路径，在计算资源受限的移动机器人平台上实现高效全景分割。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的全景分割模型复杂度高，不适合部署在移动机器人等计算资源受限的平台，需要一种既能保持性能又能大幅降低计算需求的解决方案。

Method: 提出LiPS方法，采用轻量级设计，保留基于查询的解码机制，同时引入简化的特征提取和融合路径，优化计算效率。

Result: 在标准基准测试中，LiPS达到与更重基线模型相当的精度，同时提供高达4.5倍的吞吐量（每秒帧数），计算需求减少近6.8倍。

Conclusion: LiPS的高效性使其成为现代全景分割模型与真实世界机器人应用之间的重要桥梁，特别适合资源受限的移动机器人平台。
...
TL;DR: SoftHand Model-W是一款3D打印、欠驱动、拟人化的机器人手，基于Pisa/IIT SoftHand设计，集成了拮抗肌腱机制和2自由度肌腱驱动手腕，通过远程电机布置减少远端惯性，在堆叠立方体和旋转圆盘任务中证明手腕驱动能减少手臂补偿运动，提高任务完成速度。


<details>
  <summary>Details</summary>
Motivation: 开发具有手腕功能的机器人手，以增强机器人在真实世界应用中的多功能性和拟人化操作能力，减少手臂的补偿和重新配置运动，实现更高效的任务完成。

Method: 基于Pisa/IIT SoftHand设计3D打印的欠驱动拟人化机器人手，集成拮抗肌腱机制和2自由度肌腱驱动手腕（屈伸和桡尺偏），采用腕管式肌腱布线将电机远程布置在前臂，减少远端惯性，保持紧凑外形。将SoftHand-W安装在6轴机器人手臂上进行立方体堆叠和平面圆盘旋转任务测试。

Result: 与无手腕驱动相比，添加手腕驱动能显著减少手臂的补偿和重新配置运动，缩短任务完成时间。手腕还使原本不可能的拾放操作成为可能。手腕的主动自由度是实现多功能、拟人化操作的关键。

Conclusion: SoftHand Model-W展示了近端自由度对于实现真实世界机器人应用中多功能、拟人化操作的重要性，其紧凑设计使其适用于研究和辅助场景的部署。
...
TL;DR: TacViT：基于视觉Transformer的新型触觉感知模型，能够在未见过的传感器上实现泛化，减少数据收集和重新训练需求，加速新传感器部署
...
Method: 提出TacViT模型，基于视觉Transformer架构，利用全局自注意力机制从触觉图像中提取鲁棒特征，实现接触属性推断

Result: 在五指机器人手的传感器上评估，TacViT相比CNN展现出更优越的泛化性能，能够准确推断未见传感器的接触属性
...
TL;DR: 提出了一种轻量级指尖触觉设备，通过双电机机制提供可区分的表面和边缘接触反馈，在VR环境中增强物体操纵的沉浸感和精确度。


<details>
  <summary>Details</summary>
Motivation: 传统的指尖触觉设备无法渲染某些对沉浸式和精确交互相关的触觉特征，如边缘检测，这限制了VR应用中物体操纵的体验。

Method: 开发了一种紧凑轻量（24.3克）的指尖触觉设备，采用新颖的双电机机制来提供表面和边缘接触反馈。使用6×6柔性传感器阵列进行压力分布表征，验证了两种刺激模式的不同接触模式。

Result: 初步用户研究（5名参与者）在四种条件（轻/重压力下的边缘/表面接触）下实现了93%的平均分类准确率，平均响应时间为2.79秒。压力分布表征显示两种刺激模式具有明显不同的接触模式。

Conclusion: 该设备能够有效传达边缘和表面触觉线索，有望提高VR环境中物体操纵的保真度，为更沉浸和精确的VR交互提供了新的触觉反馈解决方案。
...
TL;DR: 研究人员开发了一种基于织物的双动作软体机器人手套，用于手部功能障碍患者的辅助和康复。该手套采用定制化设计，包含五个独立控制的柔性执行器，支持手指屈曲和伸展，以及专门的拇指外展执行器。


<details>
  <summary>Details</summary>
Motivation: 神经系统疾病导致的手部功能障碍严重影响日常生活独立性，需要有效的辅助和康复策略。现有软体机器人手套在定制化、人体工程学适配和屈伸驱动方面存在限制，制约了其临床应用。

Method: 采用计算机数控热封技术制造对称腔室执行器，设计具有凹形外表面的执行器以最大化手指接触面积。手套包含五个独立控制的双动作执行器（支持屈曲和伸展）和一个拇指外展执行器。

Result: 执行器产生足够的关节力矩和指尖力用于日常生活任务；完整手套系统对常见家居物品产生足够的抓握力。健康受试者测试显示主动辅助显著减少前臂肌肉活动；脊髓损伤患者测试表明手套辅助促进更自然的抓握模式，减少对肌腱转移抓握的依赖。

Conclusion: 这种可定制、符合人体工程学的设计代表了向个性化手部康复和辅助机器人迈出的实际一步，尽管当前驱动界面增加了任务完成时间。
...
TL;DR: 提出改进的多智能体RGB-D高斯泼溅SLAM框架，通过压缩冗余3D高斯和集中式闭环检测，在保持地图质量的同时减少85-95%的通信负载


<details>
  <summary>Details</summary>
Motivation: 多智能体3D建图中，密集地图表示在有限带宽下难以实时传输，特别是高斯泼溅等新方法生成的大型地图成为实际部署的瓶颈

Method: 1) 在SLAM系统中加入压缩步骤去除冗余3D高斯而不降低渲染质量；2) 无需初始猜测的集中式闭环检测，提供纯渲染深度模式和相机深度模式两种方案

Result: 在合成和真实数据集上评估，相比现有方法减少85-95%的传输数据量，使3D高斯多智能体SLAM更接近实际部署

Conclusion: 提出的改进框架显著降低了通信负载，同时保持地图保真度，推动了3D高斯多智能体SLAM在实际场景中的应用
...
TL;DR: 本文针对无人机视觉惯性SLAM中传统传感器视野有限的问题，提出了一种全景视觉惯性SLAM框架，并构建了首个真实世界的无人机全景视觉惯性数据集，在挑战性场景下实现了更准确、鲁棒的位姿估计。


<details>
  <summary>Details</summary>
Motivation: 现有视觉惯性SLAM方法主要依赖视野有限的传感器，在复杂无人机场景中容易产生漂移甚至失败。虽然全景相机提供全方位感知以提高鲁棒性，但针对无人机的全景视觉惯性SLAM及相关真实世界数据集仍未被充分探索。

Method: 首先构建了一个覆盖多样化飞行条件的真实世界全景视觉惯性数据集。然后提出了一个全景视觉惯性SLAM框架，通过全景特征提取和全景闭环检测来利用全方位视野，增强特征约束并确保全局一致性。

Result: 在提出的数据集和公共基准上的大量实验表明，该方法相比现有方法实现了更优越的准确性、鲁棒性和一致性。在嵌入式平台上的部署验证了其实际适用性，达到了与PC实现相当的计算效率。

Conclusion: 该研究填补了无人机全景视觉惯性SLAM领域的空白，通过构建首个真实世界数据集并提出有效的全景SLAM框架，为无人机在复杂环境中的精确定位提供了更可靠的解决方案。
...
TL;DR: 设计用于灾后救援的软体夹爪机器人系统，能够测量受害者的脉搏和血压，在灾后环境中实现医疗评估功能


<details>
  <summary>Details</summary>
Motivation: 虽然机器人已广泛应用于搜救行动，受害评估领域也取得进展，但心率测量仍存在问题，且缺乏针对灾后场景的血压评估研究。需要开发能在灾后环境中准确测量受害者生命体征的系统。

Method: 设计软体夹爪并集成到移动机器人系统中，夹爪能够包裹受害者手臂并像血压计一样充气，采用专门的便携系统。使用不同信号处理算法处理测量数据。

Result: 系统实现了脉搏偏差4 bpm，收缩压和舒张压偏差约5 mmHg。统计数据和误差项同方差性验证证明了系统准确测量心率和血压的能力，适合搜救操作。

Conclusion: 该系统能够准确测量灾后受害者的心率和血压，通过灾后场景测试验证了其适应不同受害者姿势的能力、测量速度和安全性，适合搜救行动使用。
...
TL;DR: 本文提出了一种基于核均值嵌入的无限时域遍历控制器，用于解决长期覆盖任务中的计算扩展性问题，通过扩展的误差访问状态实现无限时域控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于核的遍历控制方法虽然在一般覆盖域上提供了强大的覆盖保证，但由于计算复杂度随时间的扩展性差，实际应用仅限于次遍历的有限时域，无法用于长期覆盖任务。

Method: 提出无限时域遍历控制器，配备扩展的核均值嵌入误差访问状态，递归记录状态访问历史；该扩展状态将过去访问与未来控制解耦，并将遍历控制扩展到无限时域；同时提出基于扩展误差状态的滚动时域控制变体。

Result: 提供了控制器的渐近收敛性理论证明，并在2D和3D覆盖问题中展示了遍历覆盖保证的保持。

Conclusion: 该方法解决了现有遍历控制方法的计算扩展性问题，使长期覆盖任务成为可能，同时保持了遍历覆盖的理论保证。
...
TL;DR: 提出一种基于Stein变分分布鲁棒控制的非线性动力系统控制器，用于处理潜在参数不确定性，通过任务相关的粒子近似替代保守的最坏情况优化，在保持计算并行性的同时实现更好的性能-鲁棒性权衡。


<details>
  <summary>Details</summary>
Motivation: 传统分布鲁棒优化（DRO）方法通常采用保守的最坏情况设计，可能牺牲名义性能。现有方法在处理非线性动力系统的潜在参数不确定性时，要么过于保守，要么需要限制性的参数假设。需要一种既能保持鲁棒性又不牺牲名义性能的方法。

Method: 将最优控制与Stein变分推断耦合，使用任务相关的粒子近似不确定性分布，避免对不确定性模型的限制性参数假设。通过集中关注对闭环性能影响最大的参数敏感性，使控制器能够围绕最关键的参数不确定性来塑造控制律。

Result: 在代表性控制问题上，该方法相比名义控制、集成控制和经典分布鲁棒基线，在性能-鲁棒性权衡方面表现出改进。控制器能够在不牺牲名义性能的情况下实现鲁棒性，同时保持计算并行性。

Conclusion: 该方法在单一决策理论框架中调和了鲁棒控制与变分推断，适用于具有参数不确定性的广泛控制系统类别，提供了比传统方法更好的性能-鲁棒性平衡。
...
TL;DR: BAT是一个在线策略切换框架，通过动态选择两个互补的全身RL控制器，在长时程任务中平衡敏捷性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两种范式：耦合全身策略用于全局协调，解耦策略用于模块化精度。缺乏系统方法整合两者，导致敏捷性、鲁棒性和精度之间的权衡问题未解决。

Method: 提出BAT框架，包含两个互补模块：1) 通过分层RL学习的切换策略，带有滑动时域策略预评估的专家指导；2) 选项感知VQ-VAE，从离散运动标记序列预测选项偏好以改进泛化。最终决策通过两个模块的置信度加权融合获得。

Result: 在Unitree G1人形机器人上的广泛仿真和真实世界实验表明，BAT能够实现多功能的长时程运动操作，并在多样化任务中优于先前方法。

Conclusion: BAT框架通过动态策略切换成功解决了敏捷性、鲁棒性和精度之间的权衡问题，为人形机器人的长时程运动操作任务提供了有效的解决方案。
...
TL;DR: 该论文提出了一个基于无人机采集的车辆-弱势道路使用者交互数据集(VRUD)，专门针对城市复杂交通环境中的自动驾驶安全研究，填补了现有数据集在非结构化城市环境中的空白。


<details>
  <summary>Details</summary>
Motivation: 现有开源数据集主要关注高速公路或规范交叉口等结构化场景，缺乏代表混乱、非结构化城市环境的数据。城市L4级自动驾驶面临弱势道路使用者(VRU)密度高、交互行为不确定等挑战，需要专门的数据集来提升自动驾驶系统在复杂环境中的安全性能。

Method: 采用高效、高精度的无人机数据采集方法，在深圳典型"城中村"区域收集数据。这些区域具有交通监管松散、遮挡严重的特点。数据集包含4小时4K/30Hz视频记录，提取了11,479个VRU轨迹和1,939个车辆轨迹。基于新颖的矢量碰撞时间(VTTC)阈值提取了4,002个多智能体交互场景，并提供了标准的OpenDRIVE高清地图支持。

Result: 创建了VRUD数据集，其中VRU占所有交通参与者的约87%，显著超过现有基准数据集的比例。数据集提供了丰富的边缘案例资源，专门针对复杂非结构化城市环境中的自动驾驶安全研究。数据集已开源供进一步研究使用。

Conclusion: VRUD数据集为增强自动驾驶系统在复杂非结构化城市环境中的安全性能提供了宝贵且罕见的边缘案例资源。该数据集填补了现有数据集的空白，特别关注高密度VRU环境下的交互行为，有助于推动城市自动驾驶安全研究的发展。
...
TL;DR: 论文提出了一种结合强化学习与有界极值搜索的混合控制器，用于提高机器人操作任务在分布外条件下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人操作中表现良好，但训练的策略在测试条件与训练分布不同时性能会下降。这在接触丰富的任务（如推动和抓放）中尤为突出，因为目标、接触条件或机器人动力学的变化都可能导致推理时系统处于分布外状态。

Method: 提出混合控制器，将深度确定性策略梯度（DDPG）训练的标准条件策略与有界极值搜索（bounded ES）相结合。RL策略提供快速操作行为，而有界ES确保在操作条件偏离训练条件时控制器的鲁棒性。

Result: 该控制器在多种分布外设置下进行评估，包括时变目标和空间变化的摩擦斑块条件，验证了其鲁棒性。

Conclusion: 结合强化学习与有界极值搜索的混合控制器能够有效提高机器人操作任务在分布外条件下的鲁棒性，为实际部署提供了更可靠的解决方案。
...
TL;DR: 首个基于机载感知的人形机器人乒乓球系统，通过统一的可扩展全身技能学习和机载感知，实现敏捷精准的连续击球，无需外部摄像头。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人乒乓球系统依赖外部感知，且无法实现敏捷的全身协调以执行精确任务。主要挑战在于：快速运动下的低延迟鲁棒机载感知，以及获取足够多样化的任务对齐击球动作来学习精确自然的全身行为。

Method: 提出模块化系统，统一可扩展全身技能学习与机载感知。通过紧密协调的全身控制实现敏捷精准的球交互，而非解耦的上半身和下半身行为。使用生成模型增强和多样化击球动作，获得可扩展的运动先验。实现首个仅使用机载感知的人形机器人连续击球系统。

Result: 在真实世界实验中展示了高速条件下的稳定精准球交换，包括爆发性全身扣杀和低蹲击球等多样化击球动作。系统能够仅使用机载感知实现连续击球，克服了低延迟感知、自我运动引起的不稳定性和有限视野等挑战。

Conclusion: 验证了可扩展、感知驱动的全身技能学习对于动态人形交互任务的有效性，为人形机器人系统提供了无需外部感知的敏捷精准交互能力。
...
<details>
  <summary>Details</summary>
Motivation: 高效的机器人地外探测需要多样化的机器人能力，机器人团队可以将任务分配给多个专门子系统。传统规划算法随着问题规模增大会出现组合爆炸，导致规划周期长、推理成本高。学习型方法将扩展性问题从运行时转移到训练时，是实现实时规划的关键一步。

Method: 采用多智能体近端策略优化(MAPPO)算法，开发协作规划策略来协调异构机器人团队，解决复杂的目标分配和调度问题。该方法将学习型方法应用于机器人团队协调。

Result: 通过穷举搜索获得的单目标最优解进行基准测试，并在行星探测场景中评估其在线重新规划能力。结果表明该方法在复杂目标分配和调度问题上具有可行性。

Conclusion: 基于MAPPO的协作规划策略为异构机器人团队协调提供了有效的学习型方法，相比传统规划算法具有更好的可扩展性，是实现实时规划的重要进展。
...
TL;DR: SoftAct是一个通过显式推理接触力来教授软体机器人手执行类人操作技能的框架，使用VR采集人类演示数据，通过两阶段力感知重定向算法实现技能迁移。
...
Method: 1) 使用沉浸式VR采集包含手部运动学、物体运动、密集接触斑块和详细接触力信息的人类演示；2) 两阶段力感知重定向算法：第一阶段将演示的接触力分配到各手指并比例分配机器人手指，建立力平衡映射；第二阶段结合基线末端执行器姿态跟踪和测地加权接触细化进行在线重定向。

Result: 在接触丰富的操作任务上，SoftAct控制器将指尖轨迹跟踪RMSE降低达55%，跟踪方差降低达69%；在策略层面，在零样本真实世界部署和仿真中均获得更高成功率。

Conclusion: 显式建模接触几何和力分布对于软体机器人手的有效技能迁移至关重要，仅通过运动学模仿无法恢复这些信息，SoftAct框架能够使软体机器人手再现人类演示的功能意图。
