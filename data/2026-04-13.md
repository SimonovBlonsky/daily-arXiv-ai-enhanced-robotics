...
TL;DR: 提出基于数据驱动的机器人设计新范式：从现有机械设计中学习设计搜索空间，通过运动重定向和Procrustes分析从人类运动数据直接定义损失函数，实现人形机器人上半身设计的自动化优化。


<details>
  <summary>Details</summary>
Motivation: 传统机器人形态和运动学设计依赖人类直觉，缺乏系统基础。运动设计协同优化面临两大挑战：巨大的非结构化设计空间和难以构建任务特定损失函数。

Method: 1) 从现有机械设计中学习设计搜索空间而非手工构建；2) 通过运动重定向和Procrustes分析从人类运动数据直接定义损失函数；3) 使用基于螺旋理论的关节轴表示和等距流形学习构建紧凑的几何保持潜在空间；4) 在潜在空间中使用无梯度优化进行设计优化。

Result: 建立了一个基于数据驱动的机器人设计原则框架，证明利用现有设计和人类运动可以有效指导新型机器人设计的自动化发现。

Conclusion: 该方法最小化了人类参与，通过数据驱动方法实现了机器人设计的自动化优化，为机器人设计提供了新的系统化范式。
...
TL;DR: 提出首个用于物理人机交互的零样本"文本到仿真到现实"生成框架，通过自然语言提示自动合成多样化的pHRI场景，实现大规模合成数据生成和视觉模仿学习策略训练。
...
Method: 利用大语言模型和视觉语言模型，从高级自然语言提示程序化生成软体人体模型、场景布局和机器人运动轨迹，收集大规模合成演示数据集，训练基于分割点云的视觉模仿学习策略。

Result: 在抓挠和洗澡两个辅助任务上，学习到的策略成功实现零样本仿真到现实迁移，成功率超过80%，并对非脚本化人体运动表现出鲁棒性。

Conclusion: 提出了首个用于pHRI应用的生成仿真流水线，实现了仿真环境合成、数据收集和策略学习的自动化，为物理人机交互系统开发提供了新方法。
...
TL;DR: 提出分层框架解决双手操作中的联合可操作性定位和手臂分配问题，利用视觉语言模型实现跨类别泛化，在真实世界任务中优于几何和语义基线方法。


<details>
  <summary>Details</summary>
Motivation: 双手操作需要同时考虑物体交互位置和手臂分配，这需要语义理解任务意图，而纯几何规划器无法解决。现有方法要么将可操作性预测视为粗略部分分割，要么依赖几何启发式进行手臂分配，未能联合推理任务相关的接触区域和手臂分配。

Method: 将双手操作重构为联合可操作性定位和手臂分配问题，提出分层框架：1）融合多视角RGB-D观测到一致的3D场景表示；2）生成全局6自由度抓取候选；3）通过查询视觉语言模型对每个物体进行空间和语义过滤，获取任务相关的可操作性区域和手臂分配，确保几何有效性同时尊重任务语义。

Result: 在双臂平台上评估了9个真实世界操作任务，涵盖4个类别：并行操作、协调稳定、工具使用和人类交接。方法在任务导向抓取方面始终比几何和语义基线获得更高的任务成功率。

Conclusion: 对可操作性和手臂分配的显式语义推理有助于在非结构化环境中实现可靠的双手操作，视觉语言模型能够实现跨物体类别和任务描述的泛化，无需类别特定训练。
...
TL;DR: 提出一种可泛化的四足机器人世界模型(QWM)，通过显式编码机器人形态学参数，实现跨不同四足机器人平台的零样本控制，解决了现有世界模型对特定硬件过拟合的问题。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型存在硬件锁定的问题：针对特定机器人训练的世界模型无法泛化到其他形态的四足机器人，因为模型过度拟合了特定机器人的运动学和动力学特性，而非学习通用的运动动力学。微小的执行器动态或肢体长度变化都需要重新训练模型。

Method: 提出Quadrupedal World Model (QWM)框架，通过显式地将生成动力学条件化于机器人的工程规格，而非将静态物理属性作为隐变量从运动历史中推断。方法整合了物理形态编码器和奖励归一化器，使模型能够作为神经模拟器在不同形态间泛化。

Result: 首次实现了能够零样本泛化到新形态的四足机器人世界模型。QWM能够在四足机器人形态家族内作为分布有界的插值器运行，解锁了跨不同体现的零样本控制能力。

Conclusion: 这项工作代表了向形态条件化世界模型的重要一步，虽然QWM是四足机器人形态家族内的分布有界插值器而非通用物理引擎，但为腿部运动的形态条件化世界模型开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决异构硬件平台之间协作机械臂运动规划的可移植性和通用性问题，作者基于之前开发的WinGs Operating Studio中间件，专门设计了一个最小化但灵活的实时末端执行器轨迹控制接口。

Method: 采用n次多项式插值器结合二次规划求解器，生成具有精确位置、速度和加速度轮廓的平滑、连续可微轨迹。该接口作为WOS中间件的一部分，提供语言无关的API，将不同的机器人组件抽象为统一的软件资源。

Result: 在三种不同场景中验证了方法的有效性：1）离线演示中机械臂准确绘制几何图形；2）可中断的低频重新规划场景中机械臂抓取移动机器人上的动态物体；3）遥操作实验中一个机械臂控制另一个机械臂完成灵巧操作。

Conclusion: 提出的实时低级运动规划接口具有高可靠性、多功能性和易用性，能够有效提升协作机械臂在不同硬件平台上的可移植性和应用范围，为异构机器人系统的集成提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 运动假肢的设计和选择通常依赖试错法，缺乏考虑人体运动与假肢变形相互作用的全身动力学分析。本研究旨在开发一种能够提供用户特定设计和选择见解的仿真方法。

Method: 采用混合连杆系统，结合分段恒定应变模型表示假肢的柔性变形。基于强化学习的方法，从运动捕捉数据中进行模仿学习，并结合精确的假肢动力学计算，生成单侧胫骨截肢者的全身动态运动。

Result: 在不同虚拟假肢刚度条件下模拟跑步运动，分析获得的代谢运输成本。结果表明刚度变化会影响跑步性能，证明了该方法在虚拟条件下进行仿真和分析的潜力。
...
TL;DR: HTNav是一个用于空中视觉语言导航的新型协作导航框架，通过整合模仿学习和强化学习，采用分层决策机制和地图表示学习模块，在复杂城市环境中显著提升了导航精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有空中视觉语言导航方法在复杂城市环境中面临三个主要挑战：对未见场景的泛化能力不足、长距离路径规划性能欠佳、以及对空间连续性的理解不够充分。

Method: 提出HTNav框架，整合模仿学习和强化学习，采用分阶段训练机制确保基础导航策略稳定性，同时增强环境探索能力；引入分层决策机制实现宏观路径规划与细粒度动作控制的协作交互；加入地图表示学习模块加深对开放领域空间连续性的理解。

Result: 在CityNav基准测试中，该方法在所有场景级别和任务难度上都达到了最先进的性能，实验结果表明该框架显著提高了复杂城市环境中的导航精度和鲁棒性。

Conclusion: HTNav框架通过整合IL和RL、分层决策机制和地图表示学习，有效解决了空中视觉语言导航在复杂城市环境中的关键挑战，为物流配送和城市巡检等应用提供了实用的解决方案。
...
TL;DR: AssemLM是一个专门为机器人装配任务设计的空间多模态大语言模型，通过整合装配手册、点云和文本指令来推理和预测6D装配姿态，在AssemBench数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要依赖粗粒度的2D感知，缺乏对3D几何的精确推理能力，这对于机器人精细装配任务至关重要。需要开发能够进行准确3D空间推理的模型来支持机器人装配应用。

Method: 提出AssemLM模型，采用专门的点云编码器捕捉细粒度几何和旋转特征，然后将这些特征整合到多模态语言模型中，支持准确的3D空间推理。同时构建了包含90万+多模态样本的AssemBench数据集和基准。

Result: AssemLM在各种装配场景的6D姿态推理中实现了最先进的性能。真实机器人评估表明该模型能够支持现实世界中的细粒度和多步骤装配执行。

Conclusion: AssemLM通过整合3D几何理解能力，填补了现有具身AI基准的空白，展示了在机器人装配应用中的潜力，为精细操作任务提供了有效的空间推理解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型需要大量语义连贯且物理可行的数据集，但现有场景生成方法缺乏上下文感知能力，难以合成具有丰富语义信息的高保真环境，经常导致目标位置不可达，使任务过早失败。

Method: 提出V-CAGE框架：1）引入修复引导的场景构建，系统安排上下文感知的布局；2）集成功能元数据与基于视觉语言模型的闭环验证机制，作为视觉批评家过滤错误；3）实现感知驱动的压缩算法，减少90%以上文件大小。

Result: V-CAGE自动化端到端流程，能够高度可扩展地合成多样化、高质量的机器人操作数据集，同时保持下游VLA训练效果不受影响。

Conclusion: 通过集中语义布局规划和视觉自我验证，V-CAGE解决了现有数据合成方法的局限性，为大规模VLA模型训练提供了高效、可靠的数据生成解决方案。
...
TL;DR: 本文提出了一种用于空中视觉地点识别的异构内存框架，通过分离静态卫星锚点和动态经验回放缓冲区来解决持续学习中的灾难性遗忘问题，在21个任务序列的基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 空中视觉地点识别在变化环境条件下的鲁棒性对于长期空中自主性至关重要。现有持续学习方法在处理地理特征严重类内变化时往往失败，需要一种能适应严格机载存储限制的解决方案。

Method: 提出异构内存框架，将地理知识解耦为静态卫星锚点（保留全局几何先验）和动态经验回放缓冲区（保留领域特定特征）。采用空间约束分配策略，基于样本难度或特征空间多样性优化缓冲区选择。

Result: 实验表明该架构显著提升了空间泛化能力；多样性驱动的缓冲区选择比随机基线在知识保留上高出7.8%。最大化结构多样性实现了更好的可塑性-稳定性平衡，并在随机序列中保持顺序无关的鲁棒性。

Conclusion: 在终身空中自主系统中，保持结构特征覆盖比样本难度对于解决灾难性遗忘更为关键。提出的框架通过解耦地理知识和优化缓冲区选择，有效平衡了持续学习中的稳定性和可塑性。
...
TL;DR: 提出并联机器人冗余驱动的一致术语分类，基于构型空间模型区分运动学冗余与驱动冗余，强调输入奇异性可通过冗余驱动避免


<details>
  <summary>Details</summary>
Motivation: 并联机器人(PKM)存在运动学冗余和驱动冗余，但文献中对驱动冗余的分类不一致，需要建立统一的术语体系来清晰区分不同类型的冗余

Method: 引入以构型空间为中心的动力学模型，区分构型空间、输出和输入奇异性；提出非线性控制系统，引入驱动度(DOA)概念，根据DOA与自由度(DOF)的关系分类驱动冗余

Result: 建立了并联机器人冗余驱动的一致术语分类框架，从几何角度证明冗余驱动可避免输入奇异性，将PKM分为全驱动和欠驱动两类

Conclusion: 提出的构型空间模型和驱动度概念为并联机器人冗余驱动提供了统一的分类标准，有助于更清晰地理解和设计冗余驱动系统
...
TL;DR: 该研究开发了用于微型飞行机器人栖息的软可拉伸电粘附垫，通过实验比较了法向和剪切粘附力，并在Crazyflie四旋翼上成功演示了在平滑表面的可重复附着与快速分离。


<details>
  <summary>Details</summary>
Motivation: 电粘附技术能为微型飞行机器人提供电控可切换的粘附力，使其能在平滑表面栖息。然而，适用于空中栖息的软可拉伸电粘附垫的实际应用仍然有限。

Method: 1) 开发了制造正弦波和同心圆电极软可拉伸电粘附垫的高效工作流程；2) 使用Instron测试装置在0kV和4.8kV条件下对比法向和剪切粘附力；3) 在Crazyflie四旋翼上配备电粘附脚，在平面和曲面基底上进行栖息演示。

Result: 剪切粘附力占主导地位，在部分垫接触情况下可达约3N；法向粘附力相对较小且强烈依赖基底特性。Crazyflie原型机在平滑塑料表面（包括曲面）上展示了可重复的附着，以及在电压移除时的快速分离。

Conclusion: 软电粘附脚在微型飞行器轻量化和可靠栖息方面具有巨大潜力，剪切粘附力是实现有效栖息的关键机制。
...
TL;DR: 该论文分析了激光雷达在道路环境中可能产生概率性返回的情况，通过分析数据集展示了两种机械旋转激光雷达单元的代表性累积分布函数，并提出了评估概率性多返回情况对激光雷达定位影响的方法论。


<details>
  <summary>Details</summary>
Motivation: 激光雷达在自动驾驶车辆定位中应用日益广泛，但对其在道路环境中的性能理解不足。现有定位算法通常假设激光雷达沿给定射线路径总能识别唯一表面，然而实际中当多个散射表面出现在激光雷达锥形光束内时，可能产生概率性测量结果。

Method: 分析激光雷达数据集以识别特定射线路径上的概率性返回情况，为两种不同机械旋转激光雷达单元（基座固定）提供代表性累积分布函数，并建立定性方法评估概率性多返回对激光雷达定位的影响。

Result: 展示了两种机械旋转激光雷达单元在特定射线路径上的代表性累积分布函数，证实了激光雷达确实存在概率性返回现象，特别是在多个散射表面出现在光束内的情况下。

Conclusion: 激光雷达在道路环境中确实存在概率性返回现象，这对激光雷达定位算法的准确性有重要影响。提出的定性评估方法有助于更好地理解和处理这种概率性测量问题，提高自动驾驶车辆的定位可靠性。
...
TL;DR: POMDAR是一个全面的灵巧性基准测试，通过结构化的操作和抓取任务来形式化定义灵巧性，为仿人机器人手提供标准化评估框架。


<details>
  <summary>Details</summary>
Motivation: 当前仿人机器人手设计和评估中"灵巧性"概念定义模糊且使用不一致，不同系统在不同标准下评估，难以进行有意义的比较，需要基于可测量结果的统一性能定义。

Method: 从人类运动控制的分类学系统推导出结构化操作和抓取任务集，实现真实世界和仿真环境，包含四种操作配置（垂直、水平、连续旋转、纯抓取），通过机械支架约束任务运动、抑制补偿策略，定义结合任务正确性和执行速度的定量评分指标。

Result: 开发了POMDAR基准测试，提供开源、标准化、基于分类学的评估框架，能够客观、可重复、可解释地评估不同手部设计，测量灵巧性作为吞吐量。

Conclusion: POMDAR为仿人机器人手提供了一致的比较和评估基准，促进了灵巧操作平台的系统性进步，所有CAD、仿真文件和评估视频均已公开。
...
TL;DR: 提出在线意图预测框架，用于实时估计自主系统的目标状态，即使意图随时间变化，且系统动力学或目标包含未知参数。
...
Method: 将问题建模为逆最优控制/逆强化学习任务，将意图视为目标函数中的参数；采用移动时域策略来折扣过时信息，通过在线控制知情学习实现高效梯度计算和参数更新。

Result: 在不同噪声水平下的仿真和四旋翼无人机硬件实验中，该方法在复杂环境中实现了准确、自适应的意图预测。

Conclusion: 提出的在线意图预测框架能够有效处理时间变化的意图和未知系统参数，为自主系统在动态环境中的决策提供了可靠支持。
...
TL;DR: 提出了一种鲁棒自适应反步阻抗控制策略，用于在接触丰富和不确定环境中操作的机器人，该策略考虑了系统的完整耦合动力学，并明确处理了外部干扰和未建模动态等关键不确定性源。


<details>
  <summary>Details</summary>
Motivation: 机器人操作在接触丰富和不确定环境中面临挑战，需要能够处理系统耦合动力学、外部干扰和未建模动态的鲁棒控制策略，同时不依赖机器人动态参数。

Method: 采用基于反步法的自适应阻抗控制方案作为内环来跟踪参考阻抗模型；使用基于泰勒级数的系统动力学估计器和自适应估计器处理外部力上界；通过稳定性分析证明系统的半全局实用有限时间稳定性。

Result: 在模拟移动机械臂场景和真实Franka Emika Panda机器人上进行了实验评估，相比PD控制表现出更安全的性能，同时确保轨迹跟踪和力监测。
...
TL;DR: MADRI框架通过将视频流转换为语义特征向量，并结合机器人内部传感器数据和场景图，实现多模态重建异常检测，提高人机协作中的异常检测性能。
...
Method: 提出MADRI框架：1) 将视频流转换为语义特征向量；2) 结合机器人内部传感器读数；3) 加入场景图；4) 基于多模态特征重建进行异常检测。

Result: 实验结果表明：仅基于视觉特征向量的重建就能有效检测异常，而结合其他模态能进一步提高检测性能，突显多模态特征重建在人机协作异常检测中的优势。

Conclusion: 多模态特征重建方法能够同时捕捉视觉环境中的外部异常和机器人内部的故障，为人机协作提供更鲁棒的异常检测框架。
...
TL;DR: VAG是一个基于流匹配的双流框架，联合生成视频和动作，解决现有世界模型在策略学习中缺乏配对动作轨迹的问题。


<details>
  <summary>Details</summary>
Motivation: 机器人基础模型需要大量人类遥操作数据，但收集任务特定演示成本高。现有世界模型不适合策略学习，因为它们不提供配对的动作轨迹；世界-动作模型虽然预测动作，但视频-动作对齐性差；两阶段管道效率低且误差累积。
...
Conclusion: VAG作为实用的世界-动作模型，具有为具身数据合成提供可行解决方案的潜力。
...
TL;DR: 提出基于生理可信肌肉骨骼模拟和强化学习的设备无关框架，用于为健全和临床人群生成可扩展的个性化外骨骼辅助控制策略，无需大量物理试验。


<details>
  <summary>Details</summary>
Motivation: 传统下肢外骨骼控制策略设计需要大量数据收集或迭代优化，限制了临床人群的可及性，需要一种更高效、可扩展的方法。

Method: 结合生理可信的肌肉骨骼模拟与强化学习，构建设备无关框架，生成外骨骼控制策略，能够模拟健康步态和病理步态的补偿策略。

Result: 控制策略生成的外骨骼辅助扭矩与人类实验中验证的最先进配置文件一致，能降低不同步行速度下的代谢成本；对于模拟的步态障碍模型，能提供非对称、缺陷特定的辅助，改善能量效率和双侧运动对称性。

Conclusion: 生理可信的肌肉骨骼模拟结合强化学习可作为可扩展的个性化外骨骼控制基础，适用于健全和临床人群，无需大量物理试验。
...
TL;DR: Adaptor是一个少样本框架，通过两阶段处理解决辅助遥操作中因操作者习惯差异导致的意图识别不稳定问题，实现了跨操作者的鲁棒泛化。


<details>
  <summary>Details</summary>
Motivation: 辅助遥操作通过共享控制提高效率，但不同操作者的习惯和专业知识差异导致高度异质的轨迹分布，这削弱了意图识别的稳定性。

Method: Adaptor采用两阶段方法：1)预处理阶段通过噪声注入合成轨迹扰动来建模意图不确定性，并进行几何感知的关键帧提取；2)策略学习阶段使用意图专家编码处理后的轨迹，并将其与预训练视觉语言模型上下文融合，以条件化动作专家生成动作。

Result: 在真实世界和模拟基准测试中，Adaptor实现了最先进的性能，相比基线方法提高了成功率和效率。此外，该方法在不同专业水平的操作者间表现出低方差，展示了强大的跨操作者泛化能力。

Conclusion: Adaptor框架通过有效建模意图不确定性和融合多模态信息，能够稳定地识别跨操作者的意图，为辅助遥操作系统提供了鲁棒的少样本解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的学习型四足机器人控制器虽然表现出色，但在模型不确定性、感知噪声和非结构化接触条件下缺乏形式化的安全保证，需要一种能够统一安全性和性能的框架

Method: 通过可微分的随机安全控制框架，结合概率控制屏障函数、语义上下文编码和元自适应风险校准，使用方差感知屏障约束嵌入可微分二次规划，保持端到端训练的梯度流

Result: 在Unitree A1和ANYmal C机器人上以200Hz部署，在12种地形类型、动态障碍、形态扰动和语义定义任务中验证，相比现有方法减少3-10倍安全违规，降低10-15%能耗

Conclusion: SafeMind为学习型四足控制器提供了形式化的安全保证，在保持实时控制性能的同时显著提高了安全性和能效，为机器人安全控制提供了统一的概率框架
