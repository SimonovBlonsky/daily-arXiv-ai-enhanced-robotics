...
TL;DR: OptimusVLA是一个具有全局先验记忆和局部一致性记忆的双记忆VLA框架，通过任务级先验替代高斯噪声和动态建模历史动作序列，解决了现有VLA模型推理效率低和鲁棒性差的问题。
...
Method: 提出OptimusVLA双记忆框架，包含全局先验记忆（GPM）和局部一致性记忆（LCM）。GPM从语义相似的轨迹中检索任务级先验替代高斯噪声，缩短生成路径并减少函数评估次数。LCM动态建模已执行的动作序列来推断任务进度，并注入学习到的一致性约束，确保轨迹的时间一致性和平滑性。

Result: 在三个仿真基准测试中表现优异：LIBERO上达到98.6%平均成功率，CALVIN上比π_0提升13.5%，RoboTwin 2.0 Hard上达到38%平均成功率。在真实世界评估中，在泛化和长视野任务上表现最佳，分别超越π_0 42.9%和52.4%，同时实现2.9倍推理加速。

Conclusion: OptimusVLA通过引入双记忆机制有效解决了VLA模型在动作生成过程中的效率和鲁棒性问题，在仿真和真实世界任务中均展现出显著性能提升和推理加速，为机器人操作任务提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在自主导管操控中存在奖励稀疏、依赖静态血管模型的问题，导致样本效率低且难以适应术中变化，需要开发更高效的框架来应对血管分叉等复杂解剖结构。

Method: 提出三组件框架：1)基于分割的姿态估计模块提供实时状态反馈；2)模糊控制器进行分叉感知的方向调整；3)结合专家先验的结构化奖励生成器。通过在线专家校正减少探索低效性。

Result: 在透明血管模型上的机器人平台验证显示，该方法仅需123个训练轮次即可收敛（比SAC基线减少25.9%），平均位置误差降低至基线的83.8%，在复杂分叉场景中表现优异。

Conclusion: 结合样本高效强化学习与在线专家校正能够实现可靠准确的导管操控，特别是在对血管内导航至关重要的解剖学挑战性分叉场景中，为机器人辅助血管内干预提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 准确理解人类意图是人机交互的核心挑战，也是实现更自然、直观的人机协作的关键要求。需要开发能够精确解释和执行人类指令的系统。

Method: 采用多模态HRI框架，结合Florence-2进行物体检测，Llama 3.1进行自然语言理解，Whisper进行语音识别，并使用模糊逻辑实现自适应控制。系统同时处理场景感知和动作规划。
...
TL;DR: 该研究通过100次真实机器人训练实验，系统分析了在线强化学习在物理机器人上的成功设计选择，发现了一些常用默认设置的危害，并提出了一套稳定可靠的设计方案。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究在物理机器人上成功实施在线强化学习的具体设计选择，因为现有研究通常对这些选择保持隐式，缺乏系统性的实证分析。

Method: 在三种不同的机器人平台上进行了100次真实世界训练实验，系统性地消融分析了算法、系统和实验决策，这些决策在先前工作中通常被隐式处理。

Result: 研究发现一些广泛使用的默认设置可能有害，而一套在标准RL实践中稳健且易于采用的设计选择能够在不同任务和硬件上实现稳定学习。

Conclusion: 这是首次对此类设计选择进行大规模实证研究，使从业者能够以更低的工程努力部署在线强化学习，为机器人RL实践提供了实用指导。
...
TL;DR: FACTO是一种新的轨迹优化算法，使用正交基函数的线性组合参数化轨迹表示，直接在系数空间进行优化，采用高斯-牛顿近似和指数移动平均处理非线性，在主动约束的零空间执行自适应约束更新。


<details>
  <summary>Details</summary>
Motivation: 针对单臂和多臂机械臂的轨迹优化问题，需要处理非线性约束并提高解的质量和可行性，特别是在受约束的场景中。

Method: 使用正交基函数线性组合参数化轨迹，在系数空间直接优化；采用高斯-牛顿近似和指数移动平均处理非线性；使用系数空间映射处理轨迹范围约束；在主动约束的零空间使用Levenberg-Marquardt算法执行自适应约束更新。

Result: 与基于优化的规划器（CHOMP、TrajOpt、GPMP2）和基于采样的规划器（RRT-Connect、RRT*、PRM）相比，FACTO在解的质量和可行性方面表现更好，特别是在受约束的单臂和多臂场景中。

Conclusion: FACTO算法在Franka机器人上的实验验证了其部署可行性，为受约束的机械臂轨迹优化问题提供了有效的解决方案。
...
TL;DR: UniLACT是一个基于Transformer的视觉-语言-动作模型，通过深度感知的潜在预训练融入几何结构，解决了仅从RGB视频学习的潜在动作缺乏3D几何信息的问题，在模拟和真实环境中都优于RGB基线。
...
Method: 提出UniLACT模型，通过深度感知潜在预训练融入几何结构；开发UniLARN框架，基于逆动态和正向动态目标学习RGB和深度的共享嵌入空间，并显式建模跨模态交互，为UniLACT提供伪标签。

Result: 在模拟和真实环境中的广泛实验表明，深度感知的统一潜在动作表示有效。UniLACT在域内和域外预训练机制下，以及在已见和未见操作任务上都优于RGB基线。

Conclusion: 通过深度感知的潜在预训练融入几何结构可以显著提升视觉-语言-动作模型的性能，特别是在需要精确空间感知的操作任务中。
...
TL;DR: 提出了一种用于刚体动力学接触模拟的快速、可向量化且光滑可微的框架，解决了现有可微模拟框架中接触流形生成的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有机器人模拟器的接触检测例程在设计时未充分考虑向量化和可微性，导致在可微模拟中成为性能瓶颈。需要一种既能保持高效性又能实现可微性的接触检测方法。

Method: 提出了一个全新设计的框架，采用：1）一组光滑解析有符号距离基元来实现顶点-面碰撞检测；2）新颖的可微边-边碰撞检测例程，能够提供有符号距离和有符号接触法线。

Result: 通过教学实验进行评估，并与成熟的Mujoco XLA框架的碰撞检测例程进行基准测试，观察到显著的加速效果。

Conclusion: 该框架在机器人刚体动力学接触模拟中实现了快速、可向量化和光滑可微的目标，填补了现有方法在效率和可微性之间的空白。
...
<details>
  <summary>Details</summary>
Motivation: 可靠的物体操作需要理解随物体和环境变化的物理属性。现有的视觉语言模型规划器虽然能在一般意义上推理摩擦和稳定性，但无法预测特定物体在具体表面上的行为，缺乏直接经验时表现不佳。

Method: PhysMem框架通过记录交互经验、生成候选假设，并通过有针对性的交互验证这些假设，然后将验证后的知识提升为指导未来决策。核心设计原则是"先验证后应用"，系统会测试假设对新观察的适应性，而不是直接应用检索到的经验。

Result: 在三个真实世界操作任务和四个VLM骨干的模拟基准测试中，PhysMem表现出色。在受控的砖块插入任务中，基于原则的抽象方法达到76%成功率，而直接经验检索只有23%。真实世界实验在30分钟部署会话中显示出持续改进。

Conclusion: PhysMem框架通过记忆机制使VLM机器人规划器能够在测试时学习物理原理，验证后应用的方法减少了物理条件变化时对先前经验的僵化依赖，显著提升了物理推理和操作性能。
...
TL;DR: 该研究扩展了机器人碰撞安全数据集，包含剪切接触场景，建立了首个基于能量的伤害保护数据库，为开发能量限制控制器提供基础。


<details>
  <summary>Details</summary>
Motivation: 当前机器人碰撞安全研究主要依赖EN ISO 10218-2:2025的钝器冲击数据，缺乏针对边缘或尖锐碰撞的可扩展临床数据集。在受限环境和未来人形机器人系统中，接触不可避免，需要更全面的安全验证数据。

Method: 研究扩展了先前基于猪替代物的数据集，包括无约束碰撞中的剪切接触场景。重新评估所有先前猪替代物数据，分析碰撞角度对伤害结果的影响，建立跨几何形状和接触类型的能量阈值。

Result: 研究发现碰撞角度显著影响伤害结果，无约束剪切接触比垂直碰撞导致更少的伤害。建立了首个基于能量的伤害保护数据库，涵盖不同几何形状和接触类型。

Conclusion: 该研究为开发有意义的能量限制控制器提供了基础，确保在各种现实碰撞事件中的安全性，填补了机器人安全验证数据的空白。
...
TL;DR: 该论文提出了一种统一的多任务强化学习框架，通过将参考运动作为行为塑造的先验而非部署时约束，解决了参考跟踪策略的脆弱性和纯任务驱动RL运动质量差之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临一个持续的权衡：参考跟踪策略在演示数据集之外往往很脆弱，而纯任务驱动的强化学习虽然能实现适应性，但牺牲了运动质量。需要一种能同时获得自然运动质量和适应性的方法。

Method: 提出统一的多任务RL框架，训练单一目标条件策略同时处理两个任务：1)参考引导的模仿任务，参考轨迹定义密集模仿奖励但不作为策略输入；2)目标条件泛化任务，目标独立于参考采样，奖励仅反映任务成功。通过共享观察和动作空间但不同初始化方案、命令空间和奖励结构的协同优化。

Result: 在具有挑战性的基于箱子的跑酷环境中评估，该方法学习到的控制器能够超越参考分布进行迁移，同时保持运动的自然性。通过组合多个学习到的技能展示了长时程行为生成能力。

Conclusion: 该方法通过将参考运动作为行为塑造先验而非部署约束，实现了从密集参考监督中获取结构化、类人运动技能，同时学习将这些技能适应到新目标和初始条件，无需对抗目标、显式轨迹跟踪、相位变量或参考依赖推理。
...
Method: 结合物理抓取优化和强化学习抓取适应，通过人类演示信息合成鲁棒抓取配置，采用自适应控制器发出关节修正防止手内滑动

Result: 在五种动态工具使用任务中实现零样本仿真到现实迁移，减少平移和旋转滑动，达到最高任务完成率

Conclusion: Grasp-to-Act方法在动态、接触丰富的条件下实现稳定功能抓握，优于现有基线方法
...
TL;DR: 提出一个策略驱动的框架，将高层视觉语言推理与低层闭环控制相结合，用于自主腹腔镜相机控制，在保持手术视野稳定安全的同时提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 自主腹腔镜相机控制需要在快速工具-组织交互下保持稳定安全的手术视野，同时保持对外科医生的可解释性。现有方法往往缺乏结构化策略指导，难以平衡实时响应与操作可理解性。

Method: 采用策略驱动框架：离线阶段将原始手术视频解析为相机相关时间事件并构建属性事件图，从中挖掘可重用相机操作策略原语；在线阶段使用微调视觉语言模型处理实时腹腔镜视图，预测主导策略和离散图像运动指令，由IBVS-RCM控制器在严格安全约束下执行，支持语音输入实现人机协同。

Result: 事件解析达到可靠时间定位（F1分数0.86），挖掘策略与专家解释语义对齐度高（聚类纯度0.81）。离体实验显示系统在标准化相机操作评估中优于初级外科医生，视野中心误差减少35.26%，图像抖动减少62.33%，同时保持平滑运动和稳定工作距离调节。

Conclusion: 该策略驱动框架成功将高层语义理解与低层控制相结合，实现了稳定、安全且可解释的自主腹腔镜相机控制，为手术机器人智能化提供了新思路。
...
TL;DR: BFA++是一个专门为视觉-语言-动作模型设计的动态令牌剪枝框架，通过分层剪枝策略提高计算效率，同时保持操作成功率


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型加速技术（如令牌剪枝）直接应用于VLA模型时会导致性能下降，因为它们忽略了不同视图之间的关系，也没有考虑到机器人操作的动态和任务特定特性

Method: 提出BFA++框架，采用分层剪枝策略，包含两个层次的重要性预测器：视图内预测器突出显示每张图像中任务相关区域以抑制空间噪声，视图间预测器识别不同操作阶段的关键相机视图以减少跨视图冗余

Result: 在RoboTwin基准测试和真实世界机器人任务中，BFA++始终优于现有方法，在π0和RDT模型上分别提高约10%的成功率，实现1.8倍和1.5倍的加速

Conclusion: 上下文敏感和任务感知的令牌剪枝比完整视觉处理更有效，能够在真实世界机器人系统中实现更快推理和更高操作精度
...
TL;DR: 提出低成本声学反馈机器人磨削系统(AFRG)，用接触式麦克风采集音频信号实时估计磨削力，实现闭环力控制，相比传统力传感器成本降低200倍，在不同磨削盘条件下一致性提高4倍。


<details>
  <summary>Details</summary>
Motivation: 人类进行磨削任务时依赖声学反馈判断工具与工件接触状态，而机器人磨削系统主要依赖昂贵的力传感器，难以适应不同磨削工具。音频传感器成本低廉且可安装在任何传导磨削声音的介质上。

Method: 开发AFRG系统，使用接触式麦克风采集音频信号，从音频中实时估计磨削力，实现磨削过程的闭环力控制。系统仅依赖低成本麦克风作为传感模态。

Result: 相比传统力传感方法，AFRG在不同磨削盘条件下实现4倍的一致性改进。系统使用的麦克风成本约为传统力传感器的1/200，提供易于部署、经济高效的机器人磨削解决方案。

Conclusion: AFRG系统通过声学反馈实现了低成本、高性能的机器人磨削控制，为工业应用提供了经济有效的替代方案，克服了传统力传感器成本高、适应性差的问题。
...
<details>
  <summary>Details</summary>
Motivation: 为了在家庭环境中实现快速安全的操作任务，需要开发一个能够在运动中识别周围环境、定位目标物体，并据此规划和执行动作的系统。

Method: 提出周期性采样全身轨迹规划方法RLP，利用家庭环境的独特特征提高计算效率、运动最优性和鲁棒性。通过最小化路径点规划、周期性执行轨迹规划选择更优动作，以及采用对基座位置误差鲁棒的逆运动学。

Result: 评估实验表明RLP在运动规划时间、运动持续时间和鲁棒性方面优于现有方法。在整理任务的应用实验中实现了高成功率和短操作时间，验证了其实际可行性。

Conclusion: RLP方法在家庭环境中表现出色，能够实现快速安全的操作任务，具有实际应用价值。
...
TL;DR: IG-RFT是一个用于视觉-语言-动作模型的交互引导强化微调系统，通过IG-AWR算法和混合密集奖励函数解决VLA模型在现实世界长时程任务中的泛化问题。


<details>
  <summary>Details</summary>
Motivation: VLA模型在现实世界长时程复杂任务中泛化能力有限，主要由于分布偏移和高质量演示数据稀缺。强化学习虽然提供了改进途径，但在现实世界VLA微调中面临探索效率、训练稳定性和样本成本等挑战。

Method: 提出IG-RFT系统，包含三个核心组件：1) IG-AWR算法，基于机器人交互状态动态调节探索强度；2) 混合密集奖励函数，整合轨迹级和子任务级奖励；3) 三阶段RL系统（SFT、离线RL、人在回路RL）用于VLA模型微调。

Result: 在四个具有挑战性的长时程任务上进行真实世界实验，IG-RFT平均成功率85.0%，显著优于SFT（18.8%）和标准离线RL基线（40.0%）。消融研究证实了IG-AWR和混合奖励塑形的关键贡献。

Conclusion: 该工作建立并验证了一种新颖的VLA模型强化微调系统，能够有效提升视觉-语言-动作模型在现实世界机器人操作任务中的性能。
...
TL;DR: 本文提出了一种用于开放路径激光吸收光谱测量的机器人系统，使用地面云台传感器和携带反射器的无人机，实现了60米距离内的自主跟踪和有效CO2测量。


<details>
  <summary>Details</summary>
Motivation: 开放路径可调谐二极管激光吸收光谱技术能够有效测量、绘制和监测气体浓度，但需要专用反射表面，这使得空间采样过程的自动化具有挑战性。

Method: 开发了一个机器人系统，包括安装在地面云台上的传感器和携带反射器的小型无人机。地面单元通过变焦摄像头跟踪无人机上的红色LED标记，并将传感器激光束与反射器对齐，同时结合无人机飞行控制器提供的GNSS位置信息改进跟踪方法。

Result: 室外实验验证了系统性能，在距离达60米的情况下成功实现了自主跟踪和有效的CO2测量。系统成功测量了CO2羽流，且未受无人机推进系统的干扰，相比飞行原位传感器具有优势。

Conclusion: 该系统为开放路径激光吸收光谱测量提供了一种有效的自动化解决方案，能够在大范围室外环境中进行非侵入式气体浓度监测，克服了传统方法对固定反射表面的依赖。
...
TL;DR: 提出KCFRC算法，用于高效验证足式机器人落脚点的可达性，能在2ms内完成900个潜在落脚点的可达性检查，提升机器人在复杂环境中的适应性。


<details>
  <summary>Details</summary>
Motivation: 足式机器人在复杂环境中导航面临挑战，现有方法主要基于地形几何或运动学选择落脚点，但缺乏高效验证无碰撞摆动轨迹存在性的方法。

Method: 首先形式化定义落脚点可达性问题，建立落脚点可达性的充分条件，基于此开发KCFRC算法，实现实时验证落脚点可达性。

Result: KCFRC算法时间效率显著，单腿在900个潜在落脚点上平均仅需2ms完成可达性检查，能加速轨迹优化，特别有利于受限空间中的接触规划。

Conclusion: KCFRC算法填补了现有方法的空白，提升了足式机器人在挑战性环境中的适应性和鲁棒性，为实时落脚点选择提供了有效解决方案。
...
TL;DR: GeCo-SRT提出了一种几何感知的持续适应方法，通过积累跨任务的知识来高效实现模拟到现实的迁移，相比基线平均性能提升52%，新任务适应仅需1/6数据。


<details>
  <summary>Details</summary>
Motivation: 现有模拟到现实迁移方法将每次迁移视为孤立任务，需要重复调优且浪费先前经验。需要建立持续跨任务的迁移范式，通过知识积累实现高效适应新任务。

Method: 提出GeCo-SRT方法：1）几何感知专家混合模块动态激活专家专门处理不同几何知识；2）几何专家引导的优先经验回放模块优先采样未充分利用的专家，防止遗忘并保持跨任务性能。

Result: 相比基线平均性能提升52%，新任务适应仅需1/6数据，显著提高了数据效率。

Conclusion: 通过几何特征的领域不变和任务不变知识作为可迁移基础，GeCo-SRT实现了高效低成本的跨任务模拟到现实迁移，为持续适应方法提供了新思路。
...
TL;DR: 该论文提出了一种结合人类抓取偏好和强化学习的任务导向抓取方法，使人形机器人能够根据下游任务约束进行多物体抓取。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏端到端的解决方案，无法在抓取多个物体时考虑下游任务的约束。需要使人形机器人的抓取行为符合人类社交规范并满足任务特定目标。

Method: 1. 从ContactPose数据集中提取人类抓取偏好；2. 基于变分自编码器(VAE)训练手部协同模型来模仿人类抓取动作；3. 使用强化学习训练能够考虑任务特定抓取后意图的智能体。

Result: 开发出能够根据任务特定意图抓取多个物体的智能体，结合了人类抓取行为的数据驱动洞察和强化学习的探索学习能力。

Conclusion: 通过将人类抓取行为数据与强化学习相结合，可以开发出具有情境感知操作能力的人形机器人，促进在人类中心环境中的协作。
...
<details>
  <summary>Details</summary>
Motivation: 为单自由度有理连杆机构设计提供几何方法，实现预定空间任务的有理运动生成

Method: 基于三次四元数贝塞尔曲线建立七点3D插值方案，支持运动分解和空间六杆机构合成

Result: 开发了开源CAD工具，实现运动生成和机构合成的快速可视化评估

Conclusion: 提出的几何方法为有理连杆机构设计提供了有效的运动生成和机构合成解决方案
...
TL;DR: ParkDiffusion++：用于自动泊车的联合多模态自我意图预测和多智能体轨迹预测框架，通过意图标记化、条件预测、安全引导去噪和反事实知识蒸馏提升性能。


<details>
  <summary>Details</summary>
Motivation: 自动泊车是高级驾驶辅助系统的挑战性领域，需要强大的场景理解和交互推理。现有方法通常将相互依赖的自我意图预测和周围智能体响应预测问题孤立处理，无法有效支持"如果...会怎样"的决策制定。

Method: 1. 自我意图标记器：从智能体历史和矢量化地图折线预测离散的端点意图集合；2. 自我意图条件联合预测：为每个可能的自我意图生成周围智能体的社会一致性预测；3. 轻量级安全引导去噪器：在训练期间使用不同约束优化联合场景；4. 反事实知识蒸馏：通过冻结的安全引导去噪器精炼的EMA教师模型提供伪目标，捕捉智能体对替代自我意图的反应。

Result: 在Dragon Lake Parking (DLP)数据集和Intersections Drone (inD)数据集上实现了最先进的性能。定性分析显示，其他智能体对不同自我意图做出了适当的反应。

Conclusion: ParkDiffusion++通过联合学习多模态自我意图预测和自我条件多智能体联合轨迹预测，有效解决了自动泊车中的场景理解和交互推理挑战，支持有效的"如果...会怎样"决策制定。
...
TL;DR: LST-SLAM是一个针对大规模动态户外环境的大规模立体热成像SLAM系统，通过自监督热特征学习、立体双级运动跟踪和几何位姿优化，在复杂动态场景中实现鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 热成像相机在挑战性光照和天气条件下具有强大的机器人感知潜力，但热成像SLAM在动态大规模户外环境中面临特征提取不可靠、运动跟踪不稳定、全局位姿和地图构建不一致等困难。

Method: 结合自监督热特征学习、立体双级运动跟踪和几何位姿优化；引入语义-几何混合约束抑制缺乏强帧间几何一致性的潜在动态特征；开发在线增量词袋模型用于闭环检测，配合全局位姿优化减少累积漂移。

Result: 在千米级动态热成像数据集上的广泛实验表明，LST-SLAM在鲁棒性和准确性方面显著优于最近的代表性SLAM系统，包括AirSLAM和DROID-SLAM。

Conclusion: LST-SLAM系统成功解决了热成像SLAM在动态大规模户外环境中的关键挑战，通过创新的特征学习、运动跟踪和优化方法，实现了鲁棒且准确的热成像SLAM性能。
...
TL;DR: 该论文提出了一种用于无人机搜救任务的视觉系统，通过融合深度相机测量和单目相机到人体距离估计，实现对人体目标的鲁棒跟踪和跟随，保持安全距离。
...
Method: 使用YOLO-pose进行深度相机数据的深度学习滤波和单目相机到人体距离估计，通过扩展卡尔曼滤波算法实时融合深度信息，估计深度相机与人体关键点之间的距离。
...
TL;DR: 本文提出了一种用于优化线性介电弹性体致动器寿命的自动化测试机器人系统，通过参数扫描找到最佳组合，将工作寿命提高100%，并成功应用于四足机器人
...
Method: 开发了集成机电性能测量、可编程电压输入和多通道测试能力的测试机器人，扫描了Elastosil基线性致动器的电压幅值、频率、电极材料浓度和电连接填料等参数

Result: 在边界操作条件下，最佳参数组合将工作寿命提高了100%，最终产品在模块化四足行走机器人上表现出良好的承载能力（超过无绳体重的100%，超过致动器总重的700%）

Conclusion: 这是首个将自驱动实验室方法引入机器人致动器设计的工作，为优化DEA性能提供了系统化的解决方案
...
TL;DR: COPESS软体机器人平台通过可调晶格层同时调节机械柔顺性和传感性能，实现自适应物体操控和局部感知


<details>
  <summary>Details</summary>
Motivation: 现有表面操控系统缺乏处理各种物体所需的柔顺性和触觉反馈，需要能够进行温和、精确操作的软体机器人平台

Method: 设计COPESS系统，集成电感传感器，采用可调晶格层同时调制机械柔顺性和传感性能，通过调整晶格几何形状来定制刚度和传感器响应

Result: 实验表明，仅通过调整晶格密度（7%到20%），就能显著改变灵敏度和操作力范围（分别约-23倍和9倍）

Conclusion: 该方法为创建自适应传感表面提供了蓝图，实现了机械和感官特性的协同优化，支持被动但可编程的精细操控
...
TL;DR: 该研究开发了一个包含仿真、分布式学习框架和物理机器人的综合机器人系统，提出了用于高效训练合作与竞争策略的强化学习技术，并引入OODSI方法缓解仿真到现实的迁移差距，在实验中提升了20%的性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体深度强化学习在游戏智能体开发方面取得进展，但如何高效训练集体机器人并将学习策略迁移到现实应用仍是开放研究问题。需要解决多智能体仿真到现实的迁移挑战。

Method: 1) 开发综合机器人系统（仿真、分布式学习框架、物理机器人组件）；2) 提出并评估用于高效训练合作与竞争策略的强化学习技术；3) 引入OODSI（Out of Distribution State Initialization）方法缓解仿真到现实的差距。

Result: OODSI方法将仿真到现实的性能提升了20%。通过多机器人汽车竞争游戏和现实环境中的合作任务实验证明了方法的有效性。

Conclusion: 该研究提出的综合机器人系统和OODSI方法有效解决了多智能体强化学习在仿真到现实迁移中的挑战，为集体机器人的高效训练和实际应用提供了可行方案。
...
TL;DR: 该论文提出了"相遇驱动信息扩散"问题，研究机器人在只能通过相遇交换信息且无法安排会面的情况下的信息传播模型


<details>
  <summary>Details</summary>
Motivation: 研究机器人在受限通信环境下的信息扩散问题，特别是在只能通过随机相遇交换信息且无法主动安排会面的场景

Method: 提出一个基于第一原理的两层信息扩散模型：微观模型基于"平均自由程"概念的推广，宏观模型捕捉信息扩散的全局动态

Result: 通过广泛的机器人模拟验证模型，考虑了群体规模、通信范围、环境大小和不同随机运动机制等参数

Conclusion: 讨论了该模型对信息扩散算法设计的影响，为EDID问题中的存储和路由算法设计提供了理论基础
...
TL;DR: HALO是一个统一的视觉-语言-动作模型，通过具身多模态思维链推理，结合文本任务推理、视觉子目标预测和动作预测，在机器人操作任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在长视野或分布外场景中表现不佳，缺乏明确的多模态推理机制和对动作影响下世界演变的预测能力。虽然已有工作引入了文本思维链或视觉子目标预测，但仍缺乏统一的人类式推理框架。

Method: 提出HALO模型，采用具身多模态思维链推理框架，包含文本任务推理、视觉子目标预测和EM-CoT增强的动作预测。使用混合Transformer架构，将语义推理、视觉前瞻和动作预测解耦为专门专家，同时支持跨专家协作。开发了自动化数据合成管道和精心设计的训练方案。

Result: HALO在模拟和真实环境中均表现优异，在RoboTwin基准上超越基线策略34.1%。所有提出的训练方案和EM-CoT设计组件都有助于提高任务成功率。在激进未见环境随机化下表现出强大的泛化能力。

Conclusion: HALO通过统一的具身多模态思维链推理框架，显著提升了VLA模型在机器人操作任务中的性能，特别是在长视野和分布外场景中，展示了强大的泛化能力和实际应用潜力。
...
Method: 提出ActionReasoning框架，利用LLM中已有的物理先验和现实世界知识，通过多智能体架构进行结构化。在砖块堆叠案例研究中，将准确测量的环境状态序列化后输入多智能体LLM框架，生成物理感知的动作计划。

Result: 实验表明，提出的多智能体LLM框架能够实现稳定的砖块放置，同时将工作重心从低级的领域特定编码转移到高级工具调用和提示工程，展现了更广泛泛化的潜力。

Conclusion: 这项工作通过将物理推理与LLM集成，为机器人操作中感知与执行的桥接提供了有前景的方法，展示了LLM驱动框架在机器人任务中的潜力。
...
TL;DR: 提出一种利用多分辨率体素地图的任意角度路径规划方法，结合了任意角度规划的最优性和完备性，同时通过多分辨率表示克服了搜索方法的计算可扩展性问题。
...
Method: 提出一种任意角度规划方法，通过连接障碍物角落的直线段来寻找最优路径。该方法利用多分辨率体素地图表示，结合了任意角度规划的最优性和完备性，同时通过多分辨率结构克服了传统搜索方法的计算瓶颈。

Result: 在真实和合成环境中的大量实验表明，该方法在解质量和速度方面表现优异，甚至优于基于采样的方法。该方法在保持最优性和完备性的同时，显著提高了计算效率。

Conclusion: 该方法提供了一种高效、最优且完备的路径规划解决方案，特别适用于大规模复杂环境。框架已开源，供机器人学和规划社区进一步研究和应用。
...
TL;DR: Squint是一种视觉软演员-评论家方法，通过并行仿真、分布评论家、分辨率压缩等技术，在单GPU上15分钟内训练出机器人操作策略，比现有视觉离策略和同策略方法训练更快。


<details>
  <summary>Details</summary>
Motivation: 视觉强化学习在机器人应用中很有前景但计算昂贵：离策略方法样本效率高但训练慢；同策略方法可并行化但浪费样本。现有方法在状态控制中已实现快速训练，但扩展到视觉领域仍面临高维图像输入带来的训练动态复杂、存储和编码开销大等挑战。

Method: 提出Squint视觉软演员-评论家方法，采用并行仿真、分布评论家、分辨率压缩（squinting）、层归一化、优化的更新-数据比率调整，以及高效的实现优化。

Result: 在SO-101任务集（ManiSkill3中的8个操作任务，含重度域随机化）上评估，大多数任务在6分钟内收敛，单RTX 3090 GPU上15分钟完成训练，并实现从仿真到真实SO-101机器人的迁移。

Conclusion: Squint通过技术创新实现了比现有视觉离策略和同策略方法更快的训练速度，成功解决了视觉强化学习中的计算效率问题，并在仿真到真实迁移中验证了有效性。
