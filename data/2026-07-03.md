...
TL;DR: 本文提出WaveLander，一种基于强化学习的分层控制框架，将无人机在波浪扰动海洋平台上的自主着陆问题分解为高层垂直速度决策（RL策略）和底层飞行稳定控制，在随机波浪条件下实现了鲁棒的着陆性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 无人机在波浪扰动的海洋平台上自主着陆面临平台随机运动、姿态时变和触地条件不确定等挑战，现有基于模型的方法需要精确运动预测和在线优化，而端到端学习方法训练复杂度高且可解释性有限。

Method: 提出WaveLander分层控制框架：上层RL策略将平台相对观测映射为标量垂直速度参考指令，下层常规飞控负责姿态稳定和横向跟踪，将动态平台着陆问题简化为低维、时序感知的控制问题。

Result: 在随机波浪诱导平台运动仿真下，WaveLander实现了鲁棒的着陆性能，并能泛化到未见过的扰动条件。

Conclusion: 分层学习控制方法为海洋无人机回收提供了一种有效方案，能够在不依赖显式切换规则的情况下实现平滑着陆行为。
...
TL;DR: 提出了一种基于视觉语言模型（VLM）的自适应机器人群体陪伴方法，通过语义推理实现自然陪伴，在成功率上提升15%，碰撞率降低25%。


<details>
  <summary>Details</summary>
Motivation: 人类群体通常不遵循固定队形，给机器人维持自然陪伴行为带来挑战，需要开发能够理解群体动态的适应性方法。

Method: 首先检测群体成员，感知模块生成交互群体空间的视觉表征作为VLM输入，再结合模型预测路径积分（MPPI）控制器确保稳定性与安全性。

Result: 在五个场景的实验中，成功率提升15%，碰撞率降低25%；用户研究表明生成的陪伴行为被认为是自然且 socially appropriate 的。

Conclusion: 基于VLM的群体陪伴方法能够有效提升机器人陪伴人类群体的自然性与安全性，优于基线方法。
...
TL;DR: 本文系统综述了ROS 2中间件架构，从时间（时间可预测性）、空间（物理拓扑抽象化）和状态（上下文连续性）三个结构维度分析了分布式机器人通信中间件在无线资源受限环境下的架构局限与权衡。


<details>
  <summary>Details</summary>
Motivation: 现有ROS 2中间件（DDS和Zenoh）在动态、资源受限的无线环境中存在结构性限制，但缺乏系统性的分析框架来揭示这些局限的根本原因。

Method: 采用系统性综述方法，提出时间-空间-状态三维概念框架，通过对中间件发现、数据交换和状态管理等机制的深入分析，梳理现有实现和研究的结构权衡。

Result: 在无线受限条件下，空间抽象会掩盖网络波动、削弱时间保证；状态连续性机制引入的计算与网络开销会与时间关键通信竞争资源，揭示了结构维度间的本质权衡。

Conclusion: 通过综合架构模式并识别当前建模方法的不足，为构建鲁棒且可扩展的机器人中间件架构提供了系统性的研究路线图。
...
TL;DR: 本文提出了CommonRoad-Game，一个轻量级的人机协同仿真框架，支持实时人类参与的运动规划器测试和驾驶行为分析，具有多线程架构和稳定的时间同步机制。


<details>
  <summary>Details</summary>
Motivation: 现有仿真平台大多依赖记录数据集、缺乏实时人机交互接口、与自动驾驶生态系统集成较弱，且计算开销大，不适合快速原型开发。

Method: 提出了多线程架构与鲁棒同步机制，将仿真时间与墙钟时间对齐，实现自动驾驶与人类驾驶车辆之间的确定性和时间一致性交互；同时提供场景生成模块，记录驾驶日志以构建多样化的可复现测试场景。

Result: 实现了稳定的时间同步，支持可扩展的多智能体仿真，并能无缝集成CommonRoad兼容的运动规划器生成交互式驾驶场景。

Conclusion: CommonRoad-Game为早期自动驾驶研究提供了一个轻量级、灵活的人机协同仿真工具，有助于系统性评估运动规划器与人类驾驶行为的交互。代码已开源。
...
TL;DR: BIFROST通过跨域双模拟目标学习共享历史编码器，将仿真和现实中产生等价比长期行为的观测-动作序列映射到邻近的潜在状态，实现在潜在状态上训练的仿真策略零样本迁移到现实，有效处理视觉和动力学领域的双重差距。


<details>
  <summary>Details</summary>
Motivation: 现有sim2real方法通常将视觉差距和动力学差距各自通过独立的适应模块分别处理，然后组合或叠加使用。然而，仿真的初衷是仿真与现实之间存在共享结构——等价动作从等价配置出发产生等价的长期结果。作者希望从原始观测中识别并利用这种共享结构，实现策略的零样本迁移。

Method: 提出BIFROST方法，在成对的跨域数据上通过跨域双模拟目标学习共享历史编码器；该编码器将产生等价比长期行为的观测-动作序列映射到邻近的潜在状态（无论来自哪个领域）；然后在仿真环境的这些潜在状态上训练策略，直接零样本迁移到现实。

Result: 在sim2sim视觉导航、sim2real接触式操作任务和视觉伺服任务上，BIFROST实现了有效的迁移，而在视觉和动力学双重领域差距下，领域自适应和协同训练基线方法均失败。

Conclusion: BIFROST通过跨域双模拟方法捕捉仿真与现实的共享结构，能够有效处理视觉和动力学双重领域差距，实现零样本迁移，优于现有方法。
...
TL;DR: 提出SE(2)导航网格，一种编码偏航相关可通行性的多边形表征，配合ASA路径规划策略，在复杂多层环境中实现地面机器人的高效导航。


<details>
  <summary>Details</summary>
Motivation: 现有导航网格假设偏航不变的可通行性，不适用于非圆形机器人在受限空间中的导航；点云和体占据地图缺乏显式表面结构，稠密三角网格直接寻路计算代价过高。

Method: 用足迹掩码评估偏航相关的可通行性，构建偏航特异性图层的图结构并包含平动和转动连接；提出A*-String Pulling-A*（ASA）分层优化路径；支持从流式点云增量更新SE(2) NavMesh。

Result: 仿真中SE(2) NavMesh比经典NavMesh多捕获50%以上可通行区域；SE(2) NavMesh+ASA管线在受限环境中持续优于基于采样的基线；真实机器人实验验证了实时在线生成和成功导航。

Conclusion: SE(2) NavMesh有效解决了非圆形机器人在复杂多层环境中的可通行表征与路径规划问题，具有较好的实用性和泛化能力。
...
TL;DR: 本研究提出一种可重构摇杆-转向架机构，通过少量电机实现四轮/六轮切换，在四轮模式下依靠差速驱动实现零半径转弯，速度提升5倍以上且扭矩仅需17%，同时保持40cm台阶攀爬能力（平均6.4秒）。


<details>
  <summary>Details</summary>
Motivation: 传统摇杆-转向架机构虽具备优秀的越障能力，但转向灵活性差，尤其配备非转向抓地轮时转弯半径大、速度慢。现有改进方案往往需要大量增加执行器，导致成本、重量和控制复杂度上升。因此，作者希望设计一种既保持高台阶攀爬能力，又能以少量执行器实现高效转弯的机构。

Method: 在转向架关节处安装电机，通过主动摆动转向架实现四轮与六轮构型切换；在摇杆后端安装全向轮，四轮模式下基于差速驱动模型实现平滑转弯；制作原型机器人进行实验验证。

Result: 原型机器人实现零半径转弯，转弯速度是传统六轮非转向摇杆-转向架机构的5倍以上；总平均轮扭矩仅需传统机构的17%；成功攀爬40cm台阶，平均耗时6.4秒。

Conclusion: 可重构摇杆-转向架机构在显著提升转向效率和降低能耗的同时，并未牺牲台阶攀爬能力，验证了该设计在兼顾机动性与越障性能方面的有效性。
...
TL;DR: 本文提出了一种基于多速率非线性模型预测控制（MR-NMPC）的分层规划与控制框架，使四足机器人在受限环境中借助墙壁支撑实现混合双足运动。高层MR-NMPC同时规划接触点与质心/姿态轨迹，低层全身控制器（WBC）跟踪参考。仿真验证表明，该方法在崎岖地形和高速度下的成功率比传统方法高2.9倍。


<details>
  <summary>Details</summary>
Motivation: 四足机器人在受限环境中进行混合双足运动时，实时轨迹优化面临巨大挑战：控制器需同时规划接触点与机器人连续轨迹，并处理非线性动力学、单边接触约束、欠驱动及动力学切换特性。

Method: 高层采用MR-NMPC，基于单刚体动力学模型同时规划离散接触点轨迹与连续质心/姿态轨迹；低层采用基于虚拟约束和二次规划的非线性全身控制器，执行全阶动力学并跟踪高层参考。

Result: 数值仿真表明，该框架使Unitree A1四足机器人在粗糙地形、外部扰动下实现稳健的壁辅助双足运动。相比传统MPC加启发式落足策略，MR-NMPC在不规则地形高速运动中的成功率提高2.9倍。

Conclusion: 提出的MR-NMPC框架通过将接触点规划集成到多速率最优控制中，相比启发式落足策略提升了动态稳定性，为四足机器人在复杂受限环境中的运动控制提供了有效方案。
...
TL;DR: 本文提出了一种结合全局与局部路径规划器的新型无人水面艇避障方法，全局规划器融合了Grassfire、改进Grassfire和新版概率路线图三种算法，局部规划器则基于障碍物相对运动方向的高层决策逻辑，引导USV从障碍物后方绕行而非并行等待，仿真验证了方法的有效性并与D*算法进行了对比。


<details>
  <summary>Details</summary>
Motivation: 现有无人水面艇在固定障碍物实时避碰方面已有众多方法，但在移动障碍物（如船只、游泳者等）避碰方面仍面临挑战，且现有方法（如D*算法）往往不引导USV从障碍物后方绕行，缺乏更优的规避策略。

Method: 全局路径规划器融合了三种算法：文献中已知的Grassfire、一种Grassfire的新改进版，以及一种新型且更直观的概率路线图版本。局部路径规划器采用基于障碍物相对USV全局路径运动方向的高层决策逻辑，系统引导USV从障碍物后方绕行而非并行等待。

Result: 仿真结果验证了所提方法的有效性，并与D*算法及其他动态路径规划系统进行了对比分析。

Conclusion: 本文提出的混合路径规划方法能够有效解决USV在固定和移动障碍物环境下的实时避障问题，尤其是通过引导USV从移动障碍物后方绕行，提供了比现有方法更优的规避策略。
...
TL;DR: AutoSERL是一个利用单次演示自动干预真实世界机器人强化学习的框架，在6个接触密集型操作任务中优于多基线方法，达到100%成功率。


<details>
  <summary>Details</summary>
Motivation: 机器人策略学习面临数据收集成本高和奖励设计困难的问题，现有方法要么需要大量演示，要么依赖持续人工干预。

Method: 提出三机制框架：滑动窗口干预机制持续引导探索防止局部最优和安全偏离；安全恢复机制通过预定义轨迹恢复点检测纠正失败状态；干预终止准则自动禁用引导以保持策略探索优势。

Result: 在6个接触密集型操作任务（插入、悬挂、铰链）上，AutoSERL优于SERL（20个演示）、行为克隆和MILES，匹配HIL-SERL性能，插入任务100%成功率，且对位置变化鲁棒性更强。

Conclusion: 仅需单次演示即可实现在真实机器人上的自动化强化学习，显著降低了对演示数量和人工干预的依赖。
...
TL;DR: 提出TacImag框架，通过从视觉和本体感觉预测触觉信号（触觉想象），使机器人无需实际触觉传感器即可获得触觉知识，实验表明该方法能有效提升操作性能。


<details>
  <summary>Details</summary>
Motivation: 触觉传感在实际部署中受限于硬件脆弱性、校准困难和维护负担，因此探索机器人能否在不使用触觉传感器的情况下获得触觉知识的益处。

Method: TacImag框架：从视觉和本体感觉预测触觉观测（包括力场和触觉图像），利用生成的信号引导操作策略；通过配对视觉-触觉示教训练，测试时仅需视觉观测。

Result: 在6个仿真和4个真实世界任务中，想象触觉持续提升操作性能。真实实验中，想象力场提升接触敏感任务44.4%，想象触觉图像提升纹理敏感任务23.3%。

Conclusion: 触觉想象并非简单恢复缺失的触觉测量，而是一种接触感知的监督形式，能将微妙的视觉交互线索转化为策略更易利用的表征；触觉表征与任务需求的匹配程度决定了触觉想象的有效性。
...
TL;DR: 本文提出CoRe框架，将奖励分解为形式奖励(FR)和残差奖励(RR)，利用视觉语言模型(VLM)自动构建与人类偏好对齐的奖励，在10个仿真和5个真实机器人操作任务中取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 手工设计的奖励函数难以指定且可能导致次优策略，而从偏好中学习的奖励存在效率低和训练不稳定的问题，因此需要一种无需人工参与、高效且与偏好对齐的奖励设计方法。

Method: 将奖励分解为形式奖励(FR)和残差奖励(RR)两部分，提出CoRe混合框架：FRM模块利用VLM根据任务知识和偏好反馈迭代设计和优化FR；RRM模块从视频级偏好中学习RR，利用VLM生成偏好标签并捕捉FR未覆盖的细微奖励。

Result: 在10个仿真机器人操作任务和5个真实世界任务上的实验表明，CoRe在策略学习效果和效率上均优于现有方法。

Conclusion: CoRe通过形式奖励与残差奖励的协同作用，实现了无需人工干预的高效、与偏好对齐的奖励自动构建，显著提升了机器人操作任务的策略学习性能。
...
TL;DR: 提出了一种安全约束的感知-控制一体化无人机导航框架，使用轻量级网络和基于拉格朗日的安全PPO算法，在稀疏感知和动态约束下实现高效、安全的自主导航。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法缺乏显式安全机制，导致不安全探索和危险行为；安全RL方法通过投影到安全动作集可能引入不稳定；学习型方法依赖密集输入或大网络，计算负担重，难以轻量化部署。

Method: 1) 轻量级网络使用非对称和深度可分离卷积编码稀疏观测为碰撞风险感知特征；2) 将任务建模为约束马尔可夫决策过程(CMDP)并采用分层控制架构；3) 基于拉格朗日的安全PPO算法进行求解；4) 引入课程学习提升训练稳定性。

Result: 在变障碍密度和飞行速度的实验中，相比现有RL基线方法，实现了更高的成功率、更好的安全性和更高的效率。

Conclusion: 所提框架有效解决了无人机在密集环境中高速飞行时的安全导航问题，兼顾了轻量化部署需求与安全性能。
...
TL;DR: VLA-Corrector是一种轻量级纠错推理框架，通过潜在空间视觉监控（LVM）检测视觉动态偏差，并触发在线梯度引导（OGG）进行自适应重规划，在不修改骨干网络权重的情况下，解决动作分块VLA策略中开环执行误差累积问题，提升长期、接触密集型机器人操作的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有动作分块VLA策略采用"预测后盲目执行"的开环方式，在接触密集型物理交互中，局部微小扰动会在开环盲区内快速放大，导致误差累积和任务失败，缺乏闭环反应能力。

Method: 提出VLA-Corrector框架，包含两个核心模块：（1）潜在空间视觉监控器（LVM），持续比较预测与实际的视觉特征演化，在线检测动态偏差；（2）在线梯度引导（OGG），检测到持续偏差后触发截断事件，丢弃剩余失效动作并执行纠错重规划。该机制实现了事件触发的自适应动作时域。

Result: VLA-Corrector在不重训练VLA骨干网络的情况下，能中断复合误差，保留动作分块的大部分效率优势，显著提升了长期、接触密集型机器人操作任务的鲁棒性。

Conclusion: VLA-Corrector通过检测-纠错机制缓解了静态时域在执行鲁棒性与策略调用频率之间的权衡，可集成到不同VLA模型中，为动作分块策略提供了有效的闭环补偿方案。
...
TL;DR: DL-SLAM提出了一种双层级概率框架，结合语义与几何信息计算动态概率，实现了无伪影的静态地图构建，在SLAM跟踪精度上提升高达13%。


<details>
  <summary>Details</summary>
Motivation: 现有动态SLAM方法要么直接丢弃动态物体（忽略瞬态静态物体的几何约束价值），要么使用逐像素不确定性图但会将瞬态静态物体错误地纳入静态地图导致持久伪影，且纯几何方法导致边界模糊。本文旨在解决这些问题。

Method: 提出双层级概率框架：1）结合语义和几何信息计算像素级动态概率图；2）将像素级概率提升到3D并聚合得到每个实例的物体级动态概率；3）利用物体级概率对动态高斯体进行分类剪枝，生成无伪影静态地图；4）静态地图再为像素级概率提供几何一致的精细化指导。

Result: 跟踪精度提升高达13%，生成高保真语义地图，优于现有方法。

Conclusion: 双层级概率框架有效解决了瞬态静态物体的利用与静态地图纯净性之间的矛盾，验证了语义与几何信息融合在动态SLAM中的优越性。
...
TL;DR: 本文提出SPLC算法，通过社会偏好反馈机制自动生成偏好数据，消除离线强化学习中繁琐的奖励函数设计，显著提升人群机器人导航的社会合规性表现。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习在人群机器人导航中潜力巨大，但行人运动复杂性导致设计有效奖励函数促进社会合规行为始终是难题，因此需要一种无需详细奖励设计的方法。

Method: 提出Social Preference Learning for Crowd Robot Navigation (SPLC)算法，核心是引入社会偏好反馈机制，通过原则性偏好评估标准自动生成偏好数据，显式考虑行人动力学复杂性，减轻奖励偏差并系统量化广泛社会规范。

Result: 与离线RL方法集成的广泛实验表明，SPLC在各项标准性能指标上持续优于最先进基线；TurtleBot4真实世界实验进一步验证了其在人机共存环境中的有效性。

Conclusion: SPLC通过社会偏好学习有效消除了对详细奖励设计的需求，为离线强化学习在人群机器人导航中提供了一种实用且有效的解决方案。
...
TL;DR: 提出了PhysMani框架，将物理原理驱动的3D高斯世界模型与未来感知动作策略模型相结合，用于在非结构化3D环境中操控快速移动目标，在仿真和真实机器人实验中均优于强基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型和世界模型在处理非结构化3D环境中快速动态目标时，缺乏精确的3D几何建模和物理上有意义的预测能力。

Method: 1) 物理原理驱动的3D高斯世界模型，通过在线优化学习无散度高斯速度场，实现快速且物理可信的未来动态预测；2) 未来感知动作策略模型，通过可学习token的交叉注意力模块整合预测的3D场景未来动态。

Result: 在PhysMani-Bench（包含16个任务的动态操控基准）上，在仿真和真实机器人实验中均取得了优于强基线的成功率。

Conclusion: 该框架有效提升了在非结构化3D环境中操控快速动态目标的性能，验证了物理约束的世界模型对未来预测和动作策略的促进作用。
...
TL;DR: 本文提出OCD SLAM，一种在ORB-SLAM2基础上联合处理动态物体和动态特征点的动态立体视觉SLAM框架，通过引入"交叉视差"概念进行几何特征级运动分析，并结合3D目标检测与卡尔曼滤波进行物体级运动分类，在KITTI数据集上显著提升了轨迹精度。


<details>
  <summary>Details</summary>
Motivation: 传统视觉SLAM系统在动态环境中因静态世界假设而常因运动物体导致位姿估计失败，现有方法难以有效区分动态与静态场景元素。

Method: 提出基于视差与交叉视差差异的几何方法检测动态特征点，集成SMOKE 3D目标检测与卡尔曼滤波进行物体级运动分类，实现特征级与物体级联合运动分析。

Result: 在KITTI Odometry和KITTI Raw数据集上，OCD SLAM相比ORB-SLAM2及多种先进动态SLAM方法在轨迹精度上取得显著提升，消融实验验证了交叉视差模块的有效性。

Conclusion: OCD SLAM通过特征级与物体级双重动态分析机制，有效解决了动态环境下的SLAM鲁棒性问题，且交叉视差方法能检测到纯3D目标检测遗漏的动态特征点。
...
TL;DR: 提出了一种自适应强化学习方法，通过单一策略实现水面无人艇的零样本跨平台轨迹跟踪，无需微调即可部署到不同平台上。


<details>
  <summary>Details</summary>
Motivation: 现有控制器大多针对单一平台设计，无法直接在不同动力学特性的水面无人艇间迁移部署。

Method: 采用教师-学生架构，让策略在部分可观测条件下通过交互历史推断平台动力学的潜在表征；在随机动力学仿真中训练，零样本部署到真实平台。

Result: 在两种不同真实平台上，自适应策略相比非自适应基线方法位置平均绝对误差降低高达58%，接近平台专用调优控制器的跟踪精度。

Conclusion: 该方法仅依赖简单解析动力学模型而非高保真水动力仿真器，即可实现有效的跨平台零样本迁移部署。
...
TL;DR: 本文提出Guided Action Flow框架，通过在推理时使用学习到的动作块评判器指导预训练SmolVLA策略的逆向流采样，无需重新训练基础策略。在LIBERO任务上，单任务评判器成功率提升14%，多任务评判器提升10%，验证了Q引导推理的可行性，但评判器泛化与不确定感知引导仍是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 流动匹配视觉-语言-动作策略通过迭代传输过程生成机器人动作块，这为在不重新训练基础策略的情况下进行测试时引导创造了机会。研究者旨在利用这一机会，设计一种推理时框架来提升预训练策略的性能。

Method: 保持预训练SmolVLA策略冻结，学习一个动作块评判器，该评判器从真实成功与失败轨迹中训练，可条件于冻结SmolVLA语言路径的任务描述特征，在采样过程中仅通过动作梯度进行引导。

Result: 在LIBERO操作任务上，单任务评判器将成功率从68.0%提升至82.0%（一个种子窗口）和从82.0%提升至86.0%（另一个种子窗口）；多任务描述评判器将验证成功率从46.0%提升至56.0%，但锁定保留测试集提升较小（65.0%至67.5%）。

Conclusion: Q引导推理对于冻结流动匹配VLA策略是可行的，但评判器泛化能力与不确定感知引导仍是主要瓶颈。
...
TL;DR: 本文提出了Way of Water系统，一个将自主水面艇编队作为分布式编舞平台的网络物理架构。通过浏览器端的时间线合成创作范式（Way of Water Studio），结合序贯凸规划轨迹生成和模型预测控制，实现了非编程艺术家对高性能水上机器人的可访问编舞，并在苏黎世湖和威尼斯双年展成功部署验证。


<details>
  <summary>Details</summary>
Motivation: 开放水域的机器人编舞受到非线性流体动力学、环境干扰和非线性系统动力学的严重挑战，现有方法局限于水面二维像素表达，缺乏面向非编程艺术家的三维水体编舞创作工具。

Method: 提出Way of Water Studio浏览器端时间线合成创作范式，将编队视为类似DAW的乐器进行音乐响应编舞；采用序贯凸规划（SCP）进行轨迹生成，模型预测控制（MPC）进行扰动抑制；构建完整网络物理栈：定制全向底盘、状态估计与控制栈、LTE/MQTT舰队链路及RTK-GPS时间同步。

Result: 在苏黎世湖18艘船只的《天鹅湖》演绎和威尼斯双年展8艘船只的演示中成功验证系统有效性。

Conclusion: 该工作为流体机器人编队的设计与部署建立了基础参考，将水上机器人编舞从二维平面扩展至三维体积域，并显著降低了艺术家的使用门槛。
...
TL;DR: Bridge-WA提出一种轻量级世界-动作框架，通过从冻结的未来变化教师模型中蒸馏出未来token、变化图和运动流图三个紧凑先验，在无需密集未来图像生成的情况下，提升机器人操作任务的成功率、鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型依赖大规模生成式世界模型或密集未来 rollout，计算成本高且将能力浪费在与控制弱相关的视觉细节上，因此需要一种更高效、关注动作相关场景变化的轻量级方法。

Method: 从冻结的未来变化教师模型中蒸馏出三个紧凑先验（未来token、变化图、运动流图），通过多源注意力记忆和时空偏置将先验注入动作Transformer，推理时移除教师模型。

Result: 在VLABench、RoboTwin2.0、LIBERO-Plus及真实机器人评估中，Bridge-WA在任务成功率、进展和鲁棒性上均有提升，尤其在分布外视觉偏移下增益明显。

Conclusion: 通过聚焦动作生成于场景如何变化（而非背景、光照等无关因素），Bridge-WA在不依赖部署时密集未来图像生成的前提下，实现了更好的泛化能力。
...
TL;DR: 本文提出了一种名为“执行器现实塑造”的新范式，通过控制物理执行器模拟理想化的仿真参考动力学，实现机器人策略从仿真到现实零样本迁移，无需调整仿真器或任务层面微调。


<details>
  <summary>Details</summary>
Motivation: 仿真到现实的迁移中，理想执行器动力学与物理电机非线性行为之间存在显著差异，现有方法（系统辨识、域随机化、学习执行器模型）均试图修改仿真器来匹配现实，但效果有限。

Method: 为每个关节配备二自由度前馈-反馈控制器，将参考响应塑造与鲁棒镇定解耦，使物理执行器的闭环行为匹配仿真中采用的理想化二阶参考动力学，从而为强化学习策略提供标准化的执行器接口。

Result: 在单关节高减速比伺服系统（外部负载下）、7-DOF机械臂到达任务中，该方法显著降低了仿真-现实跟踪误差，并提升了零样本任务性能；在轮腿式机器人坡面行驶和人形机器人行走中验证了零样本迁移能力。

Conclusion: 执行器现实塑造可作为一种跨多样化硬件平台的可复用接口，有效解决机器人学习中的仿真-现实迁移问题。
...
TL;DR: 提出CoFL-S，一种低层级视觉-语言-动作框架，通过预测语言条件光流场生成连续轨迹，在连续时间Habitat基准和真实世界部署中优于动作token和动作chunk基线。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言导航研究多聚焦高层指令推理、记忆与地图构建，而对低层级动作表示探索不足。

Method: 提出CoFL-S框架，预测语言条件光流场并滚动生成连续轨迹；将VLN-CE整段episode转换为帧级局部监督，包含对齐的子指令与密集流场目标；引入连续时间Habitat基准，通过共享速度命令控制器隔离低层动作接口。

Result: 在匹配编码器和训练设置下，CoFL-S在连续时间Habitat基准中不同规划频率下均优于动作token和动作chunk基线；零样本真实世界部署同样展现优势。

Conclusion: CoFL-S的低层级动作表示在仿真和真实场景中均有效，为VLN中连续轨迹生成提供了新范式。
...
TL;DR: NEUROSYMLAND是一个神经符号融合的着陆点评估系统，结合轻量级感知与显式符号推理，在72个模拟场景中优于四个基线方法，并通过100次硬件在环实验验证了边缘部署可行性。


<details>
  <summary>Details</summary>
Motivation: 现有基于纯视觉学习的着陆点评估方法在多变地形下性能退化严重，且缺乏安全决策的可解释性，亟需一种兼具鲁棒性和透明度的解决方案。

Method: 通过构建概率语义场景图（PSSG）表示环境，利用符号约束（地形平坦度、障碍物间距、空间一致性）进行结构化推理，实现感知不确定下的安全评估。

Result: 模拟实验中61/72次成功评估，优于基线（37-57次）；硬件在环实验表明符号推理仅占端到端延迟的极小部分，主要计算开销来自感知与PSSG构建。

Conclusion: 该方法在边缘受限硬件上同时实现了鲁棒性、可解释性和低延迟，证明了神经符号系统在无人机自主降落中的实用潜力。
...
TL;DR: 本文基于DJI Tello无人机平台，集成面部检测、面部识别和单目深度估计三大AI功能，构建了一个低成本、开源的个人助理无人机系统，并通过网页界面实现实时控制与监控。


<details>
  <summary>Details</summary>
Motivation: 现有商用无人机系统成本高、封闭性强，缺乏面向个人助理场景的轻量级、低成本、开源解决方案，因此本文旨在开发一个易于获取且功能完备的智能无人机系统。

Method: 采用DJI Tello无人机作为硬件平台，设计模块化架构，集成面部检测、面部识别和单目深度估计三种AI功能；使用轻量级神经网络模型在嵌入式系统上进行推理；通过Python后端处理视觉数据，前端网页界面实现无人机控制和实时视频监控。

Result: 系统在真实环境下表现出稳健性能，能够实现人员追踪、室内扫描和基于虚拟传感器的自主线跟踪等功能，验证了AI技术在实时机器人系统及受限硬件上的可行性。

Conclusion: 本工作为未来在军事、救援和监控等任务中的自主无人机研究奠定了基础，证明了在低成本硬件上部署先进AI技术的可行性。
...
TL;DR: 本文提出通过混合静态与动态视角的数据策略，有效减少VLA模型在空间泛化中的捷径学习问题，显著提升其对未见相机姿态与物体配置的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在空间泛化上表现脆弱，简单增加视角数量无法解决问题，模型容易陷入捷径学习（如依赖物体间或相机与机器人基座的固定相对位姿），而非学习真实的空间关系。

Method: 采用双臂机器人平台，一臂执行操作任务，另一臂作为移动环境相机。系统评估三种数据分布模式：固定视角、多固定视角、移动视角。提出混合策略：连续相机运动与多样化静态视角相结合。

Result: 混合策略显著优于其他方案：有效减少虚假相关性，保持训练稳定性，在未见相机位姿和物体配置上实现良好泛化。所有评估模型（ACT、Diffusion、Pi0、Gr00t）均从该策略中获益。

Conclusion: 对捷径学习的易感性和空间泛化的困难是跨不同VLA架构的共性问题，而本文提出的混合数据策略能有效缓解这一普遍困境。
...
TL;DR: 提出HEFT框架，通过特权运动引导(PMG)和窗口化有效载荷课程(WPC)训练，实现175cm/65kg全尺寸人形机器人在24kg负重下的运动跟踪与遥操作。


<details>
  <summary>Details</summary>
Motivation: 现有运动跟踪与遥操作框架多验证于小型平台或无真实负载交互场景，全尺寸人形机器人在大惯性和紧平衡约束下对噪声/漂移/重定向误差极度敏感，且负载潜力未被充分利用。

Method: 1. 特权运动引导(PMG)：从可部署但有噪声的VR参考中学习，重构物理合理的参考运动；2. 窗口化有效载荷课程(WPC)：结合专家引导的负载上限，逐步训练获取鲁棒的重负载跟踪能力。

Result: 在L7人形机器人(175cm/65kg)上实现最高24kg负载下的转身、前进/后退行走、蹲起等运动跟踪。

Conclusion: HEFT框架有效解决了全尺寸人形机器人在重负载遥操作中的噪声鲁棒性与负载利用率问题，实现了实际部署。
...
TL;DR: 提出ACID框架，通过循环动作一致性约束（前向预测+逆向恢复）提升决策时轨迹规划的可实现性，在多种任务中减少计算量并保持精度。


<details>
  <summary>Details</summary>
Motivation: 标准规划代价仅检查预测终端状态与目标的接近程度，忽略了中间轨迹的可实现性，导致规划轨迹看似合理但实际执行时偏离。

Method: 提出ACID框架，引入循环动作一致性：用逆向动力学模型从预测状态反推动作，检查是否与原始动作一致，并通过尺度不变自适应权重将残差纳入规划代价。

Result: 在4种动作条件世界模型、6个任务（刚体/可变形操作、关节控制、视觉导航）上，ACID一致提升规划性能，并在匹配基线精度的同时显著减少规划计算量。

Conclusion: 通过约束轨迹的可逆性（前向-逆向一致性），能有效提升规划的现实可执行性，降低计算开销。
...
TL;DR: 本文提出LIME，一种语言条件相机运动生成模型，能从被动第一人称视频中学习，根据自然语言意图预测相机相对位姿，实现意图驱动的主动感知。


<details>
  <summary>Details</summary>
Motivation: 自主机器人常需移动相机以观察目标、揭示遮挡区域或响应用户意图，但语言条件相机运动作为一类动作尚未被充分探索。

Method: 从第一人称视频挖掘多意图相机运动监督信号（自然语言意图+SE(3)目标位姿）；提出LIME模型，结合自回归观测增益输出与连续流匹配位姿头，联合预测下一视图应揭示什么并表达多假设目标视角。

Result: 实验表明LIME能从被动人类视频学习主动选择相机位姿，并在下游机器人任务中有效实现意图感知的主动感知。

Conclusion: 将普通第一人称视频转化为意图感知主动感知的监督信号，使模型学会从被动数据中生成主动相机运动。
...
TL;DR: WorldSample是一个面向真实机器人强化学习的数据增强框架，通过物理 rollout、世界模型生成和策略改进之间的真实-合成闭环，结合策略节奏学习（PPL）来调节训练过程，在操作任务中将策略成功率提高28%、训练步骤减少59%。


<details>
  <summary>Details</summary>
Motivation: 真实机器人强化学习面临交互成本高的问题，每次物理rollout都代价昂贵且只能反映一条行动-结果路径，限制了RL在真实机器人上的部署。

Method: 提出WorldSample框架，包含真实-合成闭环：基于真实rollout后训练世界模型生成高保真合成转移，并引入策略节奏学习（PPL）通过样本选择和调度来调节训练，平衡有用增强与值估计过高的风险。

Result: 在接触性、精细操作任务中，策略成功率提升28%，训练步骤减少59%；世界模型视觉保真度PSNR提升19.4dB，SSIM提升0.47。

Conclusion: WorldSample有效验证了真实-合成循环对策略和世界模型性能的双重提升，通过物理grounded的数据增强解决了真实机器人RL的高交互成本问题。
...
TL;DR: 本文提出任务无关预训练（TAP）框架，将VLA模型的学习解耦为物理能力（如何移动）和语义对齐（做什么）两部分，通过自监督逆动力学从廉价未标注交互数据中学习运动先验，再用少量专家数据实现语言对齐，在SIMPLER基准上达到百万级专家数据效果，并在真实机器人上展现出更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: VLA模型受限于专家示范数据稀缺，现有方法将物理技能习得与语义对齐两个目标混为一谈，导致对昂贵的语言标注数据过度依赖。

Method: 提出解耦假设（Decomposition Hypothesis），设计两阶段框架TAP：第一阶段通过自监督逆动力学目标从无标签交互数据（含离轨轨迹、自主探索数据）中学习可迁移的运动先验；第二阶段用极少量专家数据将这些先验与语言指令对齐。

Result: 在SIMPLER基准上，TAP使用数量级更少的标注数据达到百万级专家轨迹训练模型的效果，绝对性能提升10%；在真实WidowX平台上，面对相机扰动，TAP保持25%成功率，而大规模预训练基线降至0%。

Conclusion: 任务无关预训练能够产生鲁棒、可迁移的物理表示，为解决具身AI中数据瓶颈提供了可扩展的路径。
...
TL;DR: 提出一种仅使用3D方向向量和拦截器状态的学习控制策略，用于拦截入侵者，在最高10m/s速度下比使用简化质点动力学的基线方法平均提升30%。


<details>
  <summary>Details</summary>
Motivation: 现有深度强化学习方法假设可获取相对位置或距离信息，但在实际应用中仅能使用被动单目相机，无法直接获得这些信息。

Method: 利用可微分四旋翼动力学实现分析策略梯度方法，学习敏捷拦截控制策略。

Result: 在最高10m/s速度下，该方法比使用简化质点动力学的基线方法平均提升30%。

Conclusion: 提出的方法仅依赖方向向量即可实现有效的拦截控制，更符合实际应用场景。
...
TL;DR: 本文提出了QuadRocket——一种基于四旋翼的火箭原型平台，通过万向节将圆柱主体安装在四旋翼上方形成倒立摆系统，采用自适应反步控制和动态面姿态控制实现推力矢量控制，实验验证了其作为低成本推力矢量控制测试平台的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有运载火箭推力矢量控制（TVC）策略的验证通常成本高昂且风险巨大，缺乏低成本、低风险的实验测试平台。QuadRocket旨在填补这一空白，为先进TVC控制策略提供灵活且经济的验证手段。

Method: 将四旋翼与火箭主体的耦合系统建模为单轴对称刚体，采用约化姿态表示（两球面）显式利用轴对称性并解耦偏航；设计自适应反步控制器实现存在未知常值扰动下的几乎全局轨迹跟踪，通过控制点变换缓解非最小相位行为；将四旋翼视为推力矢量执行器，设计基于动态面的姿态控制器跟踪期望推力矢量，避免虚拟控制信号的显式微分。

Result: 仿真和室内运动捕捉场地的实验均验证了该平台能够实现精确的轨迹跟踪和有效的扰动补偿，确认了QuadRocket作为推力矢量控制机器人车辆测试平台的适用性。

Conclusion: QuadRocket成功提供了一种低成本、低风险的推力矢量控制验证平台，其控制架构在理论和实验中均表现出良好的跟踪性能和抗扰能力，为未来运载火箭类系统的控制策略研究提供了实用的测试工具。
...
TL;DR: CNeVA是一个可控的交通模拟代理框架，通过变分推断和整流流生成器实现逼真且可控的驾驶行为模拟，在Waymo数据集上取得了有竞争力的真实感表现。


<details>
  <summary>Details</summary>
Motivation: 现实交通模拟需要既能模仿真实日志行为、又能沿可解释轴进行控制的智能体，以实现变量隔离、边缘案例复现和自动驾驶系统安全测试。

Method: 提出CNeVA框架：1) 通过共轭变分更新从每通道折扣回报中推断高斯行为潜变量；2) 采用整流流轨迹生成器，基于混合通道掩码课程训练实现无分类器引导；3) 提出软资格门控，用平滑指数衰减替代硬二值阈值以保持梯度信号。

Result: 在Waymo Open Motion Dataset上，CNeVA达到了有竞争力的真实感基准，同时展现了高阶模仿模型缺乏的逐通道可控性。速度和加速度控制产生单调响应，安全控制在引入软资格门控后呈现单调且显著的效果。

Conclusion: CNeVA实现了可操控的地图合规性，同时验证了操控指标需与物理合理性护栏一并解读，以避免奖励黑客混淆。
...
TL;DR: 提出了Embodied.cpp，一个用于具身AI模型的便携式C++推理运行时，通过五层架构实现多速率执行、延迟优先推理和可扩展接口，在VLA和WAM模型上验证了高效部署和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有具身AI模型（VLA和WAM）的部署碎片化，依赖特定Python栈和后端假设，缺乏统一运行时；现有推理引擎主要为请求-响应服务设计，不满足具身部署的闭环控制、多速率执行、异构硬件批大小为1的延迟优先推理等需求。

Method: 通过分析代表性VLA和WAM模型的共享执行路径，设计五层架构：输入适配器、序列构建器、骨干执行、头插件和部署适配器。实现模块化多速率执行、延迟优先融合推理、可扩展算子与I/O支持，统一后端抽象以支持异构设备、机器人和模拟器。
...
TL;DR: VT-WAM是一种视觉-触觉世界动作模型，通过统一流匹配框架联合学习未来视觉预测、触觉变形预测和动作预测，在接触丰富的操作任务中达到71.67%的平均成功率。


<details>
  <summary>Details</summary>
Motivation: 接触丰富的操作需要模型对局部变形、压力、滑动和摩擦力做出反应，但这些触觉线索在时间上稀疏且视觉观测中难以捕捉，现有方法很少在动作生成过程中建模触觉变形动态。

Method: 提出VT-WAM，包含(1)非对称混合Transformer注意力机制，连接首帧视觉锚点与时间触觉动态；(2)接触门控的动作-视觉-触觉注意力引导，在接触阶段鼓励动作查询依赖触觉证据。

Result: 在6个真实世界接触丰富操作任务中达到71.67%平均成功率，比Fast-WAM高26.67%，比OmniVTLA高35.84%。
